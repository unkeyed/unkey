...
export const keywordResearchTask = task({
	...
	run: async (payload: { term: string }) => {
		...
		const searchResponse = await getOrCreateSearchResponse({query: searchQuery.query, inputTerm: searchQuery.inputTerm});
...
`
		const promptTitles = `Below is a list of titles separated by semicolons (';') from the top organic search results currently ranking for the term '${searchQuery.inputTerm}'.
		...
		==========
        ${searchResponse.serperOrganicResults.map((result) => `The title for the sourceUrl "${result.link}" (reference this url as the sourceUrl for the keyword) is: "${result.title}"`).join(";")}
		==========
		`;
		...
		const keywordsFromTitles = await generateObject({
			...
			schema: z.object({
				...
				keywordsWithBrandNames: z.array(z.object({keyword: z.string(), sourceUrl: z.string().url()})),
			}),
		});
		console.log(`3/5 - KEYWORDS FROM TITLES: ${keywordsFromTitles.object.keywordsWithBrandNames.length} keywords with brand names and ${keywordsFromTitles.object.keywords.length} keywords.`);
		...
		const THREE = 3;
		const topThreeOrganicResults = searchResponse.serperOrganicResults.filter(result => result.position <= THREE);
		const scrapedContent = await getTopResultsContent({urls: topThreeOrganicResults.map(result => result.link)});
		console.log(`4/5 - SCRAPED CONTENT: ${scrapedContent.length} results`);
		const context = scrapedContent.map((content) => {
		...
		console.log(`5/5 - KEYWORDS FROM HEADERS: ${keywordsFromHeaders.object.keywordsWithBrandNames.length} keywords with brand names and ${keywordsFromHeaders.object.keywords.length} keywords.`);
		...
		// NB: drizzle doesn't support returning ids in conjunction with handling duplicates, so we get them afterwards
		await db.insert(keywords).values(searchResponse.serperRelatedSearches.map((search) => ({
			inputTerm: searchQuery.inputTerm,
		...
		console.log(`✅ Keyword Research for ${payload.term} completed. Total keywords: ${insertedFromTitles.length + insertedFromHeaders.length + insertedRelatedSearches.length}`);
