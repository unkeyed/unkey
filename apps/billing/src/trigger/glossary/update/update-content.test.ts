...
export const runTestCase = task({
  ...
  run: async ({ testCaseName }: { testCaseName: string }) => {
    ...
    if (!testCase.expectedSuccess && result.ok) {
      ...
      safeSetMetadata(
        "reason",
        `Expected error "${testCase.expectedError || "unknown"}" but task succeeded`,
      );

      return {
        success: false,
        message: `Test failed: Expected error "${
          testCase.expectedError || "unknown"
        }" but task succeeded`,
        details: { result },
      };
    }
    ...
    if (!testCase.expectedSuccess && !result.ok) {
      ...
      return {
        success: false,
        message: `Test failed: Error message doesn't match expected. Got: "${errorMessage}", Expected to include: "${
          testCase.expectedError || "unknown"
        }"`,
        details: { error: errorMessage },
      };
