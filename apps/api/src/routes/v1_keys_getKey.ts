...
export const registerV1KeysGetKey = (gCtx: GlobalContext, app: App) => app.openapi(route,
  async (c) => {
    const authorization = c.req.header("authorization")!.replace("Bearer ", "")
    const rootKey = await gCtx.keyService.verifyKey(c, { key: authorization })
    if (rootKey.error) {
    ...
    let key = await gCtx.keyCache.get(c, keyId)
    if (!key) {
      key = await gCtx.db.query.keys.findFirst({
