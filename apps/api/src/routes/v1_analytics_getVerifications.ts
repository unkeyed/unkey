...
const route = createRoute({
  ...
  request: {
    query: z.object({
      apiId: z.array(z.string()).optional().openapi({
...
`
      ...
      externalId: z.array(z.string()).optional().openapi({
...
`
      ...
      keyId: z.array(z.string()).optional().openapi({
...
`,
      ...
      groupBy: z.enum(["key", "identity", "tags"]).or(z.array(z.enum(["key", "identity", "tags"]))).optional().openapi({
        ...
        "application/json": {
          schema: z.object({
            data: z.array(z.object({
              ...
              description: "Successful responses will always return an array of datapoints. One datapoint per granular slice, ie: hourly granularity means you receive one element per hour within the queried interval."
            })
...
export const registerV1AnalyticsGetVerifications = (app: App) =>
  app.openapi(route, async (c) => {
    ...
    * ourselves.
    */
    const groupBy = (Array.isArray(filters.groupBy) ? filters.groupBy : [filters.groupBy]).filter(Boolean)
