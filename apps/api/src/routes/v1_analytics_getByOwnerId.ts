...
export const registerV1AnalyticsGetByOwnerId = (app: App) =>
  app.openapi(route, async (c) => {
    const { ownerId, start, end } = c.req.query;
    ...
    if (keysList.some(({ key }) => key.workspaceId !== authorizedWorkspaceId)) {
      throw new UnkeyApiError({
        code: "UNAUTHORIZED",
    ...
    const verificationsFromAllKeys = await Promise.all(
      keysList.map(({ key, api }) => {
        return cache.withCache(c, "verificationsByKeyId", `${key.id}:${start}-${end}`, async () => {
