...
export class DurableObjectUsagelimiter {
  ...
  constructor(state: DurableObjectState, env: Env["Bindings"]) {
    this.state = state
    this.db = createConnection({ host: env.DATABASE_HOST, password: env.DATABASE_PASSWORD, username: env.DATABASE_USERNAME })

    const defaultFields = { durableObjectId: state.id.toString(), durableObjectClass: state.constructor.name }
    this.logger = env.AXIOM_TOKEN ? new AxiomLogger({
      axiomToken: env.AXIOM_TOKEN, environment: env.ENVIRONMENT, defaultFields
  ...
  async fetch(request: Request) {
    ...
    if (!this.key) {
      ...
      console.log("found key from origin", { key: this.key })
    }
