...
of money each month regardless of what they use. Especially when building out new unproven projects, it's essential to meet the developer where they are at: **Zero users**

As long as your customer is too small to generate revenue, they don't want to pay for your service and that's okay. 
Generous free tiers are a good solution for this, but after they upgrade, you shouldn't send them an insane bill for resources they're not even using. Pricing that scales with usage is preferrable for many, but not all, products.
...
This worked pretty well and Stripe take care of a lot of things, such as proration when subscriptions change.
...
This results in a materialized view, where there's t most one row per keyId and month, making it super cheap to query later.
...
We're still happy with the result, as it allows us to edit and tweak things as we go, whereas Stripe makes a ton of options non-editable. And we can also potentially switch providers cause we're not as locked into Stripe. 

Time will tell if this was useful or not.
