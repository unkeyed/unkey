...
The Next.js gives you a type-safe way to verify API keys, below is an example of how to use the SDK:
...
You can now disable a key via the API in the dashboard. While disabled, the key will act as an invalid key. This allows you Suppose you have a customer that has not paid their bill. You may not want to delete the key and wait for the account balance to be current. The key can be disabled temporarily, preventing access until it is enabled.
