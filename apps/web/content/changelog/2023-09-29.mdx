---
  ...
  summary: [
        "We now have an Unkey templates section on our site, built by Unkey and the community",
        "We have created segmented analytics to give a better view of usage.",
        "James is full-time!"
    ]
---
...
We just released root key analytics. The root key is the key to the kingdom, and we want to make sure you can understand where it is being used, where it is being used. You can find the root key analytics by selecting `details` for the specific key.  
...
```
Check out the docs for all the features https://docs.unkey.dev/libraries/ts/hono
...
Check out the blog post: https://unkey.dev/blog/using-unkey-with-auth.