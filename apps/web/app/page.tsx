...
export default function Home() {
  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-between px-4 py-24 bg-gradient-to-t from-transparent from-violet-400/0 to-violet-400/20">
      <Particles className="absolute inset-0 -z-10 " vy={-1} quantity={50} staticity={200} color="#7c3aed" />
      ...
      <div className="flex flex-col items-center my-4 md:my-8 lg:my-16 xl:my-24">
        ...
        <p className="text-center font-thin text-lg text-gray-300 mt-8 max-w-lg">
          Unkey is an open source API key management platform. Issue, revoke and rotate keys for your product in minutes        </p>
      ...
      <form className="relative w-full max-w-sm isolate mt-8 flex items-center pr-1" action={async (data: FormData) => {
        ...
        toast.promise(addToWaitlist(email as string), {
          ...
          success: (data) => `Thank you, you're number ${Intl.NumberFormat().format(data)} on the list`,
          error: "Error"
        })
        ...
        <button className="sm:hidden group relative isolate flex-none rounded-md p-1.5  text-white  bg-indigo-300/20">
          <kbd aria-hidden="true" className="ml-1">⏎</kbd>
        </button>
