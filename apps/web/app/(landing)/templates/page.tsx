                ...
                <div className="flex space-x-8 ">
                                ...
                                <AccordionContent>
                                    <div className="flex flex-col space-y-2">
                                        {frameworks.map((framework) => (
                                            <div className="flex items-center p-3 space-x-2 duration-150 rounded bg-gray-50 hover:bg-gray-100" key={framework.id}>
                                ...
                                <AccordionContent>
                                    <div className="flex flex-col space-y-2">
                                        {languages.map((language) => (
                                            <div className="flex items-center p-3 space-x-2 duration-150 rounded bg-gray-50 hover:bg-gray-100" key={language.id}>
                    ...
                    <div className="grid w-3/4 grid-cols-1 gap-8 auto-rows-fr lg:grid-cols-3">
                        {Object.entries(templates).map(([id, template]) => (
                            <Link href={`/templates/${id}`} key={id} className="flex flex-col items-start overflow-hidden duration-200 border border-gray-200 shadow rounded-xl hover:shadow-2xl hover:scale-[1.01]">
                                ...
                                <div className="flex flex-col justify-between h-full px-4 pb-4">
                                    <div>
                                        ...
                                        <p className="mt-5 text-sm leading-6 text-gray-500 line-clamp-3">{template.description}</p>
                                    </div>
                                    <div className="flex items-center justify-between mt-5">

                                        <p className="text-sm leading-6 text-gray-500 ">by {template.author}</p>
