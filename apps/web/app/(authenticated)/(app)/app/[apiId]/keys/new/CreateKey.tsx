...
export const CreateKey: React.FC<Props> = ({ apiId }) => {
  ...
  const key = trpc.key.create.useMutation({
    ...
    onError(err) {
      ...
      toast({ title: "Error", description: "An error occured, please try again", variant: "destructive" });
    },
  });
    ...
    <>
      {
        key.data ? (
          <div className="w-full">
            <div>
              <p className="text-xl font-bold mb-4">Your API Key</p>
              <div className="max-w-3xl	flex justify-center mx-auto p-4 mb-4 text-sm text-yellow-800 border border-yellow-300 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300 dark:border-yellow-800" role="alert">
                <svg aria-hidden="true" className="flex-shrink-0 inline w-5 h-5 mr-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>
                <span className="sr-only">Info</span>
                <div>
                  <span className="font-medium">Warning!</span>  This key is only shown once and can not be recovered. Please store it somewhere
                  safe.
                </div>
              </div>

              <div className="flex items-center justify-between gap-4 max-w-6xl px-2 py-1 mt-4 mx-auto border rounded lg:p-4 border-white/10 bg-zinc-100 dark:bg-zinc-900">
                <pre className="font-mono">{key.data.key}</pre>
                <CopyButton value={key.data.key} />
              </div>
            </div>

            <p className="mt-2 text-lg mb-2 font-medium text-center text-zinc-700 ">Try verifying it:</p>
            <div className="flex items-start  max-w-6xl mx-auto justify-between gap-4 px-2 py-1 border rounded lg:p-4 border-white/10 bg-zinc-100 dark:bg-zinc-900">
              <pre className="font-mono">{snippet}</pre>
              <CopyButton value={snippet} />
            </div>
            <div className="flex justify-around my-4 space-x-4">
              <Button className="w-1/4" onClick={() => key.reset()}>Create another key</Button>
              <Button variant="outline" className="w-1/4 border-zinc-700" onClick={() => router.push(`/app/${apiId}`)}> Done </Button>
            </div>
          </div>
        ) : (
          <>
            <div>
              <div className="w-full overflow-scroll">
                <h2 className="text-2xl text-center mb-2">Create a new Key</h2>
                <Form {...form}>
