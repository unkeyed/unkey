...
export default async function StripeRedirect() {
  ...
  const productIdToPriceId = products.data.reduce((acc, product) => {
    acc[product.id] = product.default_price as string;
    return acc;
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [
      {
        // base
        price: productIdToPriceId[e.STRIPE_PRODUCT_ID_PRO_PLAN],
        quantity: 1,
      },
      {
        // additional keys
        price: productIdToPriceId[e.STRIPE_PRODUCT_ID_ACTIVE_KEYS],
      },
      {
        // additional verifications
        price: productIdToPriceId[e.STRIPE_PRODUCT_ID_KEY_VERIFICATIONS],
      },
    ],
    mode: "subscription",
