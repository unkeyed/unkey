...
export function CopyButton({ value, className, src, ...props }: CopyButtonProps) {
  ...
  return (
    <button
      type="button"
      className={cn(
        "relative z-20 h-8 inline-flex items-center justify-center rounded-md border-border p-2 text-sm font-medium text-primary transition-all hover:border-primary hover:bg-secondary focus:outline-none ",
        className,
      )}
      onClick={() => {
