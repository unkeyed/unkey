...
<Steps titleSize="h3">
  ...
  #### Web App
  For Unkey Dashboard `apps/web/.env` the following variables will be used after you create your database:
  ...
  ### Agent
  For the agent (`apps/agent/.env`) you will need the DSN which is made up of the same values as above including the database name:
  ...
  ### Drizzle Schema
    ...
    <Tabs>
      ...
      <Tab title="Windows">
        ...
        set DRIZZLE_DATABASE_URL=mysql://{user}:{password}@{host}/{db}?ssl={"rejectUnauthorized":true}
