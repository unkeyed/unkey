...
<Tabs>
  <Tab title="pnpm">```bash pnpm add @unkey/nuxt ```</Tab>
  <Tab title="yarn">```bash yarn add @unkey/nuxt ```</Tab>
  <Tab title="npm">```bash npm install @unkey/nuxt ```</Tab>
</Tabs>
...
For more about how to use the configured helper provided by `useUnkey()`, you can see the API docs for [the TypeScript client](/libraries/js/overview).
