...
curl -X POST https://api.unkey.com/v2/analytics.getVerifications \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "query": "SELECT external_id, SUM(count) as total FROM key_verifications_per_day_v1 WHERE time >= now() - INTERVAL 30 DAY GROUP BY external_id ORDER BY total DESC LIMIT 5"
  }'
```
...
```json
{
  ...
  "data": {
    "verifications": [
      { "external_id": "user_alice", "total": 8420 },
      { "external_id": "user_bob", "total": 6105 },
      { "external_id": "user_charlie", "total": 4892 }
    ]
  }
}
```
...
<Note>
  **Raw vs Aggregated Tables:** Use `SUM(count)` instead of `COUNT(*)` when
  querying aggregated tables for better performance. Aggregated tables scan far
  fewer rows than the raw table.
</Note>
