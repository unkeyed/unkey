...
<Note>
  **Raw vs Aggregated Tables:** The raw table uses `external_id` and `api_id`, while aggregated tables use `identity_id` and `key_space_id`. When querying aggregated tables, use `SUM(total_verifications)` instead of `COUNT(*)` for better performance.
</Note>
