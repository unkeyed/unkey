...
curl -X POST https://api.unkey.com/v2/analytics.getVerifications \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "query": "SELECT external_id, SUM(total_verifications) as verifications FROM key_verifications_per_day_v1 WHERE day >= now() - INTERVAL 30 DAY GROUP BY external_id ORDER BY verifications DESC LIMIT 10"
  }'
```
