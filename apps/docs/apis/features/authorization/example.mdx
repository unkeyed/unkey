...
Sign into your [dashboard](https://app.unkey.com).
    - For `Roles`, navigate to the `Authorization` then `Roles`.
    - For `Permissions`, navigate to the `Authorization` then `Permissions`.
...
5. You can connect a role to your key by useing the `Assign role` input. Let's give this key the `dns.manager` and `read-only` roles.
