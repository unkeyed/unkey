...
When you reroll a key in Unkey, the new key inherits:
...
async function rotateKey(keyId) {
  const response = await fetch('https://api.unkey.com/v2/keys.rerollKey', {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
      keyId: keyId,
      expiration: 604800000 // 7 day grace period
    })
  });
  
  const result = await response.json();
  // Securely store and distribute the new key
  await updateKeyInVault(result.data.key);
  
  return result.data.keyId; // Store new keyId for next rotation
}
```
...
1. **Use Grace Periods**: Unless dealing with a compromise, always provide an overlap period
2. **Automate Rotation**: Set up scheduled rotations for compliance
3. **Secure Distribution**: Use secure channels (like secrets management systems) to distribute new keys
4. **Monitor Both Keys**: During the grace period, monitor usage of both old and new keys
5. **Document Rotation**: Keep audit logs of when and why keys were rotated
