...
Both the [getKey](/api-reference/keys/get) and [listKeys](/api-reference/apis/listKeys) endpoints accept a `decrypt` query parameter. If you set this to `true`, the key will be decrypted and returned in the response as `plaintext`.
