...
**Key Changes:**
- Response format: Direct response → `{meta, data}` envelope
- Enhanced response with additional metadata, mirrors the getKey response.
- Added `meta.requestId` for debugging
...
// v2 Response, responds with the permissions that are on this key now
{
  "meta": { // [!code ++]
  ...
  "data": [{
    ...
    "description": "Read documents but dont write them", // [!code ++]
  }] // [!code ++]
}
```
</Tab>
...
// v2 Response, responds with the remaining permissions
{
  ...
  "data": [ // [!code ++]
    { // [!code ++]
      "id": "perm_123", // [!code ++]
      "slug": "documents.write", // [!code ++]
      "name": "Documents Write", // [!code ++]
      "description": "Allows writing to documents" // [!code ++]
    }, // [!code ++]
  ] // [!code ++]
}
```
