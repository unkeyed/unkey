...
```sh Basic
curl \
  --url https://api.unkey.dev/v1/apis.listKeys?apiId=<API_ID> \
  --header 'Authorization: Bearer <UNKEY>'
```


```sh Filter by ownerId
curl \
  --url https://api.unkey.dev/v1/apis.listKeys?apiId=<API_ID>&ownerId=<OWNER_ID> \
  --header 'Authorization: Bearer <UNKEY>'
```

```sh Pagination
curl \
  --url https://api.unkey.dev/v1/apis.listKeys?apiId=<API_ID>&limit=10&offset=20 \
  --header 'Authorization: Bearer <UNKEY>'
```
