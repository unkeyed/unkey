...
export function BlogHero({
  ...
  return (
    <div
      className={cn(
        "flex flex-col lg:flex-row items-center w-full gap-8 xl:gap-16 relative z-100",
        className,
      )}
    >
      ...
      <div className="flex flex-col order-1 w-full lg:order-2 z-100 pt-8">
        <div className="flex flex-row justify-center gap-2 lg:justify-start">
          {tags?.map((tag) => (
            <p
              key={tag}
              className="text-white/50 text-xs bg-[rgb(26,26,26)] px-[9px] rounded-md w-fit leading-6 flex items-center capitalize py-0 z-100"
            >
              {tag.charAt(0).toUpperCase() + tag.slice(1)}
            </p>
          ))}
        </div>
        <h2 className="flex justify-center my-2 text-3xl font-medium leading-10 blog-heading-gradient sm:text-center lg:justify-start lg:text-left">
          {title}
        </h2>
        <p className="flex justify-center text-base font-normal leading-7 text-white/60 lg:pr-8 lg:justify-start">
          {subTitle}
        </p>
        <div className="flex flex-row justify-center w-full gap-8 xl:gap-24 pb-8 mt-4 lg:justify-start lg:pt-0">
