...
export function BlogQuote({ children, className, ...props }: BlogQuoteProps) {
  return (
    <div
      className={cn(
        "flex flex-col text-lg border-l-2 border-white/20 xxs:pl-8 pl-16 py-4 xxs:py-2 text-left ",
        className,
      )}
    >
      <div className="align-middle h-fit ">
        <blockquote className="not-prose my-auto xxs:my-0 font-medium text-white leading-8 xxs:text-xs">
          {children}
        </blockquote>
        {props.author && (
