...
All required and optional arguments are passed via query parameters. Some paramters may be specific multiple times, either as You may specify multiple ids such as `?param=value_1,value_2` or `?param=value_1&param=value_2`
...
> A user sees a chart of their usage over the past 24h, showing the outcomes
...
> A user sees a total usage counter and daily usage chart in the current month

```bash
?start={timestamp_start_of_month}&end={timestamp_now}&granularity=day&externalId={user_123}

[
  // up to 31 elements, one per day
...
type Datapoint = {
  ...
  */
  ...
  /**
   * Only available if we add groupBy queries.
...
Instead of aggregating multiple keyIds together, we could not allow specifying them multiple times and instead ask the user to create one request per id and then merge them together on their side.
...
- Should we add `groupBy`, `limit` and `orderBy` parameters for advanced usecases?
  - Usage per key for all keys of an identity
  - Top 10 keys of an identity
  - Top 10 identities in an API
