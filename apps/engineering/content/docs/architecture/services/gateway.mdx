...
Sentinel is an environment-scoped HTTP proxy service that receives requests from Ingress and routes them to the appropriate deployment instance.
...
<Mermaid chart={`graph LR
    ...
    Ingress --> Sentinel[Sentinel<br/><i>per-environment</i>]
...
<Mermaid chart={`sequenceDiagram
    ...
    Ingress->>Sentinel: HTTP Request + X-Deployment-ID header
    ...
    alt Deployment belongs to wrong environment
        ...
        Sentinel->>Ingress: 404 Not Found
...
Sentinel is not user-facing (only Ingress calls it), so it always returns JSON errors:
