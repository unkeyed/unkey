...
This diagram shows the control loop for deployments only. Gateways are handled in the same way, but have their own buffers and controllers.
...
**Dual Controllers**: Separate deployment and gateway controllers for each infra type.
...
The `gateways` table is a mix of desired and actual state. Because we do not care about individual pods, we only need to configure the desired cpu, memory and replicas. We update the actual state columns from kubernetes events.
