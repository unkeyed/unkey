...
Ingress is a multi-tenant service running as a Kubernetes Deployment behind a Network Load Balancer. A single Ingress instance handles traffic for all customer deployments, performing hostname lookups and routing decisions to forward requests to the appropriate environment sentinel.
...
<Mermaid chart={`sequenceDiagram
    ...
    alt Local sentinel exists
        Ingress->>Ingress: Lookup sentinel for environment_id
        Ingress->>Sentinel: HTTP proxy + X-Deployment-ID header
        Sentinel->>Ingress: Response
...
Ingress routes requests to environment-scoped sentinels based on the `environment_id` from the ingress route lookup.
...
If no local sentinel is found, Ingress forwards to the region's NLB, triggering cross-region forwarding.

### Request Headers

Ingress passes metadata to the Sentinel via HTTP headers:
...
When no local sentinel is available, Ingress forwards requests to the region's Network Load Balancer.
