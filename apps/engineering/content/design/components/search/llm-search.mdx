...
- Search is triggered after user stops typing for the specified debounce time
...
- Search is triggered while user is typing with throttling
...
When using the LLMSearch component:

### Search Mode Selection
- Use **manual mode** for precise searches where accuracy is more important than speed
- Use **debounced mode** for general search with good balance of responsiveness and efficiency
- Use **throttled mode** for real-time search where immediate feedback is crucial

### Example Queries
- Provide 3-5 relevant example queries that demonstrate the search capabilities
- Use natural language that matches your users' vocabulary
- Include examples that showcase different search patterns (errors, users, time ranges, etc.)

### Loading States
- Always provide meaningful loading text that explains what's happening
- Consider the expected duration of your search operations
- Use consistent loading patterns across your application

### Performance Considerations
- Choose appropriate debounce times based on your API response times
- Consider implementing search result caching for repeated queries
- Monitor API usage and adjust search modes accordingly

### User Experience
- Provide clear visual feedback for all search states
- Use consistent placeholder text that guides users
- Consider the context where the search is used (logs, analytics, etc.)
