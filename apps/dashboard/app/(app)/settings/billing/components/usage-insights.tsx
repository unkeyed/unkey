      ...
      <UsageProvider tier={tier} current={current} max={max}>
        <motion.div
          ...
          className={cn(
            ...
            { "h-[102px]": isLoading, className }
          )}
        >
          {isLoading ? (
            <div className="z-10 flex flex-col w-full gap-2.5">
              <Skeleton className="h-5 w-20 rounded-md" />
              <div className="flex gap-2">
                <Skeleton className="h-7 w-7 rounded-md delay-100" />
                <div className="flex flex-col items-start justify-start gap-2">
                  <Skeleton className="h-4 w-20 rounded-md delay-150" />
                  <Skeleton className="h-3 w-32 rounded-md delay-200" />
                </div>
              </div>
            </div>
          ) : (
            <div className="z-10 flex flex-col w-full gap-3">
              <h2 className="text-gray-12 text-lg capitalize leading-5">
                {tier}
              </h2>
...
export const Details = React.forwardRef<DivElement, DivProps>((props, ref) => {
  ...
  return (
    <motion.div
      ...
      ref={ref}
      className={cn(
        "duration-500 ease-out w-full flex flex-col items-start justify-start",
        {
          className,
        }
      )}
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col gap-1.5 justify-start items-start select-none">
          <h6 className="text-gray-12 text-sm leading-none line-clamp-1">
            {title}
          </h6>
          <p className="text-xs font-normal line-clamp-1">
            {formatNumber(item?.current ?? current)} of{" "}
            {formatNumber(item?.max ?? max)} {description}
          </p>
        </div>
      </div>
    )
