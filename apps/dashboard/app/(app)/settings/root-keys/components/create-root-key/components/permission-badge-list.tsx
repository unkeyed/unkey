...
const PermissionBadgeList = ({ apiId, name, selectedPermissions, title, expandCount, removePermission }: Props) => {
    ...
    return info.length > expandCount ? (
        <div className="flex flex-col gap-2">
            <CollapsibleList info={info as InfoType} title={title} expandCount={expandCount} removePermission={removePermission} name={name} />
        </div>
    ) : (
        <div className="flex flex-col gap-2">
...
const CollapsibleList = ({
    ...
    return (
        <Collapsible>
            <CollapsibleTrigger
                {...props}
                className={cn("flex flex-row gap-3 transition-all [&[data-state=open]>svg]:rotate-180 w-full",
                className)}
            >
                <ListTitle title={title} count={info.length} category={name} />
