...
if (allFilterFieldNames.length === 0) {
  throw new Error(
    "projectsFilterFieldConfig must contain at least one field definition."
  );
}

const [firstFieldName, ...restFieldNames] = allFilterFieldNames;

export const projectsFilterFieldEnum = z.enum([
  firstFieldName,
  ...restFieldNames,
]);
...
export type ProjectsFilterValue = FilterValue<
  ProjectsFilterField,
  ProjectsFilterOperator
>;
...
export const parseAsAllOperatorsFilterArray =
  parseAsFilterValueArray<ProjectsFilterOperator>([...commonStringOperators]);
