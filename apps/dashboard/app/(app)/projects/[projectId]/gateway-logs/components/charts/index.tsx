...
import { useGatewayLogsTimeseries } from "./hooks/use-gateway-logs-timeseries";

export function GatewayLogsChart({
  onMount,
}: {
  onMount: (distanceToTop: number) => void;
}) {
  const { filters, updateFilters } = useGatewayLogsFilters();
  const { timeseries, isLoading, isError, granularity } = useGatewayLogsTimeseries();
