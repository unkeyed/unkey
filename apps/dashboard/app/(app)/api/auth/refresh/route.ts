...
import { UNKEY_ACCESS_MAX_AGE, UNKEY_ACCESS_TOKEN, UNKEY_REFRESH_TOKEN, UNKEY_SESSION_COOKIE } from "@/lib/auth/types";
...
export async function POST(request: Request) {
  try {
    ...
    const currentRefreshToken = 
      ...
      await getCookie(UNKEY_REFRESH_TOKEN);
    
    if (!currentRefreshToken) {
      console.error("Access token refresh failed: no refresh token");
      return Response.json({ success: false, error: "Failed to refresh access token" }, { status: 401 });
    }
    ...
    refreshPromise = (async () => {
      try {
        ...
        const { sessionToken, accessToken, refreshToken, expiresAt, session } = await auth.refreshAccessToken(currentRefreshToken);
