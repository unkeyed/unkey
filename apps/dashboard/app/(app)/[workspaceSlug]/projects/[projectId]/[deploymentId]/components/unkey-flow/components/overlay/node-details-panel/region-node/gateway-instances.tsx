...
export function GatewayInstances({ instances }: GatewayInstancesProps) {
  const gatewayInstances = instances.filter((node) => node.metadata.type === "gateway");
  ...
  return (
    <div className="flex px-4 w-full mt-4 flex-col">
      <div className="flex items-center gap-3 w-full">
        <div className="text-gray-9 text-xs whitespace-nowrap">Gateway instances</div>
        <div className="h-0.5 bg-grayA-3 rounded-sm flex-1 min-w-[115px]" />
      </div>
      <div className="flex flex-col gap-6 mt-5">
        {gatewayInstances.map((instance) => {
          if (instance.metadata.type !== "gateway") {
            return null;
          }
              ...
              <div className="flex-1 flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <MetricPill
                    ...
                    value={rps ?? 0}
                    tooltip="Requests per second handled by this gateway instance"
                  />
                  <MetricPill
