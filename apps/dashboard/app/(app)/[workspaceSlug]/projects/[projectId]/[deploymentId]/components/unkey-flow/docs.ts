...
Since the layout engine is our backend, the engine must know the dimensions of the nodes before we do anything, and React is responsible for this. In our case, nodes live in a tree, so we first flatten them, then pass the size of each node type to the engine. Currently we predefine the dimensions of nodes. In the future to make it dynamic we could render the nodes initially on our canvas, then simply query the DOM to get their actual sizes, but for now we know the node types and sizes beforehand so there's no need for that.

If we accidentally call `calculate` before setting the dimensions, the code will panic.

Then, we invoke `calculate` to calculate the positioning of the nodes and their connection lines. First we call `buildNodeLayout` recursively. For the initial node it's simple: since its level is `0` and position is `{x:0,y:0}`, we directly push it into our `positioned: PositionedNode[] = [];` array. For the rest of the children nodes we'll run a depth first search, meaning we'll first start with the first child and consume entire subtrees until we finish. Then, move on to the next child and do the same.
...
Now we know the width of the subtree, we can calculate the actual `X` positions of the nodes. First we need to figure out where the first child's subtree starts. We want to center all children under the parent. If the parent is at X=0 and total width is `589` - meaning we have two children - that's why we divide it by 2. Now we know where to place our first node. Following nodes start where the previous node's subtree ended, plus spacing. So, if our initial node starts from `-294.5`, the next node will start from `-294.5 + 282 + 25` and so on. And finally, since we only care about the center of the nodes, we divide it by 2 again and store that.
...
First we calculate the `subtreeHeight`, then we calculate the actual `Y` positions of the nodes stacked vertically. First we need to figure out where the first child starts. Unlike horizontal spreading where we center children under the parent, here `startY` already positions itself below the parent (parent's bottom + spacing), so we just add half of the first child's height to reach its center. Following nodes start where the previous node's subtree ended, plus spacing. So, if our first child's center is at `310`, the next child starts from `310 + 50` (bottom half of first child) `+ 75` (spacing) `+ 50` (top half of second child). And since we only care about the center of the nodes, we accumulate these offsets.
...
Direct lines between parent and child nodes would overlap. We need intermediate waypoints to route connections cleanly.
...
All siblings use the same `trunkX`, creating a unified tree appearance.
