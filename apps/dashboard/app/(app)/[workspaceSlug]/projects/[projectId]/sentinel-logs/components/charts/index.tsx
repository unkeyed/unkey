...
import { useSentinelLogsTimeseries } from "./hooks/use-sentinel-logs-timeseries";

export function SentinelLogsChart({
  onMount,
}: {
  onMount: (distanceToTop: number) => void;
}) {
  const { filters, updateFilters } = useSentinelLogsFilters();
  const { timeseries, isLoading, isError, granularity } = useSentinelLogsTimeseries();
