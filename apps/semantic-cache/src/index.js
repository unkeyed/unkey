...
async function handleCacheOrDiscard(c, cache, stream, event, vector) {
...
export async function handleStreamingRequest(c, request, openai, cache) {
...
export async function handleNonStreamingRequest(c, request, openai, cache) {
  const startTime = Date.now();
  const messages = parseMessagesToString(request.messages);
  const vector = await getEmbeddings(c, messages);
  const embeddingsTime = Date.now();
