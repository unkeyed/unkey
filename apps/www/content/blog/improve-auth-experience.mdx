...
description: The sign-in flow for your application is the gateway and sometimes the first impression; one of the biggest issues is the returning user.
author: james
tags: ["tutorials"]
---
...
The problem, what account did I use to sign up for this account? Was it Github? Was it Google? Or did I use my email? In great applications, Unkey included, account linking is used behind the scenes which can help mitigate this issue.
...
2. The same User returns to the sign-in page and doesn’t remember, so they select Google and selects james@unkey.dev
...
## Adding last used to improve the User Experience

The main issue with relying only on account linking is that users potentially use different emails for accounts. For example, my GitHub primary email is my personal email, and I use my work email as my Google account. So, if I accidentally select the wrong option, I will go through the onboarding experience, which is frustrating as a user.

<Image src="/images/blog-images/auth-experience/last-used.png" alt="sign in example" width="1920" height="1080"/>

Our users certainly appreciate the effort we put into this user experience, which allows them to select the correct account each time.
...
We first need to add logic to the sign-in and sign-up flow to implement the “Last used” tag. The example below is what a Clerk custom flow might look like and what we use at Unkey.
...
This function now allows us to read and write our sign-in options to local storage. I will show how it is used in a second, but first, we need a UI element to show to the user. We opted for a simple text span displaying the last used, but you could display an icon or any text you prefer.
...
const [lastUsed, setLastUsed] = useLastUsed();
  ...
  ```
The final step is to display to the user that it was the “last used.” Below is how we implemented it at Unkey: we placed it next to the button.
