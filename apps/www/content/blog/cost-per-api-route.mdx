...
By implementing Unkey, we can remove the need for storage like Redis to handle cost-based APIs and provide analytics, ratelimiting, RBAC, and more.

With Unkey, just a few lines of code, you can protect your API; the key holds all the information, so there isn't a requirement to look at another system to find this information. API keys can be created programatically or through our dashboard.  
...
- Implementing API key creation (Are example doesn't show this)
- Setting the credit amount for a user (Are example doesn't show this)
- Safe way to look up an API key to get the usage base. 
- Redis
...
const verifyKeyWithCost = (cost: number) => createMiddleware(async (c, next) => {
 ...
 apiId: "API_ID_FROM_UNKEY",
    ...
    // There are other errors but not needed for this example
    if (!result.valid) {
      switch (result.code) {
        case "DISABLED":
...
As you can see we really simplified the code and remove additional looks ups via databases, allowing your API to be performant and scalable. You can get started for [free today](https://app.unkey.com)! 