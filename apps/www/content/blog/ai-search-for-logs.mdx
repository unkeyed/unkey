...
LLMs now touch every aspect of our lives. We find them in our favorite websites, mobile apps, IDEs, and operating systems. We can debate their usefulness, since not every app or X (insert your favorite thing here) needs AI.

LLMs excel at transforming boring, repetitive actions into human-readable formats, particularly in search tasks. These tools understand our imprecise queries better. They can interpret our ambiguous questions and provide relevant information. Thats why users increasingly choose ChatGPT, Claude, and Perplexity over Google.

We were inspired to add AI-powered search to our newly refined logs page. While users can still apply traditional filters, expressing their needs in natural language offers a simpler approach.

> Consider how you currently narrow down an issue on any logs page: you select
> the request status, choose the method, pick the date-time from two hours ago
> in the Date-time filter, and continue adding criteria. Now imagine typing "I
> need requests with GET methods, success status that happened 2 hours ago." See
> the difference? While filter selection works well, natural language provides a
> more intuitive path to find what you need.
...
> In our prompt there are lots of examples for each search variation, but in here it's omitted for brevity. For the best result make sure prompt is as detailed as possible.
...
const completion = await openai.beta.chat.completions.parse({
  ...
  response_format: zodResponseFormat(filterOutputSchema, "randomNameHere"),
});
```
...
- Each query consumes OpenAI API tokens - More AI search more money to burn
