...
The `Vault` is our new encryption management service. This service allows you to encrypt and decrypt API keys securely. Unkey only stores a hash of the key without using the `Vault` there is no way to retrieve a key.  However, The `Vault` gives you the ability to decrypt a key if needed.
...
To opt in to recovery, send us an email at support@unkey.dev. Make sure to use the email address associated with your workspace and include the API ID that you want to enable the `Vault` for.
