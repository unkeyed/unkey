...
You’ll notice that we check for `forwarded-ip` and then the `real-ip`, and finally, if nothing is available, we will use `no-ip` for the fallback. This endpoint is now protected; a user can send two requests per minute. Below is a demo of how you could present this to the user:
