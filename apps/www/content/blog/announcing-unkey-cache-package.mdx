...
With the performance figured out, we still had to make it enjoyable to use..
...
// use user
```
This does not look too bad, but it also lacks so many things:
...
Here's what it will look like with @unkey/cache:
None of this is magic, it's just nicer to use.

```ts
const value = await cache.user.swr("userId", async () => {
 ...
 * will automatically handle the cache miss and cache update for us.
