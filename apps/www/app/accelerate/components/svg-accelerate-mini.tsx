export default function SVGAccelerateMini(props: { className?: string; no: number }) {
  if (props.no === 1) {
    return (
      <svg
        width="88"
        height="87"
        viewBox="0 0 88 87"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        {...props}
      >
        <circle
          cx="56"
          cy="55"
          r="24"
          stroke="#FF1D4F"
          stroke-opacity="0.15"
          stroke-width="16"
          stroke-dasharray="2 3"
          {...props}
        />
        <g opacity="0.5" filter="url(#filter0_f_5190_171)">
          <path
            d="M24 55C24 47.9557 26.3244 41.1082 30.6127 35.5196C34.901 29.931 40.9135 25.9136 47.7178 24.0904L56 55L24 55Z"
            fill="url(#paint0_angular_5190_171)"
          />
        </g>
        <mask
          id="mask0_5190_171"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="24"
          y="24"
          width="32"
          height="31"
        >
          <path
            d="M24 55C24 47.9557 26.3244 41.1082 30.6127 35.5196C34.901 29.931 40.9135 25.9136 47.7178 24.0904L56 55L24 55Z"
            fill="#D9D9D9"
          />
        </mask>
        <g mask="url(#mask0_5190_171)">
          <circle
            cx="56"
            cy="55"
            r="24"
            stroke="url(#paint1_angular_5190_171)"
            stroke-width="16"
            stroke-dasharray="2 3"
          />
        </g>
        <path
          d="M47.7188 24.0903L64.2832 85.9096"
          stroke="url(#paint2_linear_5190_171)"
          stroke-width="2"
        />
        <defs>
          <filter
            id="filter0_f_5190_171"
            x="0"
            y="0.090332"
            width="80"
            height="78.9097"
            filterUnits="userSpaceOnUse"
            color-interpolation-filters="sRGB"
          >
            <feFlood flood-opacity="0" result="BackgroundImageFix" />
            <feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape" />
            <feGaussianBlur stdDeviation="12" result="effect1_foregroundBlur_5190_171" />
          </filter>
          <radialGradient
            id="paint0_angular_5190_171"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 55) rotate(45) scale(31.8198)"
          >
            <stop offset="0.0001" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
          </radialGradient>
          <radialGradient
            id="paint1_angular_5190_171"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 55) rotate(43.5461) scale(30.8026)"
          >
            <stop stop-color="#7002FC" />
            <stop offset="0.0001" stop-color="#FF1D4F" stopOpacity="0" />
            <stop offset="0.0002" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.352076" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.378012" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.381003" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
            <stop offset="0.9998" stop-color="#7002FC" stopOpacity="0" />
          </radialGradient>
          <linearGradient
            id="paint2_linear_5190_171"
            x1="48.2017"
            y1="23.9609"
            x2="64.7661"
            y2="85.7802"
            gradientUnits="userSpaceOnUse"
          >
            <stop stop-color="white" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
          </linearGradient>
        </defs>
      </svg>
    );
  }
  if (props.no === 2) {
    return (
      <svg
        width="103"
        height="88"
        viewBox="0 0 103 88"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        {...props}
      >
        <circle
          cx="56"
          cy="56"
          r="24"
          stroke="#FF1D4F"
          stroke-opacity="0.15"
          stroke-width="16"
          stroke-dasharray="2 3"
        />
        <g opacity="0.5" filter="url(#filter0_f_5190_448)">
          <path
            d="M24 56C24 49.671 25.8768 43.4841 29.393 38.2217C32.9092 32.9594 37.9069 28.8579 43.7541 26.4359C49.6014 24.0138 56.0355 23.3801 62.2429 24.6149C68.4503 25.8496 74.1521 28.8973 78.6274 33.3726L56 56L24 56Z"
            fill="url(#paint0_angular_5190_448)"
          />
        </g>
        <mask
          id="mask0_5190_448"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="24"
          y="24"
          width="55"
          height="32"
        >
          <path
            d="M24 56C24 49.671 25.8768 43.4841 29.393 38.2217C32.9092 32.9594 37.9069 28.8579 43.7541 26.4359C49.6014 24.0138 56.0355 23.3801 62.2429 24.6149C68.4503 25.8496 74.1521 28.8973 78.6274 33.3726L56 56L24 56Z"
            fill="#D9D9D9"
          />
        </mask>
        <g mask="url(#mask0_5190_448)">
          <circle
            cx="56"
            cy="56"
            r="24"
            stroke="url(#paint1_angular_5190_448)"
            stroke-width="16"
            stroke-dasharray="2 3"
          />
        </g>
        <path
          d="M78.6289 33.3726L33.3741 78.6274"
          stroke="url(#paint2_linear_5190_448)"
          stroke-width="2"
        />
        <defs>
          <filter
            id="filter0_f_5190_448"
            x="0"
            y="0"
            width="102.627"
            height="80"
            filterUnits="userSpaceOnUse"
            color-interpolation-filters="sRGB"
          >
            <feFlood flood-opacity="0" result="BackgroundImageFix" />
            <feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape" />
            <feGaussianBlur stdDeviation="12" result="effect1_foregroundBlur_5190_448" />
          </filter>
          <radialGradient
            id="paint0_angular_5190_448"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(45) scale(31.8198)"
          >
            <stop offset="0.0001" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
          </radialGradient>
          <radialGradient
            id="paint1_angular_5190_448"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(43.5461) scale(30.8026)"
          >
            <stop stop-color="#7002FC" />
            <stop offset="0.0001" stop-color="#FF1D4F" stopOpacity="0" />
            <stop offset="0.0002" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.352076" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.378012" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.381003" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
            <stop offset="0.9998" stop-color="#7002FC" stopOpacity="0" />
          </radialGradient>
          <linearGradient
            id="paint2_linear_5190_448"
            x1="78.9825"
            y1="33.7261"
            x2="33.7276"
            y2="78.9809"
            gradientUnits="userSpaceOnUse"
          >
            <stop stop-color="white" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
          </linearGradient>
        </defs>
      </svg>
    );
  }
  if (props.no === 3) {
    return (
      <svg
        width="103"
        height="88"
        viewBox="0 0 103 88"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        {...props}
      >
        <circle
          cx="56"
          cy="56"
          r="24"
          stroke="#FF1D4F"
          stroke-opacity="0.15"
          stroke-width="16"
          stroke-dasharray="2 3"
        />
        <g opacity="0.5" filter="url(#filter0_f_5190_448)">
          <path
            d="M24 56C24 49.671 25.8768 43.4841 29.393 38.2217C32.9092 32.9594 37.9069 28.8579 43.7541 26.4359C49.6014 24.0138 56.0355 23.3801 62.2429 24.6149C68.4503 25.8496 74.1521 28.8973 78.6274 33.3726L56 56L24 56Z"
            fill="url(#paint0_angular_5190_448)"
          />
        </g>
        <mask
          id="mask0_5190_448"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="24"
          y="24"
          width="55"
          height="32"
        >
          <path
            d="M24 56C24 49.671 25.8768 43.4841 29.393 38.2217C32.9092 32.9594 37.9069 28.8579 43.7541 26.4359C49.6014 24.0138 56.0355 23.3801 62.2429 24.6149C68.4503 25.8496 74.1521 28.8973 78.6274 33.3726L56 56L24 56Z"
            fill="#D9D9D9"
          />
        </mask>
        <g mask="url(#mask0_5190_448)">
          <circle
            cx="56"
            cy="56"
            r="24"
            stroke="url(#paint1_angular_5190_448)"
            stroke-width="16"
            stroke-dasharray="2 3"
          />
        </g>
        <path
          d="M78.6289 33.3726L33.3741 78.6274"
          stroke="url(#paint2_linear_5190_448)"
          stroke-width="2"
        />
        <defs>
          <filter
            id="filter0_f_5190_448"
            x="0"
            y="0"
            width="102.627"
            height="80"
            filterUnits="userSpaceOnUse"
            color-interpolation-filters="sRGB"
          >
            <feFlood flood-opacity="0" result="BackgroundImageFix" />
            <feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape" />
            <feGaussianBlur stdDeviation="12" result="effect1_foregroundBlur_5190_448" />
          </filter>
          <radialGradient
            id="paint0_angular_5190_448"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(45) scale(31.8198)"
          >
            <stop offset="0.0001" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
          </radialGradient>
          <radialGradient
            id="paint1_angular_5190_448"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(43.5461) scale(30.8026)"
          >
            <stop stop-color="#7002FC" />
            <stop offset="0.0001" stop-color="#FF1D4F" stopOpacity="0" />
            <stop offset="0.0002" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.352076" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.378012" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.381003" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
            <stop offset="0.9998" stop-color="#7002FC" stopOpacity="0" />
          </radialGradient>
          <linearGradient
            id="paint2_linear_5190_448"
            x1="78.9825"
            y1="33.7261"
            x2="33.7276"
            y2="78.9809"
            gradientUnits="userSpaceOnUse"
          >
            <stop stop-color="white" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
          </linearGradient>
        </defs>
      </svg>
    );
  }
  if (props.no === 4) {
    return (
      <svg
        width="111"
        height="88"
        viewBox="0 0 111 88"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        {...props}
      >
        <circle
          cx="56"
          cy="56"
          r="24"
          stroke="#FF1D4F"
          stroke-opacity="0.15"
          stroke-width="16"
          stroke-dasharray="2 3"
        />
        <g opacity="0.5" filter="url(#filter0_f_5190_521)">
          <path
            d="M24 56C24 48.2365 26.8223 40.7378 31.9411 34.9009C37.0599 29.0641 44.1261 25.2871 51.8232 24.2738C59.5202 23.2604 67.3232 25.0798 73.7783 29.393C80.2333 33.7061 84.9003 40.2189 86.9096 47.7178L56 56L24 56Z"
            fill="url(#paint0_angular_5190_521)"
          />
        </g>
        <mask
          id="mask0_5190_521"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="24"
          y="24"
          width="63"
          height="32"
        >
          <path
            d="M24 56C24 48.2365 26.8223 40.7378 31.9411 34.9009C37.0599 29.0641 44.1261 25.2871 51.8232 24.2738C59.5202 23.2604 67.3232 25.0798 73.7783 29.393C80.2333 33.7061 84.9003 40.2189 86.9096 47.7178L56 56L24 56Z"
            fill="#D9D9D9"
          />
        </mask>
        <g mask="url(#mask0_5190_521)">
          <circle
            cx="56"
            cy="56"
            r="24"
            stroke="url(#paint1_angular_5190_521)"
            stroke-width="16"
            stroke-dasharray="2 3"
          />
        </g>
        <path
          d="M86.9102 47.7178L25.0909 64.2822"
          stroke="url(#paint2_linear_5190_521)"
          stroke-width="2"
        />
        <defs>
          <filter
            id="filter0_f_5190_521"
            x="0"
            y="0"
            width="110.91"
            height="80"
            filterUnits="userSpaceOnUse"
            color-interpolation-filters="sRGB"
          >
            <feFlood flood-opacity="0" result="BackgroundImageFix" />
            <feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape" />
            <feGaussianBlur stdDeviation="12" result="effect1_foregroundBlur_5190_521" />
          </filter>
          <radialGradient
            id="paint0_angular_5190_521"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(45) scale(31.8198)"
          >
            <stop offset="0.0001" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
          </radialGradient>
          <radialGradient
            id="paint1_angular_5190_521"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(43.5461) scale(30.8026)"
          >
            <stop stop-color="#7002FC" />
            <stop offset="0.0001" stop-color="#FF1D4F" stopOpacity="0" />
            <stop offset="0.0002" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.352076" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.378012" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.381003" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
            <stop offset="0.9998" stop-color="#7002FC" stopOpacity="0" />
          </radialGradient>
          <linearGradient
            id="paint2_linear_5190_521"
            x1="87.0396"
            y1="48.2007"
            x2="25.2203"
            y2="64.7652"
            gradientUnits="userSpaceOnUse"
          >
            <stop stop-color="white" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
          </linearGradient>
        </defs>
      </svg>
    );
  }
  if (props.no === 5) {
    return (
      <svg
        width="112"
        height="89"
        viewBox="0 0 112 89"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        {...props}
      >
        <circle
          cx="56"
          cy="56"
          r="24"
          stroke="#FF1D4F"
          stroke-opacity="0.15"
          stroke-width="16"
          stroke-dasharray="2 3"
        />
        <g opacity="0.5" filter="url(#filter0_f_5190_660)">
          <path
            d="M24 56C24 51.445 24.9725 46.9424 26.8524 42.7934C28.7323 38.6443 31.4763 34.9445 34.9009 31.9411C38.3256 28.9378 42.352 26.7002 46.7109 25.3779C51.0698 24.0556 55.6608 23.6792 60.1768 24.2738C64.6929 24.8683 69.03 26.4202 72.8982 28.8255C76.7663 31.2309 80.0764 34.4344 82.607 38.2218C85.1377 42.0091 86.8306 46.2931 87.5726 50.7873C88.3146 55.2816 88.0886 59.8824 86.9096 64.2822L56 56L24 56Z"
            fill="url(#paint0_angular_5190_660)"
          />
        </g>
        <mask
          id="mask0_5190_660"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="24"
          y="24"
          width="64"
          height="41"
        >
          <path
            d="M24 56C24 51.445 24.9725 46.9424 26.8524 42.7934C28.7323 38.6443 31.4763 34.9445 34.9009 31.9411C38.3256 28.9378 42.352 26.7002 46.7109 25.3779C51.0698 24.0556 55.6608 23.6792 60.1768 24.2738C64.6929 24.8683 69.03 26.4202 72.8982 28.8255C76.7663 31.2309 80.0764 34.4344 82.607 38.2218C85.1377 42.0091 86.8306 46.2931 87.5726 50.7873C88.3146 55.2816 88.0886 59.8824 86.9096 64.2822L56 56L24 56Z"
            fill="#D9D9D9"
          />
        </mask>
        <g mask="url(#mask0_5190_660)">
          <circle
            cx="56"
            cy="56"
            r="24"
            stroke="url(#paint1_angular_5190_660)"
            stroke-width="16"
            stroke-dasharray="2 3"
          />
        </g>
        <mask
          id="mask1_5190_660"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="24"
          y="24"
          width="64"
          height="41"
        >
          <path
            d="M24 56C24 51.445 24.9725 46.9424 26.8524 42.7934C28.7323 38.6443 31.4763 34.9445 34.9009 31.9411C38.3256 28.9378 42.352 26.7002 46.7109 25.3779C51.0698 24.0556 55.6608 23.6792 60.1768 24.2738C64.6929 24.8683 69.03 26.4202 72.8982 28.8255C76.7663 31.2309 80.0764 34.4344 82.607 38.2218C85.1377 42.0091 86.8306 46.2931 87.5726 50.7873C88.3146 55.2816 88.0886 59.8824 86.9096 64.2822L56 56L24 56Z"
            fill="#D9D9D9"
          />
        </mask>
        <g mask="url(#mask1_5190_660)">
          <circle
            cx="56"
            cy="56"
            r="24"
            stroke="url(#paint2_angular_5190_660)"
            stroke-width="16"
            stroke-dasharray="2 3"
          />
        </g>
        <path
          d="M86.9102 64.2822L25.0909 47.7178"
          stroke="url(#paint3_linear_5190_660)"
          stroke-width="2"
        />
        <defs>
          <filter
            id="filter0_f_5190_660"
            x="0"
            y="0"
            width="112"
            height="88.2822"
            filterUnits="userSpaceOnUse"
            color-interpolation-filters="sRGB"
          >
            <feFlood flood-opacity="0" result="BackgroundImageFix" />
            <feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape" />
            <feGaussianBlur stdDeviation="12" result="effect1_foregroundBlur_5190_660" />
          </filter>
          <radialGradient
            id="paint0_angular_5190_660"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(45) scale(31.8198)"
          >
            <stop offset="0.0001" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
          </radialGradient>
          <radialGradient
            id="paint1_angular_5190_660"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(43.5461) scale(30.8026)"
          >
            <stop stop-color="#7002FC" />
            <stop offset="0.0001" stop-color="#FF1D4F" stopOpacity="0" />
            <stop offset="0.0002" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.352076" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.378012" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.381003" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
            <stop offset="0.9998" stop-color="#7002FC" stopOpacity="0" />
          </radialGradient>
          <radialGradient
            id="paint2_angular_5190_660"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(43.5461) scale(30.8026)"
          >
            <stop stop-color="#7002FC" />
            <stop offset="0.0001" stop-color="#FF1D4F" stopOpacity="0" />
            <stop offset="0.0002" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.352076" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.378012" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.381003" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
            <stop offset="0.9998" stop-color="#7002FC" stopOpacity="0" />
          </radialGradient>
          <linearGradient
            id="paint3_linear_5190_660"
            x1="86.7807"
            y1="64.7652"
            x2="24.9615"
            y2="48.2008"
            gradientUnits="userSpaceOnUse"
          >
            <stop stop-color="white" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
          </linearGradient>
        </defs>
      </svg>
    );
  }
  if (props.no === 6) {
    return (
      <svg
        width="112"
        height="103"
        viewBox="0 0 112 103"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        {...props}
      >
        <circle
          cx="56"
          cy="56"
          r="24"
          stroke="#FF1D4F"
          stroke-opacity="0.15"
          stroke-width="16"
          stroke-dasharray="2 3"
        />
        <g opacity="0.5" filter="url(#filter0_f_5190_741)">
          <path
            d="M24 56C24 50.7376 25.2978 45.5564 27.7785 40.9153C30.2592 36.2743 33.8462 32.3166 38.2218 29.393C42.5973 26.4693 47.6264 24.6699 52.8635 24.1541C58.1005 23.6383 63.384 24.422 68.2459 26.4359C73.1077 28.4497 77.3979 31.6315 80.7363 35.6994C84.0748 39.7673 86.3585 44.5958 87.3851 49.7571C88.4118 54.9184 88.1497 60.2533 86.6221 65.2891C85.0945 70.3249 82.3485 74.9063 78.6274 78.6274L56 56L24 56Z"
            fill="url(#paint0_angular_5190_741)"
          />
        </g>
        <mask
          id="mask0_5190_741"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="24"
          y="24"
          width="64"
          height="55"
        >
          <path
            d="M24 56C24 50.7376 25.2978 45.5564 27.7785 40.9153C30.2592 36.2743 33.8462 32.3166 38.2218 29.393C42.5973 26.4693 47.6264 24.6699 52.8635 24.1541C58.1005 23.6383 63.384 24.422 68.2459 26.4359C73.1077 28.4497 77.3979 31.6315 80.7363 35.6994C84.0748 39.7673 86.3585 44.5958 87.3851 49.7571C88.4118 54.9184 88.1497 60.2533 86.6221 65.2891C85.0945 70.3249 82.3485 74.9063 78.6274 78.6274L56 56L24 56Z"
            fill="#D9D9D9"
          />
        </mask>
        <g mask="url(#mask0_5190_741)">
          <circle
            cx="56"
            cy="56"
            r="24"
            stroke="url(#paint1_angular_5190_741)"
            stroke-width="16"
            stroke-dasharray="2 3"
          />
        </g>
        <path
          d="M78.627 78.6274L33.3721 33.3726"
          stroke="url(#paint2_linear_5190_741)"
          stroke-width="2"
        />
        <defs>
          <filter
            id="filter0_f_5190_741"
            x="0"
            y="0"
            width="112"
            height="102.627"
            filterUnits="userSpaceOnUse"
            color-interpolation-filters="sRGB"
          >
            <feFlood flood-opacity="0" result="BackgroundImageFix" />
            <feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape" />
            <feGaussianBlur stdDeviation="12" result="effect1_foregroundBlur_5190_741" />
          </filter>
          <radialGradient
            id="paint0_angular_5190_741"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(45) scale(31.8198)"
          >
            <stop offset="0.0001" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
          </radialGradient>
          <radialGradient
            id="paint1_angular_5190_741"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(56 56) rotate(43.5461) scale(30.8026)"
          >
            <stop stop-color="#7002FC" />
            <stop offset="0.0001" stop-color="#FF1D4F" stopOpacity="0" />
            <stop offset="0.0002" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.352076" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.378012" stop-color="#FF1D4F" stopOpacity="0.1" />
            <stop offset="0.381003" stop-color="#FF1D4F" />
            <stop offset="0.9997" stop-color="#7002FC" />
            <stop offset="0.9998" stop-color="#7002FC" stopOpacity="0" />
          </radialGradient>
          <linearGradient
            id="paint2_linear_5190_741"
            x1="78.2734"
            y1="78.981"
            x2="33.0186"
            y2="33.7262"
            gradientUnits="userSpaceOnUse"
          >
            <stop stop-color="white" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
            <stop offset="0.5" stop-color="white" stopOpacity="0" />
          </linearGradient>
        </defs>
      </svg>
    );
  }
}
