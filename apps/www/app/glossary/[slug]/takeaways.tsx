...
export default function Takeaways(props: z.infer<typeof takeawaysSchema>) {
  return (
    <Card className="w-full bg-white/5 shadow-[0_0_10px_rgba(255,255,255,0.1)] rounded-xl overflow-hidden relative border-white/20">
      ...
      <CardContent className="space-y-8 p-6">
        <div className="bg-white/10 p-4 rounded-md">
          ...
          <p className="text-sm text-white/80">{props.tldr}</p>
        </div>
          ...
          <Section
            ...
            content={
              <div className="space-y-2">
                {props.definitionAndStructure.map((item) => (
            ...
            content={
              <>
                <div className="flex flex-wrap gap-1 mb-2">
                  {props.usageInAPIs.tags.map((tag) => (
                    <Badge key={tag} variant="secondary" className="bg-white/10 text-white/80">
                      {tag}
                    </Badge>
                  ))}
                </div>
                <p className="text-sm text-white/60">{props.usageInAPIs.description}</p>
              </>
            }
          />
          <Section
        ...
        <Section
          ...
          content={
            <ul className="space-y-2 text-sm">
              {props.recommendedReading.map((item) => (
                <li key={item.title}>
                  <a
                    href={item.url}
                    target="_blank"
      ...
      <CardFooter className="border-t text-xs border-white/10">
        <div className="grid grid-cols-[auto_1fr_auto] gap-4 items-center w-full mt-6">
          ...
          <span className="text-white/60">{props.didYouKnow}</span>
