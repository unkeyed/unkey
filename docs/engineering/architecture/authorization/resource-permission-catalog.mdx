...
supported actions. GitHub apps and root keys are workspace-scoped. Every other
catalog resource is project-scoped.
...
└── Project [read, write, delete]
    ...
    │
    ├── Portal [read, write, delete]
    │   projects/{project_id}/portals/{portal_id}
    │   │
    │   └── Session [write]
    │       projects/{project_id}/portals/{portal_id}/sessions/{session_id}
    │
...
`POST /v2/rootKeys.createKey` accepts verified WorkOS workspace admins or root
keys with this capability, including global administrator grants:

```plaintext
unkey:v1:ws_123:rootKeys/*#write
```

The exact legacy equivalent is `workspace.*.create_root_key`. The workspace is
the caller's authorized customer workspace, not the internal storage workspace.
Creation uses `*` because the root key does not exist yet. A concrete key grant
does not cover this wildcard.

The endpoint stores URN inputs without adding legacy grants. Legacy inputs
store both equivalent grants atomically. Duplicate grants are stored once.
Callers can delegate creation, but every requested and generated grant must
remain within their effective permissions. Translations without exact safe
equivalents fail without creating a key.
...
Use `write` with a wildcard portal ID. The portal's ID is minted during the
request, so no grant can name it when the check runs. The project ID is
concrete, because it is resolved from the app or keyspace the portal maps to
before the check.

```plaintext
unkey:v1:ws_123:projects/proj_123/portals/*#write
```

Pointing a portal at a keyspace, on creation or on a later update, additionally
requires `read` on that keyspace. Choosing which resource a portal serves is not
the same authority as granting access to it.

### Mint a portal session

Minting uses `write` on the portal's `sessions` child with a wildcard session
ID, because the session does not exist yet.

```plaintext
unkey:v1:ws_123:projects/proj_123/portals/portal_123/sessions/*#write
```

A grant naming the portal does not reach its `sessions` child, so administering
a portal does not imply minting sessions for it.

<Warning>
  Minting additionally requires a root key. A session URL authenticates as an
  arbitrary end user, so no WorkOS role grants this permission and a dashboard
  token is refused even when its role carries the global admin permission.
</Warning>
