...
`POST /v2/rootKeys.createKey` requires this permission:

```plaintext
unkey:v1:ws_123:rootKeys/*#write
```

The created root key cannot have more permissions than the caller. Only
canonical URN permissions are accepted. Legacy permissions are not translated.
