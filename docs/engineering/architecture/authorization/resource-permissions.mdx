...
contract. The permission layer adds an action suffix and pattern matching rules.
...
Action wildcards are not valid in `v1`. A permission must name one explicit
action.
...
descendants only. It does not match the path before `/**`.
...
It does not match the project itself:
