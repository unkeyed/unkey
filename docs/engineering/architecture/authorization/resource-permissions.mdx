...
<AccordionGroup>
  ...
  <Accordion title="Portal session grammar (not yet honored)">
    ...
    ```plaintext
    unkey:v1:ws_123:portals/pc_123#create_portal_session
    unkey:v1:ws_123:portals/*#create_portal_session
    ```

    These show the grammar only. `portal.createSession` evaluates legacy tuples
    today, so neither form authorizes it yet. Grant
    `portal.pc_123.create_portal_session` instead until the URN migration lands.
...
Portals sit directly under the workspace, not under a project or app. A
`projects/*/apps/*` grant therefore does not imply portal access, even when the
portal is bound to that app.
