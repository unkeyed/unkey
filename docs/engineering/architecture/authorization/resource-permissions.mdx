...
Project-owned resources have an expressible project parent. Creating project
children, including keyspaces, identities, roles, and permissions, authorizes
the project that receives them. Creating a project is the exception because its
workspace parent isn't an expressible resource path in `v1`.
...
unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123#create_key
...
such as `read_keyspace` on `projects/proj_123/keyspaces/ks_123`. The action name
determines which operation is authorized.

Creating project children authorizes the project. Creating a project targets
the project collection wildcard because there is no workspace resource path:

```plaintext
unkey:v1:ws_123:projects/proj_123#create_keyspace
unkey:v1:ws_123:projects/proj_123#create_identity
unkey:v1:ws_123:projects/proj_123#create_role
...
rbac.U(
	urn.New().Workspace(workspaceID).Project(projectID).Keyspace(keyspaceID),
...
Stored permissions can contain resource patterns. Audit logs must use concrete
URNs. Authorization checks use concrete URNs when ownership is available.

Handlers for records without stored project ownership use `projects/*` as an
unknown-project placeholder. These requirements can only be satisfied by
all-project or workspace-global grants.
...
The table defines the target mapping. Each permission becomes usable when its
handler adopts the corresponding typed URN authorization check.

| Existing permission | Resource permission |
| --- | --- |
| `api.*.create_api` | `unkey:v1:{workspace_id}:projects/*#create_keyspace` |
| `api.{api_id}.read_api` | `unkey:v1:{workspace_id}:projects/*/keyspaces/{keyspace_id}#read_keyspace` |
| `api.{api_id}.create_key` | `unkey:v1:{workspace_id}:projects/*/keyspaces/{keyspace_id}#create_key` |
| `api.{api_id}.read_key` | `unkey:v1:{workspace_id}:projects/*/keyspaces/{keyspace_id}/keys/*#read_key` |
| `api.{api_id}.verify_key` | `unkey:v1:{workspace_id}:projects/*/keyspaces/{keyspace_id}/keys/*#verify_key` |
| `identity.*.read_identity` | `unkey:v1:{workspace_id}:projects/*/identities/*#read_identity` |
| `ratelimit.*.delete_override` | `unkey:v1:{workspace_id}:projects/*/ratelimits/namespaces/*/overrides/*#delete_override` |
| `rbac.*.create_role` | `unkey:v1:{workspace_id}:projects/*#create_role` |
...
| `unkey:v1:ws_123:projects/*/keyspaces/*/keys#read_key` | Does not match a catalog path shape. |
