...
Creation actions target the resource being created. When its concrete ID is not
known yet, use `*` for that resource's ID segment. For example, `create_key`
targets the key path under its owning keyspace with a wildcard key ID, while
`read_key`, `update_key`, and `delete_key` target concrete key resources or
...
unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123/keys/*#create_key
unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123/keys/key_456#delete_key
...
Create actions authorize the resource being created. When its ID is not known
until after the operation succeeds, handlers use `*` for that single ID segment.

For example, creating a key uses the key resource shape with a wildcard key ID:

```plaintext
unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123/keys/*#create_key
```

Creating project and deploy resources follows the same rule:

```plaintext
unkey:v1:ws_123:projects/*#create_project
unkey:v1:ws_123:projects/proj_123/apps/*#create_app
unkey:v1:ws_123:projects/proj_123/apps/app_456/environments/*#create_environment
unkey:v1:ws_123:projects/proj_123/apps/app_456/environments/env_789/deployments/*#create_deployment
unkey:v1:ws_123:projects/proj_123/apps/app_456/environments/env_789/domains/*#create_domain
unkey:v1:ws_123:projects/proj_123/apps/app_456/environments/env_789/variables/*#create_variable
```

Go handlers can pass typed URN builders directly to `rbac.U` when the builder
represents the resource being created. Use typed actions from `pkg/rbac/permissions`,
...
rbac.U(
	urn.New().Workspace(workspaceID).Project(projectID).Keyspace(keyspaceID).Key("*"),
	permissions.CreateKey{},
)

rbac.U(
	urn.New().Workspace(workspaceID).Project(projectID).App("*"),
	permissions.CreateApp{},
)

rbac.U(
	urn.New().Workspace(workspaceID).Project(projectID).App(appID).Environment("*"),
...
Stored permissions can contain resource patterns. Resources recorded in audit
logs must always use concrete URNs. Authorization checks also use concrete URNs,
except create checks can use one `*` for the unknown created-resource ID.
...
```plaintext
unkey:v1:ws_123:projects/proj_123/keyspaces/*/keys/*#read_key
```
...
```plaintext
unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123#read_key
unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123/keys/key_456#delete_key
unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123/keys/key_456#update_key
```
...
<AccordionGroup>
  ...
  <Accordion title="Read all keys in one keyspace">
    ...
    ```plaintext
    unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123/keys/*#read_key
    ```
  ...
  <Accordion title="Manage one role">
    ...
    ```plaintext
    unkey:v1:ws_123:projects/proj_123/rbac/roles/role_123#update_role
    ```
...
```json
{
  "actor": {
    "type": "key",
    "id": "key_root_123",
    "urn": "unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123/keys/key_root_123"
  },
  "resource": {
...
{
  "resource": {
    "urn": "unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123/keys/key_456",
    "type": "key"
  },
  "targets": [
    {
      "urn": "unkey:v1:ws_123:projects/proj_123/rbac/roles/role_123",
      "type": "role"
    }
  ],
  "action": "add_role"
...
| --- | --- |
| `api.*.create_api` | `unkey:v1:{workspace_id}:projects/{project_id}/keyspaces/*#create_keyspace` |
| `api.{api_id}.read_api` | `unkey:v1:{workspace_id}:projects/{project_id}/keyspaces/{keyspace_id}#read_keyspace` |
| `api.{api_id}.create_key` | `unkey:v1:{workspace_id}:projects/{project_id}/keyspaces/{keyspace_id}/keys/*#create_key` |
| `api.{api_id}.read_key` | `unkey:v1:{workspace_id}:projects/{project_id}/keyspaces/{keyspace_id}/keys/*#read_key` |
| `api.{api_id}.verify_key` | `unkey:v1:{workspace_id}:projects/{project_id}/keyspaces/{keyspace_id}/keys/*#verify_key` |
| `identity.*.read_identity` | `unkey:v1:{workspace_id}:projects/{project_id}/identities/*#read_identity` |
| `ratelimit.*.delete_override` | `unkey:v1:{workspace_id}:projects/{project_id}/ratelimits/namespaces/*/overrides/*#delete_override` |
| `rbac.*.create_role` | `unkey:v1:{workspace_id}:projects/{project_id}/rbac/roles/*#create_role` |
...
| Value | Reason |
| --- | --- |
| `unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123` | Missing the action suffix. |
| `unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123.read_keyspace` | Uses the old tuple separator. |
| `unkey:v1:ws_123:projects/proj_123/keyspaces/ks_123#*` | Uses an action wildcard outside the global resource pattern. |
...
| `unkey:v1:ws_123:projects/proj_123/keyspaces/*/keys#read_key` | Does not match a catalog path shape. |
