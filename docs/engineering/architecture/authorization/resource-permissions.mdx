...
`decrypt`, `verify`, and `limit` are special actions. Use them only on
catalog resources that support them.

## Create and update rules

Use `write` for create and update operations, including root key creation.
Root key creation never bypasses the caller's permission ceiling. See
[Create a root key](/architecture/authorization/resource-permission-catalog#create-a-root-key).
