...
```plaintext
{prefix[1-7]}_{random[8]}unkeyv{version[1]}{random[36]}{checksum[6]}
```
...
But the version is - by design - not random and if it's part of the shown data (first 4 chars and 4 last chars) it doesn't help users to see a difference between keys.
I chose to put it after 8 chars of randomness. This works, but also means that this is now fixed for eternity. A new format must also embed its version after the first 8 chars, otherwise we couldn't parse a key reliably if we ever wanted to.  
...
It provides 256 random bits and base58 encodes that into 44 characters, split into 8 and 36 sections. Prefixes, markers, and checksums add
no entropy.
...
checksum for partner patterns. The scanner or webhook validates CRC-32 before
...
existing `start`, and use `""` for unavailable fields.
...
We should remove the option to choose a custom length and just follow the NIST recommendation of 256 bits of entropy. There's no good reason for us to allow a user to use less.
