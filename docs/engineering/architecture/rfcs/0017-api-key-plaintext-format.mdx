...
```plaintext
{prefix[1-8]}_{random[8]}unkeyv{version[1]}{depending on the version}
```
...
```plaintext
{prefix[1-8]}_{random[8]}unkeyv1{random[36]}{checksum[6]}
```


| Field              | Length | Contents                       |
| ------------------ | ------: | ------------------------------ |
| Prefix             | 1–8    | User controlled prefix         |
| Separator          | 1      | `_`                            |
...
The complete key is between 59 and 66 characters.
...
```plaintext
^[A-Za-z0-9_]{0,7}[A-Za-z0-9]$
```

They contain one through eight ASCII letters, numbers, or underscores, and must
end in an alphanumeric character. `prod_sk`, `prod_key`, and `unkey` are valid.
`prod_sk_` or nine-character prefixes are invalid.
...
```plaintext
[A-Za-z0-9_]{0,7}[A-Za-z0-9]_[1-9A-HJ-NP-Za-km-z]{8}unkeyv1[1-9A-HJ-NP-Za-km-z]{42}
```
