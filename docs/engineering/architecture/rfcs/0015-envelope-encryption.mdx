...
2. Read `/var/run/unkey/kek/<kek_id>` matching the envelope's `kek_id` (local file, no network)
...
S3 is treated as a plain filesystem. Each KEK is its own file, encrypted individually with the root key. kek-agent lists the prefix and downloads all files.

```
s3://unkey-kek/
├── kek-2026-01.enc    ← AES-256-GCM encrypted with root key
├── kek-2026-02.enc    ← AES-256-GCM encrypted with root key
└── ...
```

Adding a new KEK is uploading a file. Removing an old KEK is deleting a file. No monolithic config to manage.
...
A DaemonSet running on every node. Its only job: list KEK files from S3, decrypt each with the root key, and write them to the node's tmpfs.

1. At startup: list `s3://unkey-kek/`, download each `.enc` file, decrypt with root key
2. Write each plaintext KEK to `/var/run/unkey/kek/<kek-id>` on host tmpfs
3. Serve a health endpoint
4. Periodically re-list S3 (picks up new/removed KEKs without DaemonSet redeploy)
...
```
/var/run/unkey/kek/
├── kek-2026-01    ← plaintext 32-byte key, base64-encoded
├── kek-2026-02
└── ...
```

The sealed envelope's `kek_id` maps directly to a filename. tmpfs means the plaintext KEKs only ever live in memory — never on disk, never in etcd. Node reboot wipes it. kek-agent rewrites it on startup.
...
1. **Add new KEK**: Upload `kek-2026-03.enc` to S3. kek-agent's periodic re-list picks it up on all nodes automatically. Nothing breaks — old envelopes still decrypt with the old key.
...
3. **Remove old KEK**: Once no active deployment uses the old `kek_id` (query the DB), delete `kek-2026-02.enc` from S3.
