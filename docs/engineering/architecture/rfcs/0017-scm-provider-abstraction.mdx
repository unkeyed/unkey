...
When we started building Deploy, Github was the most popular and the best answer. There were alternatives but not a replacement.
...
Of course, Github will have some extra love, because of fork-PR.
...
- `Complete` is the `connectRepo` write we already do today, plus creating the webhook on the
  repo. Github never needed the hook part, its App webhook already exists.
