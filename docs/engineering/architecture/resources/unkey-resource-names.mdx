...
```plaintext
unkey:v1:ws_123:projects/proj_123/keyspaces/*/keys/*
```
...
Key resources are rooted under the project and keyspace that own the key.
...
Identity resources are rooted under the project that owns them.
...
Rate limiting resources are rooted under the project that owns them. Overrides
...
RBAC resources are rooted under the project that owns them. Relationship
changes, such as adding a role to a key, are audited against both affected
resources rather than by creating a separate join-table URN.
...
| Gateway | `projects/{project_id}/apps/{app_id}/environments/{environment_id}/gateway` |
| Gateway policy | `projects/{project_id}/apps/{app_id}/environments/{environment_id}/gateway/policies/{policy_id}` |
| Gateway route | `projects/{project_id}/apps/{app_id}/environments/{environment_id}/gateway/routes/{route_id}` |
...
unkey:v1:ws_123:projects/proj_123/apps/app_456/environments/env_789/deployments/d_abc
unkey:v1:ws_123:projects/proj_123/apps/app_456/environments/env_789/gateway/policies/pol_123
```
