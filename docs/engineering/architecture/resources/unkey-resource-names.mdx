...
For records without stored project ownership, `projects/*` can precede a
concrete identity, keyspace, portal, rate-limit, role, or permission ID. This
placeholder means the owning project is unavailable when used in an
authorization request. In a stored grant, `*` retains its normal all-project
wildcard meaning.
...
The public catalog defines every canonical resource path that can appear in a
`v1` URN. Typed builders produce these paths. `ParseV1` validates the shared path
grammar, not catalog membership, so parsing an unknown path doesn't make it a
canonical resource. Pattern grants must be built from catalog paths, with `*`
replacing complete ID segments or trailing `/**` covering descendants.

APIs aren't URN resources. Legacy API operations authorize the project that
receives a keyspace or the keyspace backing the API.
...
Keyspaces belong to projects. Key resources are rooted under the keyspace that
owns the key.

| Resource | Path |
| --- | --- |
| Keyspace | `projects/{project_id}/keyspaces/{keyspace_id}` |
| Key | `projects/{project_id}/keyspaces/{keyspace_id}/keys/{key_id}` |
...
Identities are shared by all keyspaces in their project.

| Resource | Path |
| --- | --- |
| Identity | `projects/{project_id}/identities/{identity_id}` |

Example:

```plaintext
unkey:v1:ws_123:projects/proj_123/identities/id_123
```

### Rate limits

Standalone rate limiting resources belong to projects. Overrides belong to the
namespace they modify.

| Resource | Path |
| --- | --- |
| Namespace | `projects/{project_id}/ratelimits/namespaces/{namespace_id}` |
| Override | `projects/{project_id}/ratelimits/namespaces/{namespace_id}/overrides/{override_id}` |

Examples:

```plaintext
unkey:v1:ws_123:projects/proj_123/ratelimits/namespaces/rlns_123
unkey:v1:ws_123:projects/proj_123/ratelimits/namespaces/rlns_123/overrides/rlor_456
```

### RBAC

RBAC resources belong to projects. Relationship changes, such as adding a role
to a key, are audited against both affected resources rather than by creating a
separate join-table URN.

| Resource | Path |
| --- | --- |
| Role | `projects/{project_id}/rbac/roles/{role_id}` |
| Permission | `projects/{project_id}/rbac/permissions/{permission_id}` |

Examples:

```plaintext
unkey:v1:ws_123:projects/proj_123/rbac/roles/role_123
unkey:v1:ws_123:projects/proj_123/rbac/permissions/perm_456
```
...
Portal resources belong to projects. Session tokens and sessions belong to the
portal that created them.

| Resource | Path |
| --- | --- |
| Portal | `projects/{project_id}/portals/{portal_id}` |
| Portal session token | `projects/{project_id}/portals/{portal_id}/session_tokens/{portal_session_token_id}` |
| Portal session | `projects/{project_id}/portals/{portal_id}/sessions/{portal_session_id}` |
| Portal branding | `projects/{project_id}/portals/{portal_id}/branding` |

Examples:

```plaintext
unkey:v1:ws_123:projects/proj_123/portals/portal_123
unkey:v1:ws_123:projects/proj_123/portals/portal_123/sessions/ps_456
```
