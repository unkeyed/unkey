...
The gateway path has no id. It is a namespace that groups the gateway resources of an environment, such as policies and routes, so a grant can cover the gateway alone.
