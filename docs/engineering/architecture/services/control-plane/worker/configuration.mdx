...
`build.backend` selects how builds run. The default `depot` backend runs builds on Depot.dev and is what production uses; builds are enabled when `registry.password` is set, in which case `registry.repository`, `registry.username`, `build.depot.api_url`, and `build.depot.project_region` must also be set.
...
| `registry.repository` | string | - | Registry repository path, e.g. `registry.depot.dev/abc123`.
