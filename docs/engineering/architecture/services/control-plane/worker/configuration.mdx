...
| `restate.ingress_url` | string | - | Restate ingress URL, used by operator tooling (e.g. sentinel rollouts). Injected from the same secret as the worker's Restate credentials.
| `restate.ingress_api_key` | string | - | Bearer token for the Restate ingress. Injected via env.
...
ingress_url = "${UNKEY_RESTATE_INGRESS_URL}"
ingress_api_key = "${UNKEY_RESTATE_INGRESS_API_KEY}"
http_port = 9080
