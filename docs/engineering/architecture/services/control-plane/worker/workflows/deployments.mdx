...
**Replay detection.** Ctrl sends a per-request nonce and the handler returns the nonce of the request whose execution produced the response. A retry that attached reads back a different nonce, reports `replayed=true`, and answers with the row's current status instead of the journaled pending.

**Target reload.** The create request carries only ids; the worker resolves the target itself when it runs. A snapshot in the request would be journaled and could go stale. Reloading means the row always records the settings and secrets current at insert time, and a target deleted mid-create fails the invocation instead of leaving an orphan row.
