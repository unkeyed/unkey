...
Because the row is the only record a key exists, a key stays bound only while its row survives. Purging a row frees its derived id, so a reused key creates a new deployment instead of replaying. This already happens: `Delete` in the environment worker hard-deletes the environment's deployment rows, and project and app deletes cascade into it. Callers cannot observe the release, because a deleted target has no deployment left to replay. Any future retention or archival that purges rows of a live target must keep the id claimed (tombstone or archive table), or the contract breaks where callers can see it.

A keyed retry that finds an existing row is classified in `resolveKeyedRetry` ([`svc/ctrl/services/deployment/create_deployment.go`](https://github.com/unkeyed/unkey/blob/main/svc/ctrl/services/deployment/create_deployment.go)):
...
**Heal** (`healDeployment`) revives a stuck row: skip the insert, send the workflow, and write the invocation id onto the row. The build is pinned to what the row records, not the retry body, because the row defines what the deployment id means and the branch or body may have changed since the original attempt. A recorded image pins a docker build (docker-source creates record their image at insert for exactly this reason). A recorded commit without an image pins a git build: the retry body cannot repoint it at an image, and a repo connection deleted since the crash refuses the heal instead of silently falling back to the current deployment's image. That refusal also marks the row failed, spending the key: the build it pins can never run, so leaving it pending would loop the caller on the same key forever. The spend records a `queued` step ended with the reason, in the same transaction as the status change, so the dashboard shows why the deployment died instead of an empty timeline. Transient resolution errors (a GitHub outage) leave the row pending and healable. Command always comes from the row. A row recording neither an image nor a commit falls back to the retry body, but no such row can reach heal: every insert records one or the other, and rows that predate image-at-insert carry random ids that no derived id can name. That branch is defensive only. Sibling dedup runs with the stuck row's own creation time, so a heal never supersedes deployments newer than the row it revives. A send failure never marks a healed row failed, so the next retry can heal again.
