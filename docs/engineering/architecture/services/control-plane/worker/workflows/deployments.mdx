...
sequenceDiagram
  ...
  DB-->>Worker: ReportDeploymentStatus threshold met<br/>→ NotifyReadiness(STATE_READY) resolves promise
...
3. Check the database once for instances that are already healthy. If the threshold is met, return without awaiting the promise.
4. Otherwise await the promise, racing it against `regionReadyTimeout`. A result that arrived before the wait is retained by the promise.

`DeployWorkflow.NotifyReadiness` resolves the promise with `restate.Void` for `STATE_READY` and rejects it for a typed startup failure. Rejection also fails older runs that await `restate.Void`, instead of letting them interpret a failure payload as success. This `SHARED` handler runs concurrently with the suspended run. The first completion wins, including when it arrives before the run awaits. Concurrent notifications do not have a guaranteed execution order. Missing or unknown states are rejected without completing the promise. `STATE_CONTAINER_CONFIG_ERROR` is ignored because it can be transient. The deprecated `NotifyInstancesReady` RPC forwards to this handler for queued notifications.
...
6. If threshold met (and not yet notified), send `NotifyReadiness(state=STATE_READY)` to `DeployWorkflow`, keyed by deployment ID.

Krane maps the Kubernetes termination reasons `OOMKilled`, `ContainerCannotRun`, and containerd's `StartError` to `TerminationCause`. It maps the waiting reasons `CrashLoopBackOff`, `CreateContainerConfigError`, and `InvalidImageName` to `WaitingCause`. Raw reasons and messages remain available for diagnostics. `CreateContainerConfigError` remains diagnostic-only because Kubernetes can briefly report a missing Secret during startup. Temporary states such as `ErrImagePull`, `ImagePullBackOff`, and `ContainerCreating` do not trigger early failure and retain the existing readiness timeout.

`cluster.Service.ReportInstanceEvents` sends the matching failure state on the first fatal classified failure of the `deployment` container while the deployment has status `deploying` and its Deploy invocation is live. It uses Restate's asynchronous `Send`, waiting for durable acceptance rather than worker completion. The deployment lookup, liveness check, and acceptance request share a 5-second timeout per failure notification. Failed acceptance returns an error and clears Krane's deduplication entry. Krane retries only if another pod-watch update arrives. If the pod stays unchanged, early failure can be missed and the existing 15-minute readiness timeout remains the fallback. This path runs before diagnostic buffering and does not depend on ClickHouse.

Deploy the worker before ctrl-api, then deploy Krane. Older Krane versions omit `cause`, which defaults to the unspecified enum value and does not trigger early failure. Ctrl does not infer the cause from the diagnostic string.

If a failure result wins, the workflow records an actionable message in MySQL's `deployment_steps.error`. OOM advice recommends increasing the memory limit or reducing startup memory use. Other messages point to runtime logs, the image entrypoint, or the image reference. The dashboard displays this message under the deploying step. The existing failure path marks the deployment failed, stops it, and releases its build slot. No failure-history table is required.
...
The Deploy handler maintains a LIFO compensation stack registered via `Compensation.Add` (for side-effects wrapped in `restate.RunVoid`) and `Compensation.AddCtx` (for raw Restate context operations like `BuildSlotService.Release().Send`). The stack fires on any error or cancellation:
