...
2. Require `numRegions - 1` healthy regions (minimum 1, tolerating one regional outage). A region counts
   as healthy only when it runs at least one instance, or more when its `autoscaling_replicas_min` asks
   for more.
