...
`CreateDeployment` accepts an optional caller-supplied idempotency key. Restate owns the key: ctrl calls `DeploymentCreateService.Create` synchronously with an invocation idempotency key of `workspace_id/app_id/environment_id/idempotency key`, so a retry inside the retention window attaches to the original invocation and receives its journaled response instead of creating anything. There is no key table; deployment ids are random. The prefix scopes the key to its target: the same caller key in another workspace, app, or environment runs its own invocation, so one key can deploy the same commit to prod and staging, and a reused key can never answer with another target's deployment.

**Key lifetime.** The service registers with an idempotency retention of 12 hours, which is the caller-facing key lifetime: a retry inside the window replays, a retry after it creates a new deployment. This requires a Restate server version that accepts idempotency retention from service discovery; verify before deploying a worker with this handler against an older cluster. Promote control-worker before control-api: until the worker registers this handler, every create from ctrl fails with "service not found".
...
**Audit.** The `deployment.create` audit log commits in the same MySQL transaction as the deployment insert, inside the create worker, so a row created through this handler always has its create audit. Rebuilds skip the create audit; ctrl records a `deployment.rebuild` event instead. Replays answer from the journal and write nothing.

**Caller timeout.** Ctrl bounds the synchronous call at 30 seconds. On expiry the invocation keeps running to completion; the caller gets `Unavailable` and a keyed retry attaches to the still-running invocation. A keyless retry cannot attach and creates a second deployment, so the error tells keyless callers to check the named deployment instead of retrying. Persistent handler failures (for example a database outage) exhaust the service retry policy and pause the invocation for operator resolution; it still completes when resumed. Resume a paused create, do not kill it: a killed create leaves its row pending with no invocation id for `CancelDeployment` to act on.
