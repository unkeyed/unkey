...
sequenceDiagram
  ...
  and pod readiness
    Worker->>Worker: waitForDeployments stores readiness awakeable
    CtrlAPI-->>Worker: NotifyInstancesReady resolves awakeable
  end
...
After `createTopologies`, `ensureSentinelRows`, and `ensureCiliumNetworkPolicy`, the Deploy handler fires off `SentinelService.Deploy` RPCs as `RequestFuture`s (non-blocking) and then enters `waitForDeployments`. Both waits happen in parallel on krane's side.
