...
`DeployWorkflow`, the Restate workflow replacing `DeployService`, needs no awakeable or object state: its Deploy run awaits a durable promise named `instances_ready`, and its `NotifyInstancesReady` resolves that promise. A report that arrives before the run awaits is kept by the promise. It is a workflow rather than a virtual object because Deploy has no operations to serialise.
