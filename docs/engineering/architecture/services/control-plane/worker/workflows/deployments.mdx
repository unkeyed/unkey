...
That last part needs `Build` to stay unsuspended for the whole build, which is why it is bound with `WithInactivityTimeout(deploy.BuildKeepAliveWindow)`. `restate.Run`'s context is cancelled with the invocation's HTTP/2 stream and not on suspend, so a suspended Build cannot be told it was cancelled: it would run to completion and hold the workspace's slot until it did.
