...
- Only builds are capped. A rule counts an invocation for as long as Restate is running it, including time the handler spends waiting, and the slot is freed only when Restate suspends the invocation after about a minute of no activity. A rule on the deploying phase would hold a slot through the wait for pods and then send the resumed deployment to the end of the queue, so it would delay deployments without capping them.
