...
- **`DeployWorkflow`** is keyed by `deployment_id`. Each deployment is one workflow run, so multiple deployments in the same environment can build in parallel. `DeployService`, the virtual object it replaces, stays registered until every Deploy that started on it has finished.
...
`DeployWorkflow`, the Restate workflow replacing `DeployService`, needs no awakeable or object state: its Deploy run awaits a durable promise named `instances_ready`, and its `NotifyInstancesReady` resolves that promise. A report that arrives before the run awaits is kept by the promise. It is a workflow rather than a virtual object because Deploy has no operations to serialise, and Restate flow control, planned for per-workspace deploy limits, is native on workflows but experimental on virtual objects.
...
6. If threshold met (and not yet notified), send `NotifyInstancesReady(deployment_id)` to `DeployWorkflow` and, until it is deleted, to `DeployService`.
