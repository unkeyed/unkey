...
A repo connection deleted since the crash refuses the heal instead of silently falling back to the current deployment's image. The refusal leaves the row pending and healable, so reconnecting the repository makes the same key work. It reaches API callers as a 412, and clients drop a key on any 4xx, so a caller sends a new key rather than looping on this one.
