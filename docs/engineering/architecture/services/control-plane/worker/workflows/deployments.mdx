...
Deploy waits for Build (`Request`, not `Send`), so cancelling Deploy cancels the Build too. A Build still waiting is removed and never runs; one already running finishes its current step first, so the image may still be pushed.
...
- `Build` refuses to run outside scope `builds` or with a limit key other than its workspace.
...
`$ADMIN` and `$TOKEN` are the Restate Cloud admin URL and token; [runbook 0001](https://github.com/unkeyed/infra/blob/main/docs/runbooks/restate/0001-control-worker-registration.md) says where to get them. Not yet run against Cloud.

Read the rules (there is no `GET /limits/rules`):

```bash
curl -sS "$ADMIN/query" -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' -H 'Accept: application/json' \
  -d '{"query":"SELECT pattern, concurrency, disabled, version FROM sys_rules"}'
```

If `builds/*` is missing, every workspace builds without a limit until the next cron run. Run it now with `kubectl -n restate-system create job --from=cronjob/restate-build-limit-sync build-limit-sync-manual`.

Builds waiting or running:

```bash
curl -sS "$ADMIN/query" -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' -H 'Accept: application/json' \
  -d '{"query":"SELECT target, created_at, running_at FROM sys_invocation WHERE target_handler_name = '"'"'Build'"'"' AND status <> '"'"'completed'"'"'"}'
```

A row with no `running_at` is waiting for a slot.
