---
title: Deploy Billing Push
description: How month-to-date Deploy usage flows from ClickHouse to Stripe billing meters every hour.
---
...
The push reports the absolute period-to-date total every tick rather than per-tick deltas. That single decision removes most of the failure modes a usage pipeline normally has: there are no deltas to deduplicate, no exactly-once delivery requirement, and no end-of-month timing window to get right. The last value Stripe receives before the invoice finalizes is the one it bills.
...
3. Resolves each workspace's Stripe customer ID from MySQL and drops workspaces that are disabled or have no customer.
...
sequenceDiagram
    ...
    Handler-->>Restate: workspaces_pushed, meters_pushed
```
...
| ------ | --------------------------- | ------------------- |
| CPU    | `deploy.cpu_seconds`        | CPU-seconds         |
| Memory | `deploy.memory_gib_seconds` | GiB-seconds         |
| Disk   | `deploy.disk_gib_seconds`   | GiB-seconds         |
| Egress | `deploy.egress_public_gib`  | binary GiB (2^30)   |
...
1. The `deploy.*` meters are managed in the infra repo and are already applied to Flo's sandbox, so there's nothing to apply from here. (To stand up a fresh sandbox, follow the [infra Stripe guide](https://github.com/unkeyed/infra/blob/main/docs/services/stripe-billing.md).)

2. Give the worker a test-mode key. In local dev (`mise run dev`), copy `dev/.env.stripe.example` to `dev/.env.stripe` and set a `sk_test_...` key from Flo's sandbox:
