...
| --- | --- | --- |
| DeployWorkflow | `deployment_id` | Workflow: one durable run per deployment, signalled by a promise when instances are ready. |
| DeploymentService | `deployment_id` | Desired state changes: scheduled transitions with nonce-based last-writer-wins, and immediate stop and wake. |
...
| CronService | `varies per handler` | Unified entry point for all scheduled tasks (quota check, key refill, key last-used sync, audit log export, audit log outbox cleanup, ratelimit global counters cleanup, build limit sync). Per-handler VO keys: billing period, date, or a fixed per-task slug. |
| KeyLastUsedPartitionService | `partition_index` | Per-partition fan-out target for `CronService.RunKeyLastUsedSync`. |
| DeployAnomalyShardService | `<window_start_ms>/<shard>/<shard_count>` | Runs partitioned ClickHouse candidate scans for one anomaly window. |
| DeployAnomalyService | `<workspace_id>/<app_id>/<environment_id>` | Serializes anomaly candidate, open-alert, and recovery state for one production group. |
...
- [Deploy anomaly detection](/architecture/services/control-plane/worker/deploy-anomaly-detection)
