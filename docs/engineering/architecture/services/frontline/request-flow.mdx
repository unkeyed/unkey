...
handler removes it and verifies the HS256 JWT. A request without valid metadata
starts with an empty hop history. Frontline treats empty, duplicate, invalid,
expired, or oversized metadata as absent. Invalid metadata never blocks a
request.
