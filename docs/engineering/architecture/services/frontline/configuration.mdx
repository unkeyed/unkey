...
<ResponseField name="frontline_meta_signing_key" type="string" required>
  Shared key that signs metadata between Frontline regions. Set the same key in
  every region in one environment.
</ResponseField>
...
<ResponseField name="UNKEY_FRONTLINE_META_SIGNING_KEY" type="env" required>
  Shared key that signs metadata between Frontline regions.
</ResponseField>
