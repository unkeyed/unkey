...
<ResponseField name="version" type="string">
  Schema version of the Principal payload. Currently `"v1"`. Bumped only on breaking changes to the JSON shape. Kept as a string so future schemes (date-based, semver, named) can replace the value without changing the field type.
</ResponseField>
...
<ResponseField name="type" type="PrincipalType">
  Which authentication method produced this Principal. `API_KEY` or `JWT`.
</ResponseField>
