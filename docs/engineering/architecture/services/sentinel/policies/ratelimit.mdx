...
RateLimit defines gateway-level rate limiting with configurable identifiers. Frontline executes Sentinel RateLimit policies and delegates counter state to [rate limiting](/architecture/ratelimiting/overview), so policy execution uses the same distributed counters as API and Frontline rate-limit checks.
