...
Auth policies (KeyAuth, JWTAuth, BasicAuth) produce a [Principal](/architecture/services/sentinel/policies/principal) containing a subject and claims. Downstream policies can reference this principal. For example, a RateLimit policy can use the authenticated subject as its bucket key, giving each user their own rate limit window instead of a shared one.

```
Policy 1: KeyAuth        → authenticates the request, sets Principal
Policy 2: RateLimit      → rate limits per Principal.subject
```
...
```
Policy 1: RateLimit  match: /v1/*           → 1000 req/min
Policy 2: RateLimit  match: /v1/expensive/* → 10 req/min
```

Because policies evaluate in order, place more specific match expressions before broader ones. A request to `/v1/expensive/foo` matches both policies, and the first match wins since a rejection stops evaluation.
...
| [BasicAuth](/architecture/services/sentinel/policies/basicauth)        | Schema only |
| [RateLimit](/architecture/services/sentinel/policies/ratelimit)        | Schema only |
| [IP rules](/architecture/services/sentinel/policies/ip-rules)          | Schema only |
