...
2. If the deployment was recently moved or recreated, the deployment cache may be stale (fresh: 30s, stale: up to 5min). Wait for the entry to expire.
...
During the stale window, sentinel serves old data while refreshing in the background. Deployment changes can take up to 5 minutes to propagate to every sentinel node, because each node refreshes its caches on its own TTL.
