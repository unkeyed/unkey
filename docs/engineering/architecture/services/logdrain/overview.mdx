---
title: "Overview"
description: "How the logdrain service streams audit logs and key verifications to customer destinations"
---

The logdrain service (`svc/logdrain`) streams audit logs and key verifications from ClickHouse to
...
`key_verifications` selects `KeyVerificationStreamConfig`. Its `outcomes` list
matches exact verification outcomes, such as `VALID`, `RATE_LIMITED`, and
`EXPIRED`. Empty selects all outcomes, including future outcomes. The dashboard
uses the existing verification outcome catalog, including the empty outcome
shown as “Unspecified”. Audit event names are not verification outcomes.
Its `key_space_ids` list selects exact keyspace IDs within the workspace. Empty
selects all keyspaces. When both lists are set, a verification must match both
its outcome and keyspace. Changing either filter retains the composite cursor
and fences in-flight acknowledgments; skipped history is not replayed.
Tags remain in the payload for destination-side filtering.
...
selector. Older workers do not support verification drains.
...
Both streams have a lightweight `proj_logdrain` projection. The verification
...
Before enabling verification drains, apply ClickHouse migration
`20260910000000.sql` and the MySQL stream enum change. The ClickHouse migration
materializes zero insertion timestamps for historical rows, then enables the
server timestamp default and materializes the projection. Historical rows do
not become newly ingested events. This mutation reads existing parts and needs
capacity planning. It requires the same `_part_offset` projection support and
settings as audit logs; local tests use ClickHouse 26.2. Deploy workers before
the dashboard can create verification drains. Existing drains cannot change
streams; create a separate drain instead.
...
or the verification `request_id` as a deduplication key within its stream.

## Verification payload

Each HTTP record contains `event` (the verification payload) and `timestamp`
...
| `identity` | Object with `id` and `externalId`, both empty when absent. |
