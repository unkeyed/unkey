...
environments automatically. Selecting every current environment keeps
**Specific sources** mode and its resource restrictions. Clearing every source
in **Specific sources** mode blocks submission. Only **All sources** mode removes
resource restrictions. HTTP status and severity filters still apply. Switching modes
retains each stream's selections independently.
...
The stored protobuf `batch_size` limits events per page for each drain. Zero
or an omitted value uses 10,000 events. Backend create and update inputs accept
`batchSize`; the dashboard has no batch-size control. This is an event-count
limit, not a byte limit on the destination request.

Gateway reads also enforce a conservative 16 MiB serialized-size budget before
ClickHouse returns payload rows to Go. Each row is charged six times the JSON
length of its selected values, plus 1,024 bytes for field names and destination
envelopes. The factor covers worst-case JSON escaping. An ordered cumulative
sum selects a contiguous prefix. Identical cursor groups stay together at both
the row and byte limits. This bounds returned payloads, not ClickHouse query
memory or total process memory. Concurrent workers and encoding buffers need
additional memory.

An empty gateway result requires a metadata-only probe to distinguish an empty
window from a first cursor group that cannot fit. An oversized group returns an
error without returning its payload or advancing the cursor. It follows the
existing retry and pause policy. Neither a payload nor a cursor group is split
or truncated. If the probe observes a fitting group after an empty result, the
source returns a retryable error rather than declaring the window exhausted.

After each commit, the engine immediately reads another page unless
`caughtUp` is true. Only an empty page confirms window exhaustion. A nonempty
page always continues from its last event, even below the event-count limit.
It does not cause a poll delay while a backlog remains.
...
stateDiagram-v2
    ...
    Partial --> Page: last-event cursor / not caught up / reset size to 1 minute
    ...
    note right of Page
        ...
        Only empty pages can be caught up at the watermark.
...
windows stay at 60 minutes. Every nonempty batch resets the next query to
1 minute from the last event's cursor, including its event ID. Every query is
capped at the fixed watermark, even when the chosen window is larger than the
remaining interval.
...
A nonempty batch advances the cursor to its last event. Only an empty batch
...
The watermark is the poll's enqueue timestamp minus the fixed stream lag.
...
Audit logs use a fixed 5-minute watermark lag and a 60-second poll interval.
Key verifications, gateway HTTP requests, runtime logs, and rate limits use a fixed
...
Timing has no operator settings. Local development uses the same timing policy.
The engine scans MySQL every 5 seconds. A caught-up drain remains
...
Each HTTP record contains the verification fields at the top level,
`stream: "key_verifications"`, and `time`
(occurrence time in UTC RFC3339 with millisecond precision). Batch metadata
stays in request headers. Axiom adds `_time` equal to `time` to the same flat
record. HTTP JSON sends an array; NDJSON
