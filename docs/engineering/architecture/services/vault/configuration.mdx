...
(`success`, `canceled`, or `error`), and `error_code`.
Expected missing objects count as `success`: the storage operation completed
without an error, but the object does not exist.
...
logs. GET responses with HTTP 404 retain the existing missing-object behavior:
they count as `success` regardless of the provider error code.

A rising `outcome="canceled"` rate can also indicate a slow storage endpoint
causing callers to give up.
...
For reads, `success` includes missing objects and does not prove a DEK was read.
