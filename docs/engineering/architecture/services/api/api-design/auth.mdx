...
Authenticated endpoints must document required permissions in their OpenAPI
description. Use clear prose and wrap permission strings in backticks.

For endpoints that accept more than one valid permission, describe the alternatives
explicitly:

```plaintext
Requires `ratelimit.*.set_override` or
`ratelimit.<namespace_id>.set_override`.
```

Don't add machine-readable permission extensions by default. Permission prose is
the source of truth until API tooling needs a stricter contract.
