...
<ResponseField name="auth" type="object[]" required>
  ...
  <Expandable title="Fields">
    ...
    <ResponseField name="auth.issuer" type="string" required>
      Expected JWT `iss` claim for `type = "jwt"`. When this is the WorkOS
      issuer (`https://api.workos.com`), the API translates WorkOS permission
      slugs into canonical Unkey permissions; any other issuer is verified
      generically and its permission claims pass through unchanged.
    </ResponseField>
    ...
    <ResponseField name="auth.jwks_url" type="string">
      JWKS endpoint for `type = "jwt"`. The API fetches the JSON Web Key Set
      on first use and verifies incoming JWTs against usable RSA signing keys
      from the response. When a token fails verification against every cached
      key, the key set is refetched (rate limited to once per minute), so
      signing-key rotations are picked up without a restart. Configure either
      `jwks_url` or `secrets`, never both.
    </ResponseField>
  </Expandable>
</ResponseField>
...
configured with the WorkOS issuer and JWKS URL. WorkOS access tokens include the
organization as `org_id`, and the API reads Unkey RBAC permissions from the
token's `permissions` claim. The WorkOS JWT template also sets `aud` to
