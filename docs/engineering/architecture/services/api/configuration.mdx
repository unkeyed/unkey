...
<ResponseField name="auth" type="object[]">
  ...
  <Expandable title="Fields">
    ...
    <ResponseField name="auth.issuer" type="string">
      Expected JWT `iss` claim for `type = "jwt"`.
    </ResponseField>
    <ResponseField name="auth.audience" type="string">
      Optional expected JWT `aud` claim for `type = "jwt"`. Omit this field
      for providers that do not emit an audience claim.
    </ResponseField>
    ...
    <ResponseField name="auth.jwks_url" type="string">
      JWKS endpoint for `type = "jwt"`. The API fetches the JSON Web Key
      Set at startup and verifies incoming JWTs against usable RSA signing keys
      from the response. Configure either `jwks_url` or `secrets`, never both.
    </ResponseField>
  </Expandable>
</ResponseField>
