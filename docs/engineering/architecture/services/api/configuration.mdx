...
<ResponseField name="jwt_secrets" type="string[]">
  Ordered JWT verification secrets for short-lived API bearer authentication.
  Leave unset to disable JWT authentication. During rotation, put the active
  signing secret first and keep retired secrets later in the list until every
  token signed with those secrets has expired.
</ResponseField>
...
The dashboard proxy needs a signing secret and an API base URL. The API must
include the same signing secret in `jwt_secrets` before it accepts dashboard
proxy requests.
...
<ResponseField name="UNKEY_JWT_SECRET" type="env">
  Dashboard proxy signing secret. Add the same value to the API
  `jwt_secrets` list so the API can verify dashboard-minted JWTs.
</ResponseField>
