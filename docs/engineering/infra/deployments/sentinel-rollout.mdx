...
The Restate ingress URL and bearer token are already injected into the `restate-worker` pod as `UNKEY_RESTATE_INGRESS_URL` and `UNKEY_RESTATE_INGRESS_API_KEY` (see [worker configuration](/architecture/services/control-plane/worker/configuration#restate-configuration)). Pull them straight from the pod so you don't have to touch AWS Secrets Manager:

```bash
POD=$(kubectl -n unkey get pod -l app.kubernetes.io/component=restate-worker -o jsonpath='{.items[0].metadata.name}')
export RESTATE_URL=$(kubectl -n unkey exec "$POD" -- printenv UNKEY_RESTATE_INGRESS_URL)
export RESTATE_TOKEN=$(kubectl -n unkey exec "$POD" -- printenv UNKEY_RESTATE_INGRESS_API_KEY)
```
