...
-- Server metrics (resource usage: CPU, memory, merge/mutation pool, cache, uptime)
GRANT SELECT ON system.metrics TO grafana_readonly;

-- Server error counters (Keeper, S3, network, ...)
GRANT SELECT ON system.errors TO grafana_readonly;

-- Currently running queries
GRANT SELECT ON system.processes TO grafana_readonly;

-- Query log (performance dashboards)
GRANT SELECT(
    event_time, query_start_time, query_duration_ms, type,
    user, initial_user, query_kind, query_id, query, normalized_query_hash,
