...
When a GitHub pull request closes, the
...
job closes its open deploy request and deletes its `pr-<number>` branch. The
database change must already be in PlanetScale `main` before the pull request
merges, so the branch and its deploy requests are never needed after the pull
request closes.
