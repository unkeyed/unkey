...
Assumes familiarity with Kubernetes, AWS, and the existing repo layout. For architecture details see the main [README](../../README.md).
...
- **CLI tools** installed: `awscli`, `eksctl`, `kubectl`, `helm`, `argocd` (see [README Prerequisites](../../README.md#prerequisites))
...
- **CIDR allocation** — confirm the target region has an entry in [`networks`](networks). The generator script will refuse to run if the CIDR is missing.
...
- VPC CIDR matches the [networks](networks) assignment
...
The region isn't in the `CIDR_MAP` in `generate-region-config.sh`. Add it there and in [networks](networks).
