...
Production heimdall ships via the `unkeyed/infra` repo's standard Helm + ArgoCD + promotions flow. Key components:

- **Helm chart**: `eks-cluster/helm-chart/heimdall/` in the infra repo. Ports the dev manifest (hostNetwork/hostPID, six host mounts, CAP_SYS_ADMIN+BPF+NET_ADMIN, BPF map pin dir `/sys/fs/bpf/heimdall/v1`). Values pull `UNKEY_CLICKHOUSE_URL` from AWS Secrets Manager via an ExternalSecret.
- **ArgoCD ApplicationSet**: `eks-cluster/argocd/heimdall.yaml`, sync-wave 2 (needs external-secrets from wave 1). Matrix generator: clusters × per-environment promotion files.
- **Promotions**: `eks-cluster/promotions/{staging,production001}/heimdall.yaml`. Revision is pinned per environment. Use `./scripts/promote <env> heimdall <sha>` to advance.
- **Prometheus alerts**: `eks-cluster/helm-chart/observability/templates/prometheus-alerts-heimdall.yaml` ships 7 warning-level alerts (DaemonSet health, no-checkpoints-while-alive, ticks skipped, BPF map near capacity, CRI reconnect storm, attach failures, informer cache unsynced). Promote to critical once thresholds are validated.
- **ClickHouse user**: `heimdall` user granted `metering_writer_role` — see [users/heimdall](/infra/clickhouse/users/heimdall) and [roles/metering-writer-role](/infra/clickhouse/roles/metering-writer-role). SQL runs once per ClickHouse Cloud cluster.
- **Secret source**: AWS Secrets Manager `unkey/heimdall` with key `UNKEY_CLICKHOUSE_URL`.

Dashboard panels to build in Grafana: checkpoints written per node, collection duration p50/p95/p99, krane pods per node, BPF map depth vs 16384 cap, lifecycle drops by reason, CRI events by kind, network attach failures by reason.
