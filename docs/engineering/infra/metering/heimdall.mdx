...
`dev/heimdall-sandbox-recreation-test.sh` exercises this path end to end against the local `mise run dev` cluster: it starts a krane-labelled gVisor pod, OOM-kills it so kubelet recreates the sandbox, and fails unless the checkpoints written after the new sandbox are attached and their egress counter grows. Run it after touching the attach or sandbox-resolution code.
