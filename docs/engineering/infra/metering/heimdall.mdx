...
- The Go agent ships as the `heimdall` binary in the `ghcr.io/unkeyed/heimdall:<tag>` image.
...
- **There is no separate "deploy the eBPF program" step.** Updating heimdall means bumping the heimdall image tag in the Helm chart. ArgoCD rolls the DaemonSet, and the new pod loads the embedded `.o` on first attach.
...
- **tc-eBPF on the host-side veth** (the second thing we built, and what we shipped first to prod) is silently broken on EKS with Cilium in BPF host-routing mode. Cilium's `bpf_redirect_peer` (see [bpf/lib/local_delivery.h](https://github.com/cilium/cilium/blob/main/bpf/lib/local_delivery.h) and [bpf_lxc.c](https://github.com/cilium/cilium/blob/main/bpf/bpf_lxc.c)) punts packets from `cil_from_netdev` on the host NIC straight into the target pod's netns, so the per-pod host-side veth (`lxc<hash>` / `eni<hash>`) sees ~zero data packets. Production incident: host-side veth RX/TX counters sat at a few hundred bytes while the pod was actively moving MBs. Frontline nodes worked because Frontline runs kube-proxy without host-routing; untrusted workload nodes did not. Unusable at scale.
...
A pod UID can have more than one such record. Under gVisor a memory-limit kill takes down the whole runsc sandbox; kubelet stops it and creates a new sandbox, with a new netns and usually a new pod IP, under the same pod UID. containerd keeps the stopped sandbox's container record and OCI spec until kubelet GC runs `RemovePodSandbox` about a minute later, but `StopPodSandbox` already removed its netns file. Heimdall therefore only accepts a record whose netns path still exists, and treats "every record's netns is gone" as "the new sandbox is not up yet" (debug log, `netns_gone` metric, retry next tick). Each attach record also remembers its netns path; when that file disappears while the pod is still in the informer, `Attach` drops the dead links and map entry and re-attaches to the new sandbox (`network_reattaches_total`). This does not depend on the CRI exit event or an informer transition arriving, both of which can be dropped. Before this check existed, one recreation could mark the pod as permanently terminated and every later checkpoint carried `network_attached=false`, which the billing query drops.

`dev/heimdall-sandbox-recreation-test.sh` exercises this path end to end against the local `mise run dev` cluster: it starts a krane-labelled gVisor pod, stops its sandbox with `crictl stopp` so kubelet recreates it under the same pod UID, and fails unless the checkpoints written after the new sandbox are attached and their egress counter grows. Run it after touching the attach or sandbox-resolution code.
...
Host-network pods (heimdall itself, kube-proxy) share the host net namespace and have no pod-side eth0, so heimdall skips them via `pod.Spec.HostNetwork`. Completed/Failed pods are also skipped because their sandbox containers have been GC'd from containerd.
...
- `bpf/network.bpf.c`. The eBPF program, compiled by `bpf2go` into Go bindings and an embedded `.o`. Run `mise run generate-bpf` after editing.
