...
The short version: snapshots of cumulative kernel counters are idempotent under duplicate writes; rate or delta rows aren't. For the full walkthrough of the pipeline (counter vs gauge, the five stages from kernel to chart, why duplicates are invariant) see [metrics-architecture](/infra/metering/metrics-architecture). For the citation-heavy defence against "just write deltas" proposals specifically, see [why-counters-not-deltas](/infra/metering/why-counters-not-deltas).
