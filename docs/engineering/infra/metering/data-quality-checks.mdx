...
The number of instances seen per node should match `kubectl get pods --field-selector spec.nodeName=<node>` restricted to billable pods (`managed-by=krane`, `component=deployment`). A gap means heimdall is either not running on that node, or can't reach that pod's cgroup.
...
- [metrics-architecture](/infra/metering/metrics-architecture). The pipeline design these checks are asserting properties of.
- [heimdall](/infra/metering/heimdall). The collector these checks verify.
