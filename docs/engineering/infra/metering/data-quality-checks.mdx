...
Run the same query substituting `network_egress_public_bytes`, `_private_bytes`, `network_ingress_public_bytes`, `_private_bytes`. All four must be monotone per `container_uid`.
