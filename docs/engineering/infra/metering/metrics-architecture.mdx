...
| Gauge (memory, disk) | the sample average over the window (≈ time-weighted only when checkpoint spacing is even — lifecycle events and missed ticks make the spacing irregular) | `sum(value) / sample_count` |
...
For network, we can't read a file because the kernel doesn't expose per-pod byte counts to userspace. Instead we run a tiny program inside the kernel that counts bytes as packets flow past. It writes into an in-memory table we can read. See [ebpf-primer](/infra/metering/ebpf-primer) if you want to understand how that works.
...
The raw table keeps rows for 95 days (long enough to backfill from outage-shaped agent gaps and to support debug queries against the source data). Step 4 rolls them up into longer-retention tables for the dashboards and billing windows that don't need raw granularity.
...
- `instance_resources_per_15s_v1` groups raw rows into 15-second buckets (TTL 7 days).
- `instance_resources_per_minute_v1` groups them into 1-minute buckets (TTL 30 days).
- `instance_resources_per_hour_v1` groups them into 1-hour buckets (TTL 90 days).
- `instance_resources_per_day_v1` groups them into 1-day buckets (TTL 365 days). Drives month-over-month dashboards.
- `instance_resources_per_month_v1` groups them into calendar-month buckets (TTL 5 years). Drives billing-window + long-range trend charts.
...
With the snapshot model, `max - min` ignores duplicates automatically. Writing the same row twice gives the same answer as writing it once. This is the biggest reason and the full argument lives in [why-counters-not-deltas](/infra/metering/why-counters-not-deltas).
...
Why not drop CRI and use only the informer: the informer is eventually-consistent and polls at watch-resync cadence — it sees state transitions seconds after they happen, so the final cgroup read is taken well past the moment the container died and we miss the CPU used between the last 5s tick and exit. CRI fires on the actual `/tasks/exit` syscall path inside containerd, giving us millisecond-precision reads against live cgroup files. The informer is the drop-resilience net, not a replacement.
...
Three reasons we store both, on every row:
...
3. **Consumers can pick either column.** CPU/memory aggregates today tend to lean on allocated (reservation-based); network/disk lean on used (counter-based). The schema is agnostic either way.
...
<AccordionGroup>
  <Accordion title="Why don't we just write one row per 15 seconds saying 'container X used Y CPU'?">
    Because a duplicate row would inflate any sum built on top. See the "Why not just write..." section above, or the full defence in [why-counters-not-deltas](/infra/metering/why-counters-not-deltas).
  </Accordion>
...
- `svc/heimdall/internal/network/`. The BPF byte counter side. See [ebpf-primer](/infra/metering/ebpf-primer).
...
- [why-counters-not-deltas](/infra/metering/why-counters-not-deltas). The narrow defence of the `max - min` design with citations from Prometheus, OpenTelemetry, GCP, RRDtool, the Linux kernel.
- [heimdall](/infra/metering/heimdall). The agent's internal design: CRI watcher, informer backup, network-specific details.
- [ebpf-primer](/infra/metering/ebpf-primer). Background on eBPF for anyone touching the network metering code.
