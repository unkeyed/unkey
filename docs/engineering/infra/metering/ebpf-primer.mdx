...
Our program does the bare minimum: skip the Ethernet header, read the IP header, classify destination IP as RFC1918/public, `__sync_fetch_and_add` into one of four counter slots in a shared map keyed by the pod's netns cookie (`bpf_get_netns_cookie`, a stable per-netns 64-bit identifier). It returns `TC_ACT_UNSPEC` (non-terminating; see [heimdall.mdx](/infra/metering/heimdall) for why this matters with Cilium in the chain), never modifies the packet, and runs alongside Cilium's own programs without interfering. Userspace (the Go side) periodically reads the map and writes the raw counter values to ClickHouse. Same pattern as CPU/memory; the only difference is that the counters live in a BPF map instead of a sysfs file.
...
- **Safe under the customer load path.** Because of the verifier, our code is structurally incapable of breaking customer pod networking. [See the Safety invariants section of heimdall.mdx](/infra/metering/heimdall#safety-invariants).
...
- `svc/heimdall/internal/network/bpf/network.bpf.c`. The only eBPF C program we ship. Around 180 lines, compiled by `bpf2go` into an embedded `.o` plus Go bindings. Run `mise run generate-bpf` after editing.
...
See [heimdall.mdx](/infra/metering/heimdall) for how the whole thing fits together.
