...
```
To get your depot token, sign in to [depot.dev](https://depot.dev), then navigate to organization \> settings, and then `Add API token`. Name it something like `<your-name>-local`.
...
The product ID lists come pre-filled in `.env.example` (Flo's sandbox catalog),
so you only add two values:

- `STRIPE_SECRET_KEY` - a test-mode key (`sk_test_...`) from Flo's sandbox.
