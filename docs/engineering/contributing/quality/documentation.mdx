...
Default to no new code comments. Make intent clear through names and structure. Add a comment only when explicitly requested or when essential context cannot be expressed in the code. Keep it concise: explain the constraints or tradeoffs a future engineer needs to understand why the solution was written this way. Keep existing comments accurate when changing the code they describe.

This rule applies to symbol, package, SQL, and test documentation. The sections below explain how to write comments when needed, rather than requiring a comment on every declaration. Preserve directives required by tooling, such as SQL query names and Go build tags.
...
**Completeness**
- [ ] Comments add context the code cannot clearly convey or were explicitly requested
- [ ] Package has a `doc.go` when its purpose or constraints need explanation
...
- [ ] SQL comments explain non-obvious query guarantees and tradeoffs
...
Document public API contracts when callers need context that names, types, and code cannot clearly convey, such as ownership, concurrency constraints, or surprising failure behavior.

Apply the same rule to unexported functions and methods. Being exported or unexported does not by itself justify a comment.

Match depth to the context a reader needs. A straightforward getter needs no comment. A distributed algorithm may need paragraphs explaining its constraints and tradeoffs.
...
Use a `doc.go` file when a package's purpose, constraints, or design tradeoffs need context beyond its code. Include only the concepts, usage examples, and cross-references a reader needs to understand those decisions.
...
Comment on test helpers and scenarios only when names and code cannot explain the guarantee being protected or the reason for unusual setup.
