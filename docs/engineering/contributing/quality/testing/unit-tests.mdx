...
Add a short docstring to every test, even when its name is descriptive. Explain the guarantee and why it matters, not the steps the test performs. Prefer a concrete input and expected result from the test, as described in the [testing standard](/contributing/quality/testing/index).
...
// For example, a key for workspace A receives 404 when it reads workspace B's API.
