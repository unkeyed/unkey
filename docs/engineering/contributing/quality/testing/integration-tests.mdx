...
them across Go test processes, so tests only start the services they actually
...
unless they need isolation. Keep expensive setup explicit so readers can see
why the test belongs in the integration suite.
