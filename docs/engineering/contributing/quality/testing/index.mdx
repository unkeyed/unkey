...
Give every test a short docstring or comment that explains its guarantee and why it matters. A descriptive test name does not replace this explanation. Prefer a concrete example from the test: given this input or sequence, expect this result. For table-driven tests, explain the shared guarantee once and use case names to describe individual variations.
...
Describe the behavior under test, not change history or mutation-testing scores. Do not document obvious mechanics. A comment that says "creates a workspace" above `createWorkspace(t)` adds noise. A comment that explains why cross-workspace access must return 404 instead of 403 documents a guarantee.
