---
title: Express rate limiting quickstart
description: Add serverless rate limiting to your Express API routes with the @unkey/ratelimit SDK. Protect endpoints without Redis or infrastructure.
---
...
<AccordionGroup>
  ...
  <Accordion title="TypeScript version?">
    Install types and use imports: ```bash npm install -D typescript
    @types/express @types/node ``` ```ts import express from "express"; import{" "}
    {Ratelimit} from "@unkey/ratelimit"; ```
  </Accordion>
</AccordionGroup>
