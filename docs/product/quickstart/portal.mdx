...
The portal currently exposes only the keys page, so these scopes gate what the end user can do there rather than which pages they see. Rerolling is reached from that page, so `keys:reroll` requires `keys:read` in the same session and is rejected on its own.
