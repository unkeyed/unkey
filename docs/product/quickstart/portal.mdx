...
description: "Give your end users a white-labeled self-service portal for API key management, with a Stripe-style session auth flow."
hidden: true
noindex: true
---
...
The Customer Portal is a white-labeled web app you can offer to your end users. They get self-service key management, without you building any UI.
...
Disabling a portal (`"enabled": false`), deleting it, or re-pointing it at a different app or keyspace with `keyspaceId` or `appId` revokes its live sessions. Those end users lose access on their next request rather than at token expiry. Disabling also stops new sessions from being minted, and re-enabling the portal does not bring the old sessions back.
...
body := map[string]any{
    "portal":     "my-portal",
    "externalId": "user_123",
    "scopes":     []string{"keys:read", "keys:reroll"},
}
```
...
3. Redirect to the keys page

## Scopes

Scopes come from a fixed vocabulary. Every scope is bound to the end user in the session: `keys:*` applies only to keys that user owns within the portal's keyspace. An end user can never see another identity's keys.

| Scope | Grants |
|-------|--------|
| `keys:read` | List their own keys |
| `keys:reroll` | Roll a key they own |
| `analytics:read` | See their own key usage over time |

The portal exposes one page, the keys page, so these scopes gate what the end user can do there rather than which pages they see. Rerolling and usage are both reached from that page, so `keys:reroll` and `analytics:read` each require `keys:read` in the same session and are rejected on their own.
...
| Setting | Default |
|---------|---------|
| Primary color | None; the header falls back to a dark neutral |
| Logo | None; the header shows the portal's display name instead |

Both are optional. `primaryColor` must be a six-digit hex value like `#6366f1`, and `logoUrl` must be an absolute `https://` URL.
...
| `analytics:read` | `api.*.read_analytics`, or read on the keyspace's log resource |

Requesting a scope you do not hold returns 403 for the whole request rather than a session with fewer capabilities, so a missing grant shows up immediately instead of as an end user who cannot do what you expected.
...
| `keys:reroll` requested without `keys:read` | 400 | `The "keys:reroll" scope requires "keys:read" in the same session.` |
| `analytics:read` requested without `keys:read` | 400 | `The "analytics:read" scope requires "keys:read" in the same session.` |
