---
title: Authenticate API keys in Express
description: "Add API key authentication to your Express API using Unkey. Protect routes by verifying keys on every incoming request with minimal setup."
---
...
<Accordion title="Getting 500 errors?">
  ...
  <Accordion title="TypeScript version?">
    ...
    ```bash npm install -D typescript @types/express @types/node ``` ```ts
    import express from "express"; import {verifyKey} from "@unkey/api"; ```
  </Accordion>
</AccordionGroup>
