...
| Stream | Filters |
| --- | --- |
| Audit logs | **Event types** selects all events or specific [audit event names](/audit-log/types) grouped by category. |
...
For gateway requests and runtime logs, expand a project in **Sources** to view
...
**Select all**, or checking every current environment, removes the source
restriction and includes future resources. **Clear all** requires you to select
at least one source before saving.
