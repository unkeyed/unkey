...
filters. **Unspecified** matches an empty verification outcome. Tags remain in
the payload for filtering at your destination.
...
same `event` object with `stream: "key_verifications"` and `_time` in place of
the HTTP `timestamp`.
