...
For rate limits, use the pair of `event.request_id` and `event.check_index`.
...
| Rate limits | **Namespaces** selects workspace namespaces. **Results** selects **Passed** or **Blocked**. **Identifiers** matches exact customer identifiers, including spaces. |
...
`ratelimit.multiLimit`, not audit events for configuration changes. Each check
in a multi-limit request has a zero-based `check_index`. A single-limit check
has index zero. HTTP JSON sends an array such as:
