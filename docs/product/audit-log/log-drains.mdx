---
title: Log drains
description: "Send audit logs and key verifications to HTTPS endpoints or Axiom, filter events, and diagnose failed deliveries."
---

A log drain sends workspace audit logs or key verifications to a destination
...
one stream: audit logs or key verifications.
...
for key verifications as the deduplication key within each stream.
...
90 days. See [audit log retention](/audit-log/introduction#retention) for audit
logs.
...
6. Select **Audit logs** or **Key verifications** in **Stream**.
