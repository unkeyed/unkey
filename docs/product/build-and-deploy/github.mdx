...
### Approve a fork deployment

Fork deployments appear in your project's deployment list with an `awaiting_approval` status. Click the row to open the approval dialog, which shows:

- The commit SHA being deployed (links to the commit on GitHub)
- The source branch (links to the branch on the contributor's fork)
- The target environment (for example, `preview`)
- A link to review the pull request

Review the changes on GitHub before approving. Once you click **Approve Deployment**, the build and deploy pipeline starts. To reject the deployment, close the dialog without approving — no build runs and no resources are provisioned.
