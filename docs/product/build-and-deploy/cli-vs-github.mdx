...
| **Build concurrency quota**      | Counts against workspace [`builds_concurrent_max`](/build-and-deploy/deployments#build-concurrency) | Waits in the same queue, then holds a slot only for the instant it takes to resolve the image |
