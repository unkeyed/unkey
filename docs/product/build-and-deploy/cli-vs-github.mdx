...
| **Build concurrency quota**      | Counts against workspace [`builds_concurrent_max`](/build-and-deploy/deployments#build-concurrency) | Does not count, the image is already built so it never waits for a build slot |
