...
| **Build concurrency quota**      | Counts against workspace [`builds_concurrent_max`](/build-and-deploy/deployments#build-concurrency) | Holds a slot only for the instant it takes to resolve the image |
