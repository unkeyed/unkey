...
We rotate encryption keys regularly and run Vault on separate servers from the main database. An attacker would need access to all systems simultaneously.
...
To opt in to recovery, send us an email at [support@unkey.com](mailto:support@unkey.com?subject=Recovery%20Opt%20In).
...
<Note>
  This doesn't apply retroactively. Existing keys were never stored and cannot be recovered. Only keys created after opting in to recovery can be recovered.
</Note>
...
Both the [getKey](/api-reference/v2/keys/get-api-key) and [listKeys](/api-reference/v2/apis/list-api-keys) endpoints accept a `decrypt` query parameter. If you set this to `true`, the key will be decrypted and returned in the response as `plaintext`.
