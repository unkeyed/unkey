...
Each domain must be unique within a workspace. You cannot add the same domain to multiple projects or environments in the same workspace.

Different workspaces can use the same domain independently. This is useful when you maintain separate workspaces for staging and production, or when different teams manage their own workspaces.
