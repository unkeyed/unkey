...
Each domain must be unique within a workspace. You cannot add the same domain to a single workspace more than once. However, different workspaces can use the same domain independently, each with its own verification and certificate.
