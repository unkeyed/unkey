...
## Where custom domains appear

A custom domain always routes traffic to the current deployment in its environment. In the dashboard, custom domains are only listed on the deployment detail view of the current deployment for that environment. When viewing a previous deployment, only the platform domains (`*.unkey.app`) that were assigned to that specific deployment are shown.

This reflects how requests are routed: promoting a new deployment shifts the custom domain to that deployment, so previous deployments no longer serve traffic on the custom hostname.
