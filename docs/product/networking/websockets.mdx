---
title: WebSockets
description: "Deploy WebSocket servers on Unkey with long-lived connections, no request timeouts, and gateway auth and rate limits applied to the upgrade."
---
...
Routing rules apply to the upgrade request the same way they apply to any HTTP request: the `Host` header determines which deployment receives the connection, and gateway policies run before the upgrade is forwarded. A request that fails [authentication](/platform/gateway/authentication) or [rate limiting](/platform/gateway/policies/rate-limiting) is rejected before it ever becomes a WebSocket.
