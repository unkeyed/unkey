...
| `error_code`                                              | Unkey error code when the gateway rejected the request itself. Empty when an instance served the response |
...
Only requests recorded in gateway request logs are eligible. Requests the
gateway rejected before they reached an instance, such as a key, rate limit,
or firewall policy denial, are exported and carry `error_code`. Requests
excluded from logging are not exported.
