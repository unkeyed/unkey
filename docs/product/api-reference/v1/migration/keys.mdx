...
Your root key must have the `api.<API_ID>.verify_key` permission for v2 key verification.

[Learn more](#understanding-v2-root-key-permissions-for-key-verification)
</Danger>
...
When creating a root key for key verification, you need to grant it the appropriate `verify_key` permission for your API:

<Tabs>
  <Tab title="Specific API Permission (Recommended)">
  ...
  <Tab title="Wildcard Permission (All APIs)">
    ...
    This root key can verify keys from any API in your workspace. Only use this if you specifically need to authenticate users across all APIs.
  </Tab>
</Tabs>
