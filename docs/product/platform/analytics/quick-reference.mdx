...
description: "Quick lookup for common Unkey Analytics SQL patterns, available tables, aggregate functions, and filtering syntax for verification data."
---
...
```sql
-- Usage per keyspace (last 30 days)
...
-- Keyspace success rate comparison (last 7 days)
SELECT key_space_id,
...
<Tip>
  See [Query Examples - WITH
  FILL](/platform/analytics/query-examples#filling-gaps-in-time-series-with-fill)
  for hourly, daily, and monthly examples with outcome breakdowns.
</Tip>
...
### Unkey Deploy filters

Use `source` to select Deploy traffic and `app_id` to select one app.

```sql
-- Verifications performed by an Unkey Deploy API key policy
WHERE source = 'gateway'

-- Verifications performed for one app
WHERE source = 'gateway' AND app_id = 'app_1234'
```
