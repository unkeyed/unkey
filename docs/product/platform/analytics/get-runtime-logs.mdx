...
Use a root key with the `project.*.read_logs` permission. This permission gives
access to the runtime logs of all projects in the workspace.

Runtime logs contain the output of your own code. Thus `read_logs` is a separate
permission. A root key with `project.*.read_analytics` reads gateway request
data, but it does not read runtime logs.
...
| 403 | The root key does not have the `project.*.read_logs` permission. |
