...
| `instance_id` | String | Instance that processed the request. Empty when the gateway rejected the request before it reached one |
...
| `error_code` | String | Unkey error code when the gateway rejected the request itself, for example a key, rate limit, or firewall policy. Empty when an instance served the response |
