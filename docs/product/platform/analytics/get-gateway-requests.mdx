...
request. If you do not add a time filter, Unkey limits the results to your
workspace retention range. A query can request a range longer than the
retention of a table, but it then gets only the rows that stay in that table.
For example, a 30 day query on the raw table can contain partial data, because
that table keeps 7 days. Select the hour table for a full 30 day range.
