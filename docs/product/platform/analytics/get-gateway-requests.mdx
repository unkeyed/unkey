...
A query must name the columns that it needs. `SELECT *` fails on the raw table,
because the table also contains columns of the Unkey infrastructure that a
...
The function that you use depends on the table.

The raw table keeps `total_latency`, `instance_latency`, and
`frontline_latency` as plain numbers. Use `quantile` on these columns.
...
The aggregated tables do not contain these columns. They keep the latency data
in ClickHouse aggregate states, in one column for each percentile. A query that
selects one of these columns directly fails. Use `quantileTDigestMerge` to read
a percentile from them.
...
curl --request POST \
  ...
  --header "Authorization: Bearer $UNKEY_ROOT_KEY" \
  --header "Content-Type: application/json" \
  --data '{"query":"SELECT quantileTDigestMerge(0.5)(latency_p50) AS p50, quantileTDigestMerge(0.95)(latency_p95) AS p95, quantileTDigestMerge(0.99)(latency_p99) AS p99 FROM gateway_requests_per_hour_v1 WHERE app_id = '\''app_1234'\'' AND time >= now() - INTERVAL 24 HOUR"}'
```
...
curl --request POST \
  ...
  --header "Authorization: Bearer $UNKEY_ROOT_KEY" \
  --header "Content-Type: application/json" \
  --data '{"query":"SELECT app_id, response_status, sum(count) AS total FROM gateway_requests_per_hour_v1 WHERE time >= now() - INTERVAL 7 DAY GROUP BY app_id, response_status ORDER BY app_id, total DESC"}'
```
