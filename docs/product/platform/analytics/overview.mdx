---
title: Overview
description: "Query your API key verification data with SQL using Unkey Analytics. Run custom queries to analyze usage patterns, billing, and trends."
---

<Warning>
  **Analytics is currently in private beta and available by request only.**

See [Getting Started](/platform/analytics/getting-started) for access instructions.
...
| `key_space_id`  | String        | Your KeySpace identifier (e.g., `ks_1234`). **Automatically filtered** if your root key is scoped to a single keyspace.                   |
...
<CardGroup cols={3}>
  <Card
    title="Billing Teams"
    icon="credit-card"
    href="/platform/analytics/query-examples#billing-usage-based-pricing"
  >
  ...
  <Card
    title="Monitoring "
    icon="chart-line"
    href="/platform/analytics/query-examples#usage-analytics"
  >
  ...
  <Card
    title="Product Teams"
    icon="users"
    href="/platform/analytics/query-examples#usage-by-user"
  >
