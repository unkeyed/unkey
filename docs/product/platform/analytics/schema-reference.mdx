...
| `source`        | String        | Verification origin. `api` means the Verify API, and `gateway` means an Unkey Deploy API key policy                                                |
| `app_id`        | String        | Unkey Deploy app that verified the key. Empty when `source` is not `gateway`                                                                        |
...
| `app_id`        | String        | Unkey Deploy app that verified the key. Empty for non-Deploy verifications                                             |
...
## Filter Unkey Deploy verifications by app

Deploy verifications use `source = 'gateway'` and include the app ID that ran
