...
| `source`        | String        | Verification origin. `api` means the Verify API, and `gateway` means a gateway API key policy                                                       |
| `app_id`        | String        | App whose gateway policy verified the key. Empty when `source` is not `gateway`                                                                     |
...
| `app_id`        | String        | App whose gateway policy verified the key. Empty for non-gateway verifications                                        |
...
Verifications from the gateway use `source = 'gateway'` and include the app ID that ran
