...
restrictions](/platform/analytics/query-restrictions). Each `SELECT` may use one
rate limit source. You can combine `SELECT` branches with `UNION`, but
relational `JOIN`s remain unsupported. The query must contain a positive
literal filter in one of these forms. You can optionally qualify `namespace_id`
with a table alias:
