---
title: Overview
description: "Protect any API endpoint from abuse with Unkey's globally distributed rate limiting. No infrastructure required, just a single API call."
---

Rate limiting controls how many requests a user, IP, or any identifier can make in a given time window. Unkey provides globally distributed rate limiting that runs across the Unkey network without infrastructure for you to manage.
...
<AccordionGroup>
  <Accordion title="No infrastructure to manage" icon="server">
    No clusters, hosted counter stores, or connection strings. Install the SDK
    and call the API.
  </Accordion>
...
<Accordion title="Globally distributed" icon="globe">
  ...
  rate limits are checked close to your users, not in a single region. Regions
  publish active counters and import counts from other regions, so identifiers
  approaching their limit converge globally within seconds. See [how rate
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/platform/ratelimiting/how-it-works"
  >
