---
title: API key
description: "Reference for Principal fields produced by gateway API key authentication including key ID, owner, permissions, and custom metadata."
---

When the [Principal's](/platform/gateway/principal/overview) `type` is `"API_KEY"`, the `source.key` object contains the full details of the verified API key. This page documents every field in the API key source.
...
<ResponseField name="source.key.keySpaceId" type="string" required>
  The ID of the keyspace the key belongs to. Useful when your gateway is configured with multiple keyspaces and your application needs to distinguish which keyspace matched.
</ResponseField>
...
<ResponseField name="source.key.expiresAt" type="integer">
  Unix timestamp in **milliseconds** when the key expires. Absent when the key has no expiry. The gateway already rejects expired keys, so if this field is present the key is still valid. Your application can use it to warn users about upcoming expiration.
</ResponseField>

<ResponseField name="source.key.credits" type="integer">
  The number of credits remaining after this request's cost was deducted. Absent when the key has unlimited credits. A value of `0` is included (it means the key just spent its last credit), so your application can distinguish "no credits left" from "unlimited".
</ResponseField>
...
<ResponseField name="source.key.permissions" type="string[]">
  ...
  These are the raw permissions from Unkey. If the gateway enforced a [permission query](/platform/gateway/policies/api-key) and the key lacked the required permissions, the request was already rejected before reaching your app. The permissions here let your application make additional fine-grained authorization decisions beyond what the gateway enforces.
</ResponseField>
