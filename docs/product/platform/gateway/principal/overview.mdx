...
The Principal is a JSON object the gateway sets on the `X-Unkey-Principal` header after a request passes [authentication](/platform/gateway/authentication). It contains a stable set of top-level fields that are the same across all authentication methods, plus a `source` object with method-specific detail.
...
<ResponseField name="source" type="object" required>
  ...
  - [API key source](/platform/gateway/principal/sources/api-key)
</ResponseField>
