...
5. Optionally configure [match conditions](/platform/gateway/policies/overview#match-expressions), a custom [key location](#key-location), a [permission query](#permission-query), or a [credit cost](#credit-cost).
...
4. **Credits.** If remaining credits are configured, the key must have enough to cover the request's [credit cost](#credit-cost). Verification deducts that cost (one credit by default). A cost of `0` skips this check.
...
- Set the cost to `0` to verify the key without spending any credits. This is useful for read-only endpoints, or when you only want the gateway to confirm the key is valid before proxying to a backend that meters usage itself.
- Set the cost higher than `1` to charge more for expensive endpoints (for example, `3` credits per request).
