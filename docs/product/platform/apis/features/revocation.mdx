...
description: "Revoke API keys instantly by deleting or disabling them in Unkey. Revoked keys fail verification immediately with no propagation delay."
---
...
try {
  ...
  const { meta, data } = await unkey.apis.listKeys({
    apiId: "api_...",
    externalId: "user_123",
  });
  ...
  for (const key of data.keys) {
    await unkey.keys.deleteKey({ keyId: key.id });
  }
} catch (err) {
  console.error(err);
  return Response.json({ error: "Internal error" }, { status: 500 });
...
<CardGroup cols={2}>
  <Card
    title="Key rerolling"
    icon="arrows-rotate"
    href="/platform/apis/features/rerolling-key"
  >
