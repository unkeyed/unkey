---
title: Keys
description: "API keys are credentials your users include in requests. Unkey handles creation, verification, rotation, revocation, and metadata."
---
...
| Option        | Description                                                                                                          |
| ------------- | -------------------------------------------------------------------------------------------------------------------- |
| `prefix`      | Override the API's default prefix for this key. Falls back to the keyspace `default_prefix` when omitted.            |
| `byteLength`  | Number of random bytes used to generate the key. Falls back to the keyspace `default_bytes`, then to `16` when both are unset. |
...
<Tip>
  Set `default_prefix` and `default_bytes` once on the API in the dashboard (or
  via the keyspace) and you can call `keys.createKey` with just an `apiId` —
  every key inherits the configured prefix and length unless you override it
  per request.
</Tip>
