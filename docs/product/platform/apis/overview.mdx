...
| Default prefix       | Prefix prepended to every key created in this API (for example, `sk_live`). Used when a `keys.createKey` request omits `prefix`.         |
| Default bytes        | Number of random bytes used to generate new keys (default: 16). Used when a `keys.createKey` request omits `byteLength`.                 |
...
Per-request values always win over keyspace defaults. Setting `default_prefix` and `default_bytes` lets you keep `keys.createKey` calls minimal — pass just `apiId` and Unkey applies the keyspace settings — without giving up the ability to override either field on a single key.
