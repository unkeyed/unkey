...
Every API has a keyspace (internally called `key_auth`). The keyspace is the storage and validation boundary for all keys in that API. When the gateway or the verification endpoint receives a key, it resolves the keyspace by key prefix and validates the key against that keyspace's configuration.
...
Keyspace IDs appear in analytics queries as `key_space_id` and in gateway configuration when connecting a keyspace to a deployment.
...
| [Gateway authentication](/platform/gateways/authentication)         | Connect keyspaces to gateway policies  |
