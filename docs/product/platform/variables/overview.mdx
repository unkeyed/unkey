...
Click any variable row to open the inline editor. You can update the key, value, or note, and toggle whether the variable is sensitive. Click **Add Note** to expand the optional description field. Use the action menu on each row to delete a variable.
...
The environment a variable belongs to is fixed once created. To move a value to a different environment, create a new variable in the target environment and delete the old one.
...
### Convert an existing variable to sensitive

You can promote a recoverable variable to sensitive at any time. The reverse is not possible — once a variable is sensitive, the value can never be revealed again.

- **One variable** — open the edit row and turn on the **Sensitive** toggle, then save. The toggle is hidden for variables that are already sensitive.
- **Multiple variables** — select the rows you want to convert using the row checkboxes, then click **Make Sensitive** in the selection bar at the bottom of the page. Confirm the prompt to apply the change. Variables that are already sensitive are skipped.

<Warning>
  Marking a variable as sensitive is permanent. Make sure you have the value stored elsewhere if you might need to read it again later.
</Warning>
