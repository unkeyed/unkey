...
## Project and deployment permissions

These permissions control project and deployment operations. Project-level reads, updates, and deletes can use either `project.*` or a specific project ID such as `project.proj_1234abcd`. Creating projects and listing projects require workspace-wide permissions because no project-specific scope exists for those operations.

### Projects

<ResponseField name="project.*.create_project">
  Create projects in the workspace. Required for `projects.createProject`.
</ResponseField>

<ResponseField name="project.*.read_project">
  List projects in the workspace and read any project. Use `project.{projectId}.read_project` to read one specific project with `projects.getProject`.
</ResponseField>

<ResponseField name="project.*.update_project">
  Update any project's name, slug, or delete protection setting. Use `project.{projectId}.update_project` to update one specific project.
</ResponseField>

<ResponseField name="project.*.delete_project">
  Delete any project in the workspace. Use `project.{projectId}.delete_project` to delete one specific project.
</ResponseField>
...
| Project automation    | `project.*.create_project`, `project.*.read_project`, `project.*.update_project` |
