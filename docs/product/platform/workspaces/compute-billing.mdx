...
Compute bills for measured CPU and memory usage over time, allocated ephemeral
disk capacity over time, egress, and the active keys verified through the
Compute gateway. Your plan fee comes with usage credits worth the same amount,
so the fee already covers your first slice of usage. Anything past those
credits is added to your invoice at the end of the month.

Metered usage is separate from [workspace-wide Compute limits](/limits). Billing
measures CPU and memory usage, plus allocated disk capacity, over time. Resource
quotas reserve the maximum CPU, memory, and disk that all running deployments
in the workspace can allocate at their autoscaling limits.
...
- **Per-meter quantities**: CPU hours, memory GiB-hours, egress GiB, disk
  GiB-hours, and active keys. Workspace disk quotas can also be expressed in
  TiB.
