...
A deployment with two regions and two instances per region creates four instances total. Each instance receives traffic through the [gateway](/platform/gateway/overview) in the same region.
...
The Starter plan supports up to 4 instances per region, the Pro plan supports up to 8, and the Business plan supports up to 16.
...
- CPU: 1/4 vCPU to 16 vCPU, depending on your plan (default 1/4 vCPU, billed on usage)
- Memory: 256 MiB to 32 GiB, depending on your plan (default 256 MiB, billed on usage)
- Storage: None to 10 GiB ephemeral disk (default none, mounted at `/data`, billed on allocation)
...
The Starter plan supports up to 2 vCPU and 2 GiB per instance. The Pro plan supports up to 8 vCPU and 8 GiB. The Business plan supports up to 16 vCPU and 32 GiB.

See [App settings](/platform/apps/settings#cpu) for details on each option.

## Load balancing

The [gateway](/platform/gateway/overview) in each region distributes incoming requests uniformly at random across all running instances. Each instance has an equal probability of handling any given request.
