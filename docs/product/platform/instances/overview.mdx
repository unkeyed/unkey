...
Available replica settings depend on your workspace plan and any approved
workspace-specific limits. See [Compute limits](/limits#plan-limits).
...
Each instance has configurable limits for CPU, memory, and storage. Available
resource settings depend on your workspace plan and any approved
workspace-specific limits. See [Compute limits](/limits#plan-limits).

- CPU and memory billing use measured usage over time.
- Ephemeral disk billing uses allocated capacity over time.

Configure limits in **Settings > Runtime settings**:

- CPU (default 1/4 vCPU)
- Memory (default 256 MiB)
- Ephemeral disk (default none, mounted at `/data`)
