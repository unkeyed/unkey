...
A deployment with two regions and two instances per region creates four instances total. Each instance receives traffic through the [gateway](/platform/gateways/overview) running in the same region.
...
The [gateway](/platform/gateways/overview) in each region distributes incoming requests uniformly at random across all running instances. Each instance has an equal probability of handling any given request.
