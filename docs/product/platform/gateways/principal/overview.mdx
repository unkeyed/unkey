---
title: Principal
description: "The Principal is the verified identity object the gateway produces after authentication and forwards to your application via headers."
---
...
The Principal is a JSON object the gateway sets on the `X-Unkey-Principal` header after a request passes [authentication](/platform/gateways/authentication). It contains a stable set of top-level fields that are the same across all authentication methods, plus a `source` object with method-specific detail.
...
<ResponseField name="source" type="object" required>
  ...
  - [API key source](/platform/gateways/principal/sources/api-key)
</ResponseField>
