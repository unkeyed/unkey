...
The API key authentication policy verifies Unkey API keys before requests reach your app. On success, it produces a [Principal](/platform/gateways/principal/overview) containing the key's identity, metadata, roles, and permissions.

## Configure API key authentication

To enable API key authentication for your deployment:

1. Navigate to your project's **Gateway Policies** page from the sidebar.
2. Click **Add policy**.
3. Select **Key Auth** as the policy type.
4. Choose one or more keyspaces to verify against.
5. Optionally configure [match conditions](/platform/gateways/policies/overview#match-expressions), a custom [key location](#key-location), or a [permission query](#permission-query).
...
5. **Rate limits.** All rate limit configurations attached to the key are evaluated. See [rate limiting](/platform/gateways/policies/rate-limiting) for details on response headers and behavior.
6. **Permissions.** If a permission query is configured, the key must satisfy it.

If all checks pass, the gateway produces a [Principal](/platform/gateways/principal/overview) and forwards the request with the `X-Unkey-Principal` header. See the [API key source](/platform/gateways/principal/sources/api-key) for the full list of fields your app receives, including roles, permissions, and key metadata.
