---
title: Rate limiting
description: "Enforce rate limits on matching routes at the gateway. Configure limits per route pattern with no application code changes."
---
...
The rate limiting policy enforces request limits on any route that matches the policy's [match expressions](/platform/gateways/policies/overview#match-expressions). Requests that exceed a configured limit receive a `429` response, protecting your app from traffic spikes and abuse.

Each rate limit policy specifies a maximum number of requests within a time window (for example, 100 requests per 60 seconds) and an identifier that determines how the gateway groups requests for counting.

Rate limit state is managed by Unkey's distributed rate limiting service, so limits are consistent across multiple gateway replicas.

## Configuration

You can create and manage rate limit policies from the **Gateway Policies** page in your project dashboard. Each policy requires:
...
| Authenticated subject | Limit by the authenticated [Principal's](/platform/gateways/principal/overview) `subject` field. Requires an authentication policy earlier in the list. |
...
```json
{
  ...
  "error": {
    ...
    "status": 429,
    "type": "https://unkey.com/docs/errors/unkey/rate_limited/rate_limited"
  }
}
```
