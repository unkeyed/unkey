---
title: Sentinel
description: A Sentinel is a reverse proxy that sits in front of your deployment, enforcing policies on every request before it reaches your service.
---
...
- [**Authentication**](/platform/sentinel/authentication): Verifies API keys before your code runs, and forwards the authenticated identity to your service via a request header
...
Other policy types include [IP rules](/platform/sentinel/policies/ip-rules), [OpenAPI request validation](/platform/sentinel/policies/openapi-validation), and more. See [Policies](/platform/sentinel/policies/overview) for the full list.
...
See [Authentication](/platform/sentinel/authentication) for details on the Principal header your service receives after successful verification.
