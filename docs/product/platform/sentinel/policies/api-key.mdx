...
description: Verify Unkey API keys with the Sentinel and forward the authenticated identity to your service.
---
...
The API key authentication policy verifies Unkey API keys before requests reach your service. On success, it produces a [Principal](/platform/sentinel/authentication#the-principal) containing the key's identity and metadata.
...
Once configured, the Sentinel verifies every incoming request against the selected keyspaces. Requests without a valid API key receive a `401` response and never reach your service.
...
If all checks pass, the Sentinel produces a Principal and forwards the request with the `X-Unkey-Principal` header. See [Authentication](/platform/sentinel/authentication) for the header format and how to read it in your application.
