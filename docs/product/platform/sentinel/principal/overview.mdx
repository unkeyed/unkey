...
<ResponseField name="version" type="string" required>
  The schema version of the Principal payload. Currently `"v1"`. Check this field if you need to handle different payload shapes during migrations. Unkey bumps the version when fields are removed or renamed, but adding new optional fields does not change the version. The value is always a string so future schemes (date-based like `"2026-04-13"`, semver, named releases) can replace the current value without changing the field's type.
</ResponseField>
...
<ResponseField name="source" type="object" required>
  Method-specific data from the authentication source. Contains exactly one field whose name is the lowercase of `type` (`key` for `API_KEY`, `jwt` for `JWT`). Most applications only need `subject` and `identity`, but `source` provides the full detail when you need it: key permissions, JWT claims, expiration times, and other method-specific attributes.
