...
When you create a project, Unkey creates a default app automatically. For projects with a single service, this is all you need.
...
To add another app to an existing project:

1. Open the project from your workspace. The project home lists every app in the project.
2. Click **Create new app**.
3. Enter an **App name** and **Slug**. The slug is URL-friendly and auto-generated from the name; it must be unique within the project.
4. Connect a GitHub repository or skip to deploy with the [CLI](/build-and-deploy/cli) or a container image. If you've previously deployed an image, the onboarding flow suggests it so you don't have to retype the full reference.
5. Configure build and runtime settings, environment variables, and trigger the first deployment.

You can also create an app from the CLI by deploying with a new `--app` value. Unkey creates the app on first deploy.
