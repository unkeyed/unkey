...
Sentinel policies have moved to a dedicated **Sentinel Policies** page, accessible from the project sidebar. From there you can create, edit, reorder, and delete policies, configure [match conditions](/platform/sentinel/policies/overview#match-expressions), and control which environments each policy applies to.

See [Policies](/platform/sentinel/policies/overview) for the full list of policy types and [API key authentication](/platform/sentinel/policies/api-key) for details on configuring keyspace verification.
