...
Key rerolling (or key rotation) generates a new API key token while preserving all configuration from the existing key. Rotate credentials regularly without disrupting your application's permissions or settings.
...
<warning>
  **Security Critical**: The `key` field contains the actual API key token. This is the only time you'll receive it — Unkey stores only a hashed version. Never log or expose this value. Send it to your user through a secure channel.
</warning>
