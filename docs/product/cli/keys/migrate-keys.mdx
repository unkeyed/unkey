...
<ParamField body="--keys" type="JSON string" required>
...
Assigns roles to this key for permission management through role-based access control. Existing roles resolve in the target project. Missing roles are created in that project.
</ResponseField>
...
unkey api keys migrate-keys \
  ...
  --keys='[{"hash":"c4fbfe7c69a067cb0841dea343346a750a69908a08ea9656d2a8c19fb0823c64","enabled":true}]'
```
```bash With metadata and roles
unkey api keys migrate-keys \
  --migration-id=your_company \
  --api-id=api_123456789 \
  --keys='[{"hash":"c4fbfe7c69a067cb0841dea343346a750a69908a08ea9656d2a8c19fb0823c64","name":"Production API Key","externalId":"user_abc123","enabled":true,"meta":{"plan":"enterprise"},"roles":["admin"],"permissions":["api.read","api.write"]}]'
```
```bash JSON output for scripting
unkey api keys migrate-keys \
  --migration-id=your_company \
  --api-id=api_123456789 \
  --keys='[{"hash":"abc123","enabled":true}]' \
  --output=json
```
</CodeGroup>
