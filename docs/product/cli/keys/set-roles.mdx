...
Comma-separated list of roles. Replaces all existing roles on the key with this exact set. All roles must already exist in the key's project. Invalid role references cause the entire operation to fail atomically. Providing an empty value removes all role assignments from the key.
</ParamField>
