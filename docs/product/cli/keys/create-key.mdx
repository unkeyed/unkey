---
title: "create-key"
description: "Create a new API key using the Unkey CLI with options for expiration, metadata, rate limits, roles, and permissions in a single command."
---
...
Comma-separated list of role names to assign. Roles must already exist in the API's project. During verification, all permissions from assigned roles are checked against requested permissions.
</ParamField>
...
Default output shows the request ID, followed by the created key:
