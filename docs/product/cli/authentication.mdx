...
description: "Store your root key locally so the Unkey CLI can authenticate without passing it on every command."
---

The `unkey auth login` command stores your [root key](/security/root-keys) in a local config file so you don't have to pass `--root-key` on every CLI invocation.

## Prerequisites

- An Unkey account with a [root key](/security/root-keys)
...
- **Use a dedicated root key for the CLI** — create a separate key with only the permissions your CLI workflows need. See [root key permissions](/security/root-keys#create-a-root-key) for guidance.
