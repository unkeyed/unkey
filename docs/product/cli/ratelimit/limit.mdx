...
The cost is echoed back as `tokens` in the response so you can attribute spend per request without tracking it client-side.
</ParamField>
...
The `tokens` field reports how much of the quota this request consumed and matches the `--cost` flag (defaults to `1`). It is recorded for both passed and rejected requests so you can chart spend by outcome.
