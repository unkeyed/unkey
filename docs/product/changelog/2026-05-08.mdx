...
### Customer Portal early access

The white-labeled [Customer Portal](/quickstart/portal) is now available in early access. Give your end users self-service key management, usage analytics, and API docs without building any UI. Authentication uses a Stripe-style session flow your backend drives.
...
### Cross-region denial propagation for ratelimits

When a ratelimit decision denies a request in one region, the denial now propagates to other regions through a shared blocklist so the next request to a remote edge sees the same limit. This tightens enforcement for high-throughput, globally distributed traffic. See the [ratelimit modes guide](/platform/ratelimiting/modes) for the consistency model.
