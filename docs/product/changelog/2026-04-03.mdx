...
Requests proxied through the gateway now correctly preserve the original client IP address via the `X-Forwarded-For` header, so your application receives accurate client location data.
