...
Even so, we'd be foolish not to explain how you can add safeguards along the way.


For severe network degradations or other unforeseen events, you might want to set an upper bound on how long you wait for a response from Unkey.
By default the SDK rejects a request if it hasn't received a response from Unkey within 5 seconds. You can tune this via the `timeout` config in the constructor (see below).
...
Both the `fallback` property of the `timeout` config and `onError` are callback functions. They receive the original request identifier as a parameter. Use it to decide whether to reject the request.
