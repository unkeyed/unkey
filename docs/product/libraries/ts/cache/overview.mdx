...
This library is a rewrite of our own caching layer after some users started replicating it. It's used in production by Unkey and others.
...
The goal is that this is invisible to you. Everything behaves like a single cache.
...
To make data fetching as easy as possible, the cache offers a `swr` method, that acts as a pull through cache. If the data is fresh, it returns from the cache. If stale, it returns the cached value and triggers a background refresh. If not in the cache, it fetches synchronously from the origin.
