...
By default it extracts the API key from the `Authorization` header and verifies it with Unkey.
The verification result is written to the context, and you can access it with `c.get("unkey")`.
...
By default, the middleware only writes the verification response to the context.
You'll likely want to return a `401` response if the key is invalid.
