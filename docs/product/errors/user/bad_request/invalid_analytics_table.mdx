...
For security, each analytics endpoint accepts only its own public, versioned
table aliases.

`analytics.getVerifications` accepts:
...
`analytics.getRatelimits` accepts:

- `ratelimits_v1` - Raw rate limit events
- `ratelimits_per_minute_v1` - Minute-level aggregates
- `ratelimits_per_hour_v1` - Hour-level aggregates
- `ratelimits_per_day_v1` - Day-level aggregates
- `ratelimits_per_month_v1` - Month-level aggregates

Physical `default.*` table names, system tables such as `system.*`, and other
database tables are blocked.
