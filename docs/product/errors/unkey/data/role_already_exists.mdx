...
description: "A role with this name already exists in your Unkey project. Use the existing role or choose a different unique name when creating a new one."
---

<Danger>err:unkey:data:role_already_exists</Danger>
...
```json Example
{
  ...
  "error": {
    "detail": "A role with name \"admin\" already exists in this project",
    "status": 409,
    "title": "Conflict",
    "type": "https://unkey.com/docs/errors/unkey/data/role/duplicate"
  }
}
```
...
This error occurs when you create a role with a name that already exists in the project. Role names must be unique within a project.
...
- **Cross-project duplication**: Using the same role name twice in one project
