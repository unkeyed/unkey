...
description: "A permission with this slug already exists in the project. Use the existing permission or choose a different slug."
---
...
This error occurs when you create a permission with a slug that already exists in the target project. Permission slugs must be unique within each project. The same slug can exist in a different project.
...
async function getOrCreatePermission(name, slug, description) {
  try {
    ...
    if (error.status === 409) {
      // Extract the permission slug from the error message and get it
...
- **Case sensitivity**: Permission slugs are case-insensitive - "Admin" and "admin" are the same
