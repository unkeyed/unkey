...
description: "A permission with this slug already exists in your Unkey project. Use the existing permission or choose a different unique slug for the new one."
---

<Danger>err:unkey:data:permission_already_exists</Danger>
...
```json Example
{
  ...
  "error": {
    "detail": "A permission with slug \"admin\" already exists in this project",
    "status": 409,
    "title": "Conflict",
...
This error occurs when you create a permission with a slug that already exists in the project. Permission slugs must be unique within a project.

Common scenarios that trigger this error:

- Creating a permission with a slug that's already in use
- Re-creating a previously deleted permission with the same slug
...
```bash
# Attempting to create a permission with a slug that already exists
...
1. **Use a different slug**: If creating a new permission, use a unique slug
...
- **Not checking for existing permissions**: Failing to check if a permission slug already exists before creating it
- **Case sensitivity**: Permission slugs are case-insensitive. `Admin` and `admin` are the same slug
- **Retry loops**: Repeatedly trying to create the same permission after a failure
- **Cross-project duplication**: Using the same permission slug twice in one project
