...
```json Example
{
  ...
  "error": {
    "detail": "Environment \"preview\" cannot be deployed: Port must be greater than 0 (is 0); CPU millicores must be at least 250 (is 0); MemoryMib must be at least 256 (is 0); no schedulable regions are configured. Update the environment's settings before deploying.",
    "status": 400,
    "title": "Bad Request",
