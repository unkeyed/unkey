...
The secrets file uses standard `.env` syntax, one variable per line. Values are single-quoted, so they can contain spaces, special characters, and even span multiple lines (for example a PEM key or a JSON service account):
...
The `set -a && . /run/secrets/.env && set +a` pattern loads every variable from the file into the shell environment for that `RUN` step, multi-line values included. The secret file is not persisted in the final image.
...
- **Frameworks that read `.env` files directly** (Node.js with dotenv, Ruby with dotenv): reference `/run/secrets/.env` in your build script instead of sourcing it. Most dotenv libraries handle the single-quoted, multi-line format, but a value containing a literal single quote (`'`) is escaped for the shell and may not parse correctly when read directly. Source the file with the `set -a` pattern if you rely on such values.
