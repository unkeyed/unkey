...
A source build turns your source code into a container image that Unkey can
deploy. Deployments from prebuilt images bypass the source build process.
...
4. The source build slot is released.
5. The image is deployed to your configured [regions](/build-and-deploy/regions).
...
Each source build runs on a dedicated 16 vCPU, 32 GiB machine isolated to your project. No build shares resources with another project.

<Note>
  Source build concurrency is [workspace-wide](/limits#build-limits-and-resources).
  Additional source builds queue and run as active builds finish. Production
  builds receive priority over queued preview builds.
</Note>
...
```bash
unkey deploy ghcr.io/acme/api:v1.0.0 --project=acme-platform --app my-api
```
