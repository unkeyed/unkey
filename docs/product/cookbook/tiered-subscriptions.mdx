...
description: "Implement Free, Pro, and Enterprise subscription tiers with Unkey. Set different API key limits, rate limits, and permissions per plan."
---
  ...
  await unkey.keys.updateKey({
    ...
    ...(config.credits
      ...
      : {
          ...
          // (remaining: null also clears the refill schedule)
          credits: {
            remaining: null,
          },
        }),
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    ...
    usage: {
      credits: {
        used: tier.credits ? tier.credits - (data.credits ?? 0) : null,
        limit: tier.credits,
