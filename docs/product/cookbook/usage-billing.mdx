...
description: "Track API usage per customer for metered billing with Unkey. Use the remaining credits feature to count requests and enforce usage caps."
---
...
export async function createBillingKey(options: CreateBillingKeyOptions) {
  ...
  // The SDK throws on failure, so any error propagates to the caller.
...
export async function recordOverage(keyId: string, amount: number) {
  ...
  const { data: key } = await unkey.keys.getKey({ keyId });
...
export async function GET(request: Request) {
  ...
  const remaining = key.credits?.remaining ?? 0;
  const limit = key.credits?.refill?.amount ?? 0;
  ...
  return NextResponse.json({
    ...
    usage: {
      ...
      refillInterval: key.credits?.refill?.interval ?? null,
      refillDay: key.credits?.refill?.refillDay ?? null,
    },
  });
}
```
...
export async function upgradePlan(
  ...
  // Get current usage
  const { data: currentKey } = await unkey.keys.getKey({ keyId });
  const currentRemaining = currentKey.credits?.remaining ?? 0;
  const currentRefillAmount = currentKey.credits?.refill?.amount ?? 0;

  // Option 1: Add the difference (pro-rated)
  const creditsToAdd = newCredits - currentRefillAmount;
...
const { data: verification } = await unkey.keys.verifyKey({
  key: userKey,
...
<CardGroup cols={2}>
  <Card
    ...
    icon="layer-group"
    href="/cookbook/tiered-subscriptions"
  >
  ...
  <Card
    ...
    icon="coins"
    href="/platform/apis/features/remaining"
  >
