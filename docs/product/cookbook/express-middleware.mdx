...
description: "Build a reusable Express middleware for Unkey API key authentication. Verify keys automatically on every request with error handling."
---
...
export async function unkeyAuth(
  ...
  let data;
  try {
    // 2. Verify with Unkey (throws on transport/SDK errors)
...
app.get("/api/data", unkeyAuth, (req, res) => {
  ...
  const { identity, meta, credits } = req.unkey!;
...
export async function unkeyAuth(
  ...
  let data;
  try {
    ({ data } = await unkey.keys.verifyKey({ key: authHeader.slice(7) }));
  } catch {
...
export function requirePermission(permission: string) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    let data;
    try {
      ({ data } = await unkey.keys.verifyKey({
...
interface AuthOptions {
  ...
  onInvalid?: (
    req: Request,
    res: Response,
    result: V2KeysVerifyKeyResponseData,
  ) => void;
}
...
export function createAuthMiddleware(options: AuthOptions = {}) {
  const {
    ...
    onInvalid = (req, res, result) =>
      res.status(401).json({ error: result.code }),
  } = options;
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      return onError(req, res, err as Error);
    }
...
export async function unkeyAuth(
  ...
  try {
    ...
    // Log for monitoring
    console.error("Unkey verification error:", e);
