...
description: "Authenticate API keys with Cloudflare Workers and Unkey. Copy-paste recipe for low-latency key verification at the edge with global distribution."
---
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      // 2. Verify with Unkey (throws on transport/SDK errors — handled below)
...
export function authMiddleware(options: AuthOptions = {}) {
  return async (c: Context, next: Next) => {
    const getKey =
      options.getKey ??
      ((c) => c.req.header("Authorization")?.replace("Bearer ", "") ?? null);
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      const id = env.COUNTER.idFromName(
        data.identity?.externalId ?? "anonymous",
      );
