...
func X(){
	...
	p := Gateway[struct{X string}]{
		NewContext: func() struct{X string}{
			return struct{X string}{X: "string"}
		},
...
type  Gateway[SessionContext any] struct {


    // Define how the `ctx` should be created.
    NewSessionContext func()  SessionContext;
    ...
    requestFilter func(session Session[SessionContext]) (bool, error)
    ...
    requestBodyFilter func(

        session Session[SessionCoSessionContext],
        body io.Reader,
    ) error
    ...
    requestCacheFilter func(session  Session[SessionContext]) error
    ...
    cacheKeyCallback func(session Session[SessionContext]) (string, error)

    // This callback is invoked when a cacheable response is ready to be admitted to cache
    cacheMiss func( session Session[SessionContext])
    ...
    // actually needed after cache miss.
     proxyUpstreamFilter func(
        session Session[SSessionContext],
    ) (bool, error)

    // Decide if the response is cacheable
  //responseCacheFilter func(
  //      session *Session[SessionContext],
  //      resp &ResponseHeader,
  ...
  //  }
   ...
   // }
    ...
    // to the upstream.
    upstreamRequestFilter func(
        session *Session[SessionContext],
        upstreamRequest *RequestHeader,
    ) error
