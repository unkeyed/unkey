...
// WindowCountsUpsertParams is one row to upsert into ratelimit_window_counts.
...
// contribution.
type WindowCountsUpsertParams struct {
...
// BulkUpsertWindowCounts writes a batch of per-region count observations in a
...
//
// Hand-rolled rather than using sqlc's bulk-insert plugin for the same reason
// as BulkInsertBlocklist: adopting the plugin would require switching this
// package to emit_methods_with_db_argument, a wider refactor than warranted
// for one query.
func (d *Database) BulkUpsertWindowCounts(ctx context.Context, args []WindowCountsUpsertParams) error {
	if len(args) == 0 {
		return nil
	}
	...
	query := "INSERT INTO ratelimit_window_counts " +
