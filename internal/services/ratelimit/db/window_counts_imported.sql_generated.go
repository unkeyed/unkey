...
// versions:
//   sqlc v1.30.0
// source: window_counts_imported.sql
...
const windowCountsImported = `-- name: WindowCountsImported :many
SELECT
    workspace_id,
    namespace,
    identifier,
...
type WindowCountsImportedParams struct {
	Now        uint64 `db:"now"`
	SelfRegion string `db:"self_region"`
}

type WindowCountsImportedRow struct {
...
//	  AND region != ?
//	GROUP BY workspace_id, namespace, identifier, duration_ms, sequence
func (q *Queries) WindowCountsImported(ctx context.Context, arg WindowCountsImportedParams) ([]WindowCountsImportedRow, error) {
	rows, err := q.db.QueryContext(ctx, windowCountsImported, arg.Now, arg.SelfRegion)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []WindowCountsImportedRow
	for rows.Next() {
		var i WindowCountsImportedRow
		if err := rows.Scan(
			&i.WorkspaceID,
			&i.Namespace,
			&i.Identifier,
