...
//	FROM `limits`
//	WHERE workspace_id = ?
func (q *Queries) FindLimitsByWorkspaceID(ctx context.Context, db DBTX, workspaceID string) (Limit, error) {
	row := db.QueryRowContext(ctx, findLimitsByWorkspaceID, workspaceID)
	var i Limit
	err := row.Scan(
		&i.Pk,
		&i.WorkspaceID,
