...
Important: Role names must be unique within the default project. Once created, roles are immediately available for assignment.
...
For full documentation, see https://www.unkey.com/docs/api-reference/v2/permissions/create-role` + util.Disclaimer,
		...
		Flags: []cli.Flag{
			...
			util.ConfigFlag(),
			util.OutputFlag(),
			cli.String("name", "Unique name for the role within the default project.", cli.Required(), cli.MutuallyExclusive("body")),
