...
func getPortalCmd() *cli.Command {
	return &cli.Command{Name: "get-portal", Usage: "Read one portal, by its id or slug, or by the resource it serves.", Description: `Read one portal, by its id or slug, or by the resource it serves.
...
For full documentation, see https://www.unkey.com/docs/api-reference/portal/get-portal` + util.Disclaimer, Examples: []string{"unkey api portal get-portal --portal=acme-portal", "unkey api portal get-portal --keyspace-id=ks_1234abcd", "unkey api portal get-portal --app-id=app_1234abcd"}, Flags: []cli.Flag{cli.String("body", "Decode this JSON as the endpoint request body. Request-building flags are mutually exclusive."), util.RootKeyFlag(), util.APIURLFlag(), util.ConfigFlag(), util.OutputFlag(), cli.String("portal", "Portal ID or slug.", cli.MutuallyExclusive("body")), cli.String("keyspace-id", "ID of the keyspace served by the portal.", cli.MutuallyExclusive("body")), cli.String("app-id", "ID of the app served by the portal.", cli.MutuallyExclusive("body"))}, RequireOneOf: [][]string{{"portal", "keyspace-id", "app-id"}}, Action: func(ctx context.Context, cmd *cli.Command) error {
		...
		if portal := cmd.String("portal"); portal != "" {
			req = components.CreateV2PortalGetPortalRequestBodyUnionV2PortalGetPortalRequestBody1(components.V2PortalGetPortalRequestBody1{Portal: portal, KeyspaceID: nil, AppID: nil})
		} else if keyspaceID := cmd.String("keyspace-id"); keyspaceID != "" {
			req = components.CreateV2PortalGetPortalRequestBodyUnionV2PortalGetPortalRequestBody2(components.V2PortalGetPortalRequestBody2{Portal: nil, KeyspaceID: keyspaceID, AppID: nil})
		} else {
			req = components.CreateV2PortalGetPortalRequestBodyUnionV2PortalGetPortalRequestBody3(components.V2PortalGetPortalRequestBody3{Portal: nil, KeyspaceID: nil, AppID: cmd.String("app-id")})
		}
