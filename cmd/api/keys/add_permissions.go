...
func addPermissionsCmd() *cli.Command {
	return &cli.Command{
...
For full documentation, see https://www.unkey.com/docs/api-reference/v2/keys/add-key-permissions` + util.Disclaimer,
		...
		Flags: []cli.Flag{
			...
			util.ConfigFlag(),
			util.OutputFlag(),
			cli.String("key-id", "The key ID to add permissions to.", cli.Required(), cli.MutuallyExclusive("body")),
			cli.StringSlice("permissions", "Comma-separated list of permission names to add.", cli.Required(), cli.MutuallyExclusive("body")),
		},
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			if cmd.FlagIsSet("body") {
				...
				res, err := util.SendBody(ctx, client.Keys.AddPermissions, body)
				if err != nil {
					return err
				}
				return util.Output(cmd, res.V2KeysAddPermissionsResponseBody)
			}
