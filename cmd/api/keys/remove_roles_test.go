...
func TestRemoveRoles(t *testing.T) {
	tests := []struct {
		name string
		args string
		want openapi.V2KeysRemoveRolesRequestBody
	}{
		{
			name: "minimal required fields only",
			args: "keys remove-roles --key-id=key_1234abcd --roles=admin",
			want: openapi.V2KeysRemoveRolesRequestBody{
				KeyId: "key_1234abcd",
				Roles: []string{"admin"},
			},
		...
		{
			name: "multiple roles",
			args: "keys remove-roles --key-id=key_1234abcd --roles=admin,billing_reader,api_admin",
			want: openapi.V2KeysRemoveRolesRequestBody{
				KeyId: "key_1234abcd",
