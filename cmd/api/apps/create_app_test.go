...
func TestCreateApp(t *testing.T) {
	tests := []struct {
		...
		{
			"with git",
			`apps create-app --project=payments --name=Payments --slug=payments-api --git='{"repository":"unkeyed/api","defaultBranch":"main"}'`,
			map[string]any{
				"project": "payments",
				"name":    "Payments",
				"slug":    "payments-api",
				"git": map[string]any{
					"repository":    "unkeyed/api",
					"defaultBranch": "main",
				},
			},
		},
		{
			"with OCI",
			`apps create-app --project=payments --name=Payments --slug=payments-api --oci='{"image":"ghcr.io/acme/payments:v1.2.3"}'`,
			map[string]any{
				"project": "payments",
				"name":    "Payments",
				"slug":    "payments-api",
