#!/bin/bash
# Generate separate systemd-networkd configuration files for 32-bridge setup
# Each bridge needs its own .netdev and .network file

set -e

BRIDGE_COUNT=32
BASE_DIR="$(dirname "$0")/multi-bridge-32"

echo "Generating 32-bridge configuration files in $BASE_DIR"
mkdir -p "$BASE_DIR"
