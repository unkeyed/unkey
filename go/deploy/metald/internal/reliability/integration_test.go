...
func TestVMRegistryAdapter(t *testing.T) {
	...
	registry.AddVM(vmID, processID, config, metaldv1.VmState_VM_STATE_CREATED)
	...
	if vm.GetState() != metaldv1.VmState_VM_STATE_CREATED {
		t.Errorf("Expected state CREATED, got %s", vm.GetState())
	}
	...
	vm, _ = registry.GetVM(vmID)
	if vm.GetState() != metaldv1.VmState_VM_STATE_UNSPECIFIED {
		t.Errorf("Expected state UNSPECIFIED, got %s", vm.GetState())
	}
