...
func TestLoadConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			envVars: map[string]string{
				"UNKEY_VMCP_PORT":    "9999",
				"UNKEY_VMCP_ADDRESS": "127.0.0.1",
			},
			want: &Config{
				Server: ServerConfig{
					Port:    "9999",
		...
		},
		{
			...
			envVars: map[string]string{
				"UNKEY_VMCP_CH_ENDPOINT": "unix:///var/run/ch.sock",
			},
			want: &Config{
				Server: ServerConfig{
					Port:    "8080",
		...
		},
		{
			...
			envVars: map[string]string{
				...
				"UNKEY_VMCP_OTEL_SERVICE_NAME":       "test-service",
				"UNKEY_VMCP_OTEL_SERVICE_VERSION":    "2.0.0",
				"UNKEY_VMCP_OTEL_SAMPLING_RATE":      "0.5",
				"UNKEY_VMCP_OTEL_ENDPOINT":           "otel-collector:4318",
				"UNKEY_VMCP_OTEL_PROMETHEUS_ENABLED": "false",
				"UNKEY_VMCP_OTEL_PROMETHEUS_PORT":    "8888",
			},
			want: &Config{
				Server: ServerConfig{
					Port:    "8080",
		...
		},
		{
			...
			envVars: map[string]string{
				"UNKEY_VMCP_BACKEND": "firecracker",
			},
			want: &Config{
				Server: ServerConfig{
					Port:    "8080",
...
func TestLoadConfigWithSocketPath(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			socketPath: "",
			envVars: map[string]string{
				"UNKEY_VMCP_CH_ENDPOINT": "unix:///env/path.sock",
			},
...
func TestOpenTelemetryConfigValidation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			envVars: map[string]string{
				"UNKEY_VMCP_OTEL_ENABLED":       "true",
				"UNKEY_VMCP_OTEL_SAMPLING_RATE": "0.0",
			},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			envVars: map[string]string{
				"UNKEY_VMCP_OTEL_ENABLED":       "true",
				"UNKEY_VMCP_OTEL_SAMPLING_RATE": "-0.1",
			},
			wantErr: true,
		...
		},
		{
			...
			envVars: map[string]string{
				"UNKEY_VMCP_OTEL_ENABLED":       "true",
				"UNKEY_VMCP_OTEL_SAMPLING_RATE": "1.1",
			},
			wantErr: true,
			errMsg:  "tracing sampling rate must be between 0.0 and 1.0",
		},
		...
		// or using dependency injection for the defaults.
		{
			...
			envVars: map[string]string{
				...
				"UNKEY_VMCP_OTEL_SAMPLING_RATE": "5.0", // Invalid but should be ignored
				"UNKEY_VMCP_OTEL_ENDPOINT":      "",    // Empty but should be ignored
			},
			wantErr: false,
		},
	}
...
func clearEnv() {
	envVars := []string{
		"UNKEY_VMCP_PORT", "UNKEY_VMCP_ADDRESS", "UNKEY_VMCP_BACKEND", "UNKEY_VMCP_CH_ENDPOINT", "UNKEY_VMCP_FC_ENDPOINT",
		"UNKEY_VMCP_OTEL_ENABLED", "UNKEY_VMCP_OTEL_SERVICE_NAME", "UNKEY_VMCP_OTEL_SERVICE_VERSION",
		"UNKEY_VMCP_OTEL_SAMPLING_RATE", "UNKEY_VMCP_OTEL_ENDPOINT",
		"UNKEY_VMCP_OTEL_PROMETHEUS_ENABLED", "UNKEY_VMCP_OTEL_PROMETHEUS_PORT",
	}
