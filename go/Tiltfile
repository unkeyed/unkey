...
allow_k8s_contexts('admin@talos-default-7')
...
namespace_create('untrusted', allow_duplicates=True)
...
if start_mysql:
    ...
    if not local('docker images -q unkey/mysql:local 2>/dev/null', quiet=True):
...
if start_clickhouse:
    ...
    # Only build if image doesn't exist
    if not local('docker images -q unkey/clickhouse:local 2>/dev/null', quiet=True):
...
if start_api or start_ctrl or start_krane or start_preflight:
...
if start_preflight:
    print("Setting up Preflight webhook...")
...
- Use frontline for service access
