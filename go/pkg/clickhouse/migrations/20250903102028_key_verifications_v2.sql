...
PRIMARY KEY (`time`, `workspace_id`) ORDER BY (`time`, `workspace_id`) SETTINGS index_granularity = 8192;
-- Create "api_requests_per_hour_v1" table
CREATE TABLE `default`.`api_requests_per_hour_v1` (
  `time` DateTime,
  `workspace_id` String,
  `path` String,
...
CREATE TABLE `default`.`api_requests_per_minute_v1` (
  `time` DateTime,
  `workspace_id` String,
  `path` String,
  `response_status` Int32,
...
CREATE TABLE `default`.`api_requests_per_day_v1` (
  `time` DateTime,
  `workspace_id` String,
  `path` String,
  `response_status` Int32,
...
PRIMARY KEY (`workspace_id`, `time`, `key_space_id`, `identity_id`, `key_id`, `tags`, `outcome`) ORDER BY (`workspace_id`, `time`, `key_space_id`, `identity_id`, `key_id`, `tags`, `outcome`) TTL time + toIntervalDay(100) SETTINGS index_granularity = 8192;
...
PRIMARY KEY (`workspace_id`, `time`, `key_space_id`, `identity_id`, `key_id`, `tags`, `outcome`) ORDER BY (`workspace_id`, `time`, `key_space_id`, `identity_id`, `key_id`, `tags`, `outcome`) TTL time + toIntervalDay(30) SETTINGS index_granularity = 8192;
...
PRIMARY KEY (`workspace_id`, `time`, `key_space_id`, `identity_id`, `key_id`, `tags`, `outcome`) ORDER BY (`workspace_id`, `time`, `key_space_id`, `identity_id`, `key_id`, `tags`, `outcome`) TTL time + toIntervalDay(7) SETTINGS index_granularity = 8192;
...
PRIMARY KEY (`workspace_id`, `time`, `key_space_id`, `identity_id`, `key_id`, `tags`, `outcome`) ORDER BY (`workspace_id`, `time`, `key_space_id`, `identity_id`, `key_id`, `tags`, `outcome`) TTL time + toIntervalYear(3) SETTINGS index_granularity = 8192;
...
PRIMARY KEY (`workspace_id`, `time`, `key_space_id`, `identity_id`, `key_id`) ORDER BY (`workspace_id`, `time`, `key_space_id`, `identity_id`, `key_id`) TTL toDateTime(fromUnixTimestamp64Milli(time)) + toIntervalMonth(1) SETTINGS index_granularity = 8192, non_replicated_deduplication_window = 10000;
...
PRIMARY KEY (`workspace_id`, `time`, `namespace_id`, `identifier`) ORDER BY (`workspace_id`, `time`, `namespace_id`, `identifier`) TTL toDateTime(fromUnixTimestamp64Milli(time)) + toIntervalMonth(1) SETTINGS index_granularity = 8192, non_replicated_deduplication_window = 10000;
...
CREATE MATERIALIZED VIEW `default`.`billable_verifications_per_month_mv_v2` TO `default`.`billable_verifications_per_month_v2` AS SELECT workspace_id, sum(count) AS count, toYear(time) AS year, toMonth(time) AS month FROM verifications.key_verifications_per_month_v2 WHERE outcome = 'VALID' GROUP BY workspace_id, year, month;
-- Create "key_verifications_per_day_mv_v2" view
CREATE MATERIALIZED VIEW `default`.`key_verifications_per_day_mv_v2` TO `default`.`key_verifications_per_day_v2` AS SELECT workspace_id, key_space_id, identity_id, key_id, outcome, count(*) AS count, avgState(latency) AS latency_avg, quantilesTDigestState(0.75)(latency) AS latency_p75, quantilesTDigestState(0.99)(latency) AS latency_p99, toStartOfDay(fromUnixTimestamp64Milli(time)) AS time FROM default.key_verifications_raw_v2 GROUP BY workspace_id, key_space_id, identity_id, key_id, outcome, time;
-- Create "key_verifications_per_hour_mv_v2" view
CREATE MATERIALIZED VIEW `default`.`key_verifications_per_hour_mv_v2` TO `default`.`key_verifications_per_hour_v2` AS SELECT workspace_id, key_space_id, identity_id, key_id, outcome, count(*) AS count, avgState(latency) AS latency_avg, quantilesTDigestState(0.75)(latency) AS latency_p75, quantilesTDigestState(0.99)(latency) AS latency_p99, toStartOfHour(fromUnixTimestamp64Milli(time)) AS time FROM default.key_verifications_raw_v2 GROUP BY workspace_id, key_space_id, identity_id, key_id, outcome, time;
...
CREATE MATERIALIZED VIEW `default`.`ratelimits_per_month_mv_v2` TO `default`.`ratelimits_per_month_v2` AS SELECT workspace_id, namespace_id, identifier, count(*) AS total, countIf(passed > 0) AS passed, avgState(latency) AS latency_avg, quantilesTDigestState(0.75)(latency) AS latency_p75, quantilesTDigestState(0.99)(latency) AS latency_p99, toStartOfMonth(fromUnixTimestamp64Milli(time)) AS time FROM default.ratelimits_raw_v2 GROUP BY workspace_id, namespace_id, identifier, time;
