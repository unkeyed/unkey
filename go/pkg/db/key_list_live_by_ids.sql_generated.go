...
// versions:
//   sqlc v1.27.0
// source: key_list_live_by_ids.sql
...
const listLiveKeysByIDs = `-- name: ListLiveKeysByIDs :many
SELECT
...
type ListLiveKeysByIDsRow struct {
	ID                 string         `db:"id"`
...
//	    AND ws.deleted_at_m IS NULL
//	ORDER BY k.id
func (q *Queries) ListLiveKeysByIDs(ctx context.Context, db DBTX, keyIds []string) ([]ListLiveKeysByIDsRow, error) {
	query := listLiveKeysByIDs
	...
	defer rows.Close()
	var items []ListLiveKeysByIDsRow
	for rows.Next() {
		var i ListLiveKeysByIDsRow
		if err := rows.Scan(
			&i.ID,
			&i.KeyAuthID,
			&i.Hash,
