...
// versions:
//   sqlc v1.27.0
// source: key_list_live_by_api_id.sql
...
const listLiveKeysByApiID = `-- name: ListLiveKeysByApiID :many
SELECT
...
type ListLiveKeysByApiIDParams struct {
...
type ListLiveKeysByApiIDRow struct {
	ID                 string         `db:"id"`
...
//	ORDER BY k.id ASC
//	LIMIT ?
func (q *Queries) ListLiveKeysByApiID(ctx context.Context, db DBTX, arg ListLiveKeysByApiIDParams) ([]ListLiveKeysByApiIDRow, error) {
	rows, err := db.QueryContext(ctx, listLiveKeysByApiID,
		arg.ApiID,
		arg.WorkspaceID,
		arg.IDCursor,
		arg.Identity,
	...
	defer rows.Close()
	var items []ListLiveKeysByApiIDRow
	for rows.Next() {
		var i ListLiveKeysByApiIDRow
		if err := rows.Scan(
			&i.ID,
			&i.KeyAuthID,
			&i.Hash,
