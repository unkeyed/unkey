...
// Config defines the parameters needed to establish partition database connections.
// It supports separate connections for read and write operations to allow
// for primary/replica setups across regions.
type Config struct {
...
// New creates a new partition database instance with the provided configuration.
...
func New(config Config) (*database, error) {
	...
	if config.ReadOnlyDSN != "" {
		...
		config.Logger.Info("partition database configured with separate read replica")
	} else {
		config.Logger.Info("partition database configured without separate read replica, using primary for reads")
	}
