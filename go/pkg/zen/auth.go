...
func Bearer(s *Session) (string, error) {
	...
	if header == "" {
		return "", fault.New("empty authorization header", fault.WithTag(fault.BAD_REQUEST))
	}

	header = strings.TrimSpace(header)
	if !strings.HasPrefix(header, "Bearer ") {
		return "", fault.New("invalid format", fault.WithTag(fault.BAD_REQUEST),
	...
	if bearer == "" {
		return "", fault.New("invalid token", fault.WithTag(fault.BAD_REQUEST))
	}
