...
func TestCacheInvalidation(t *testing.T) {
	...
	t.Run("verify cache invalidation", func(t *testing.T) {
		...
		_, hit, err := h.Caches.LiveApiByID.SWR(ctx, cache.ScopedKey{WorkspaceID: h.Resources().UserWorkspace.ID, Key: api.ID}, func(ctx context.Context) (db.FindLiveApiByIDRow, error) {
		...
		_, hit = h.Caches.LiveApiByID.Get(ctx, cache.ScopedKey{WorkspaceID: h.Resources().UserWorkspace.ID, Key: api.ID})
