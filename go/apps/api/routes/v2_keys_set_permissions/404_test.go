...
func TestNotFound(t *testing.T) {
	...
	t.Run("non-existent key ID", func(t *testing.T) {
		...
		permissionSlugAndName := "documents.read.notfound"
	...
	t.Run("non-existent permission slug", func(t *testing.T) {
		...
		req := handler.Request{
			KeyId:       keyID,
			Permissions: []string{"doesnt.exist"},
		}
	...
	t.Run("key from different workspace (isolation)", func(t *testing.T) {
		...
		permissionID := uid.New(uid.TestPrefix)
		permissionSlugAndName := "documents.read.isolation"
		err := db.Query.InsertPermission(ctx, h.DB.RW(), db.InsertPermissionParams{
			PermissionID: permissionID,
			WorkspaceID:  workspace.ID,
			Name:         permissionSlugAndName,
	...
	t.Run("permission from different workspace (isolation)", func(t *testing.T) {
		...
		permissionID := uid.New(uid.TestPrefix)
		permissionSlugAndName := "documents.read.otherWorkspace"
		err := db.Query.InsertPermission(ctx, h.DB.RW(), db.InsertPermissionParams{
			PermissionID: permissionID,
			WorkspaceID:  otherWorkspace.ID,
			Name:         permissionSlugAndName,
