...
func TestNotFoundErrors(t *testing.T) {
	...
	t.Run("non-existent role ID", func(t *testing.T) {
		...
		require.NotNil(t, res.Body.Error)
		require.Equal(t, "Not Found", res.Body.Error.Title)
		require.Contains(t, res.Body.Error.Detail, fmt.Sprintf("Role with ID %q was not found", nonExistentRoleID))
		require.Equal(t, 404, res.Body.Error.Status)
	})
	...
	t.Run("valid role format but non-existent", func(t *testing.T) {
		...
		require.NotNil(t, res.Body.Error)
		require.Equal(t, "Not Found", res.Body.Error.Title)
		require.Contains(t, res.Body.Error.Detail, fmt.Sprintf("Role with ID %q was not found", validFormattedRoleID))
		require.Equal(t, 404, res.Body.Error.Status)
	})
}
