...
func TestListOverridesSuccessfully(t *testing.T) {
	...
	t.Run("get by namespace name", func(t *testing.T) {
		...
		require.Len(t, res.Body.Overrides, 1)
		require.Equal(t, overrideID, res.Body.Overrides[0].OverrideId)
		require.Equal(t, namespaceID, res.Body.Overrides[0].NamespaceId)
		require.Equal(t, identifier, res.Body.Overrides[0].Identifier)
		require.Equal(t, int64(limit), res.Body.Overrides[0].Limit)
		require.Equal(t, int64(duration), res.Body.Overrides[0].Duration)
	})
