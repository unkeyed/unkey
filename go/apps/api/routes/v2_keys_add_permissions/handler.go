...
func New(svc Services) zen.Route {
	return zen.NewRoute("POST", "/v2/keys.addPermissions", func(ctx context.Context, s *zen.Session) error {
		svc.Logger.Debug("handling request", "requestId", s.RequestID(), "path", "/v2/keys.addPermissions")
		...
		if len(permissionsToAdd) > 0 {
			...
			if len(auditLogs) > 0 {
				...
				if err != nil {
					return fault.Wrap(err,
						fault.Code(codes.App.Internal.ServiceUnavailable.URN()),
						fault.Internal("audit log error"), fault.Public("Failed to create audit log for permission additions."),
					)
				}
			}
		...
		finalPermissions, err := db.Query.ListDirectPermissionsByKeyID(ctx, svc.DB.RW(), req.KeyId)
		if err != nil {
			return fault.Wrap(err,
				fault.Code(codes.App.Internal.ServiceUnavailable.URN()),
