...
func TestAuthorizationErrors(t *testing.T) {
	authz.Test403(t,
		authz.PermissionTestConfig[handler.Request, handler.Response]{
			...
			RequiredPermissions: []string{"api.*.update_key", "rbac.*.add_permission_to_key"},
			SetupResources: func(h *testutil.Harness) authz.TestResources {
				...
				permID := h.CreatePermission(seed.CreatePermissionRequest{
					WorkspaceID: h.Resources().UserWorkspace.ID,
					Name:        "documents.read",
					Slug:        "documents-read",
				})
			...
			CreateRequest: func(res authz.TestResources) handler.Request {
				return handler.Request{
					KeyId:       res.KeyID,
					Permissions: []string{res.Custom["permission_id"]},
				}
			},
		},
	)
