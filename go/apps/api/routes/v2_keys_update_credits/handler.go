...
func (h *Handler) Handle(ctx context.Context, s *zen.Session) error {
	...
	key, err = db.TxWithResult(ctx, h.DB.RW(), func(ctx context.Context, tx db.DBTX) (db.FindKeyByIDRow, error) {
		...
		if err != nil {
			return db.FindKeyByIDRow{}, fault.Wrap(err,
				fault.Code(codes.App.Internal.ServiceUnavailable.URN()),
				fault.Internal("database error"),
		...
		if keyErr != nil {
			...
			return db.FindKeyByIDRow{}, fault.Wrap(keyErr,
				fault.Code(codes.App.Internal.ServiceUnavailable.URN()),
				fault.Internal("database error"),
				fault.Public("Failed to retrieve key information."),
			)
