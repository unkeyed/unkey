...
func TestBadRequests(t *testing.T) {
	...
	t.Run("missing external id", func(t *testing.T) {
		...
		require.Equal(t, "POST request body for '/v2/identities.createIdentity' failed to validate schema", res.Body.Detail)
	...
	t.Run("metadata exceeds maximum size limit", func(t *testing.T) {
		...
		require.Equal(t, "https://unkey.com/docs/errors/bad_request", res.Body.Type)
		require.Equal(t, fmt.Sprintf("Metadata is too large, it must be less than %dMB, got: %.2f", handler.MAX_META_LENGTH_MB, float64(len(rawMeta))/1024/1024), res.Body.Detail)
		require.Equal(t, http.StatusBadRequest, res.Body.Status)
		require.Equal(t, "Bad Request", res.Body.Title)
		require.NotEmpty(t, res.Body.RequestId)
		require.Nil(t, res.Body.Instance)
