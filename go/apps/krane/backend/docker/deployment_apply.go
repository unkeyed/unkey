...
// ApplyDeployment creates or updates containers for a deployment in an idempotent manner.
//
// This method ensures that the specified number of containers exist for the deployment.
// If containers already exist with the same deployment ID, they are reused rather than
// recreated. The operation is fully idempotent and can be safely retried.
//
// The method:
//   - Checks for existing containers with the deployment ID label
...
func (d *docker) ApplyDeployment(ctx context.Context, req backend.ApplyDeploymentRequest) error {
	...
	// Check existing containers for this deployment
	...
	for i := range req.Replicas {
		containerName := fmt.Sprintf("%s-%d", req.DeploymentID, i)
