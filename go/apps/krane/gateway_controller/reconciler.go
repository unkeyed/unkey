...
func (gc *GatewayController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		if k8serrors.IsNotFound(err) { // not found, we can delete the resources
			logger.Info("gateway crd not found, deleting...")
	...
	if err != nil {
		if k8serrors.IsNotFound(err) {
			...
			gc.logger.Info("new gateway deployment created")
	...
	if err != nil {
		if k8serrors.IsNotFound(err) {
			...
			gc.logger.Info("new gateway service created")
			return requeueNow, nil
		}
		return ctrl.Result{}, err
	}
	...
	gc.logger.Info("gateway is fully synced")
	return requeueNever, nil
}
...
func buildDeploymentObject(desired gatewayv1.Gateway) (*appsv1.Deployment, *corev1.Service) {

	labels := k8s.NewLabels().WorkspaceID(desired.Spec.WorkspaceId).ProjectID(desired.Spec.ProjectId).EnvironmentID(desired.Spec.EnvironmentId).GatewayID(desired.Spec.GatewayId).ManagedByKrane().ToMap()
						...
						Containers: []corev1.Container{
							{
								...
								Env: []corev1.EnvVar{
									...
									{Name: "UNKEY_GATEWAY_ID", Value: desired.Spec.GatewayId},
...
func (gc *GatewayController) updateDeployment(ctx context.Context, deployment *appsv1.Deployment) (ctrl.Result, error) {
...
func (gc *GatewayController) updateService(ctx context.Context, service *corev1.Service) (ctrl.Result, error) {
