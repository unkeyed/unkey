 ...
 * remain the billing write path. The query reads the physical table with FINAL
 * because some ClickHouse versions do not propagate the equivalent view's
 * primary-key order and force the window input through a redundant full sort.
 */
export function getDeployMeterUsage(ch: Querier) {
  ...
  return async (args: {
    ...
    const result = await query({ ...args, maxGapMs: MAX_SAMPLE_GAP_MS });
    if (result.err) {
