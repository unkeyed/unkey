...
function buildHybridQuery(metric: Metric, mvTable: string): string {
  ...
  return `
    ...
    ORDER BY time ASC
    WITH FILL
      FROM toStartOfInterval(now() - INTERVAL {windowSeconds: UInt32} SECOND, INTERVAL {bucketSeconds: UInt32} SECOND)
      TO toStartOfInterval(now(), INTERVAL {bucketSeconds: UInt32} SECOND)
      STEP INTERVAL {bucketSeconds: UInt32} SECOND`;
}
