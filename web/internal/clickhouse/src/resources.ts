...
function buildHybridQuery(metric: Metric, mvTable: string): string {
  ...
  return `
    ...
    ORDER BY x ASC
    WITH FILL
      FROM toInt64(toUnixTimestamp(toStartOfInterval(now() - INTERVAL {windowSeconds: UInt32} SECOND, INTERVAL {bucketSeconds: UInt32} SECOND)) * 1000)
      TO toInt64(toUnixTimestamp(toStartOfInterval(now(), INTERVAL {bucketSeconds: UInt32} SECOND)) * 1000)
