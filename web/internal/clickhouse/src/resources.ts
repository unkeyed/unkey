...
function buildHybridQuery(metric: Metric, mvTable: string): string {
  // Pin a single `now()` evaluation as `pinned_now`, then derive every other
  // boundary (cold/live split, MV window start, WITH FILL FROM/TO) from it.
  // Without this, each independent `now()` call could resolve to a different
  // millisecond — and if two calls land in different buckets, the cold/live
  // split disagrees with the WITH FILL padding, producing duplicate or
  // missing buckets at the edges. The -GRACE offset on the cutoff gives the
  // MV a few seconds to catch up during ingest bursts before its values
  // become authoritative for a closed bucket.
  const cutoffCTE = `pinned AS (SELECT now() AS n),
  cutoff AS (
    SELECT
      (SELECT n FROM pinned) AS pinned_now,
      toStartOfInterval(
        (SELECT n FROM pinned) - INTERVAL {graceSeconds: UInt16} SECOND,
        INTERVAL {bucketSeconds: UInt32} SECOND
      ) AS t,
      toStartOfInterval(
        (SELECT n FROM pinned) - INTERVAL {windowSeconds: UInt32} SECOND,
        INTERVAL {bucketSeconds: UInt32} SECOND
      ) AS window_start,
      toStartOfInterval(
        (SELECT n FROM pinned),
        INTERVAL {bucketSeconds: UInt32} SECOND
      ) AS window_end
  )`;
  ...
  return `
    ...
    FROM (
      ...
      FROM ${mvTable}
      WHERE ${RESOURCE_FILTER}
        AND time >= (SELECT window_start FROM cutoff)
    ...
    ORDER BY x ASC
    WITH FILL
      FROM toInt64(toUnixTimestamp((SELECT window_start FROM cutoff)) * 1000)
      TO toInt64(toUnixTimestamp((SELECT window_end FROM cutoff)) * 1000)
