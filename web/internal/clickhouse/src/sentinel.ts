...
// ─────────────────────────────────────────────────────────────
// App Requests Timeseries (app-level: sums across deployments)
// ─────────────────────────────────────────────────────────────

export const appRequestsTimeseriesRequestSchema = z.object({
  workspaceId: z.string(),
  projectId: z.string(),
  deploymentIds: z.array(z.string()),
  windowHours: z.number().int().positive(),
});
export type AppRequestsTimeseriesRequest = z.infer<typeof appRequestsTimeseriesRequestSchema>;

const appRequestsPointSchema = z.object({
  x: z.number().int(),
  requests: z.number().int(),
  errors: z.number().int(),
});

// Request + error counts bucketed at 15m, summed across every deployment that
// served the app in the window. Reads the raw table rather than the per-15m MV
// because the MV doesn't carry response_status, which the error count needs.
// Buckets with no traffic are absent here and filled with zeros caller-side.
export function getAppRequestsTimeseries(ch: Querier) {
  return async (args: AppRequestsTimeseriesRequest) => {
    const query = ch.query({
      query: `
        SELECT
          toUnixTimestamp(
            toStartOfInterval(toDateTime(intDiv(time, 1000)), INTERVAL {intervalMinutes: UInt8} MINUTE)
          ) * 1000 AS x,
          toUInt64(count()) AS requests,
          toUInt64(countIf(response_status >= 500)) AS errors
        FROM ${TABLE}
        WHERE workspace_id = {workspaceId: String}
          AND project_id = {projectId: String}
          AND deployment_id IN {deploymentIds: Array(String)}
          AND time >= toUnixTimestamp(now() - INTERVAL {windowHours: UInt16} HOUR) * 1000
        GROUP BY x
        ORDER BY x ASC`,
      params: appRequestsTimeseriesRequestSchema.extend({
        intervalMinutes: z.number(),
      }),
      schema: appRequestsPointSchema,
    });
