...
import { type MetricsScope, type UsageMetric, buildMetricsChart } from "./metrics-chart-data";
...
function createScopeFilter(field: MetricsScopeFilter["field"], value: string): MetricsScopeFilter {
  return { field, value };
}
...
function metricsPeriod(now: Date, monthsAgo: MonthsAgo) {
...
export function MetricsExplorer({ scope }: { scope: MetricsScope }) {
  const [metric, setMetric] = useState<UsageMetric>("egressGiB");
  const [groupBy, setGroupBy] = useState<DeployUsageTimeseriesGroup>("project");
  const [interval, setInterval] = useState<DeployUsageTimeseriesInterval>("day");
  const [projectId, setProjectId] = useState(ALL_PROJECTS);
  const [scopeFilters, setScopeFilters] = useState<MetricsScopeFilter[]>([]);
  ...
  const period = metricsPeriod(now, monthsAgo);
  ...
  const periodOptions = PERIODS.map((value) => ({
    ...
    label: metricsPeriod(now, value).label,
  }));
  const scopeProjects = useMemo(
    () =>
      projectId === ALL_PROJECTS
        ? scope.projects
        : scope.projects.filter((project) => project.projectId === projectId),
  ...
  const deploymentAnnotations = trpc.billing.queryDeployUsageAnnotations.useQuery(
    interval === "hour"
      ? { interval, day: selectedDayStart, scope: queryScope, monthsAgo }
      : { interval, scope: queryScope, monthsAgo },
    {
  ...
  const overview = buildMetricsChart({
      ...
      <div className="flex flex-wrap items-end gap-3 px-4 py-3">
        ...
        <ChartSelect
          ...
          onValueChange={(value) => {
            if (
              value === ALL_PROJECTS ||
              scope.projects.some((project) => project.projectId === value)
            ) {
              setProjectId(value);
              setScopeFilters([]);
            }
...
function MetricsDayNavigator({
  ...
  periodLabel: string;
  data: ReturnType<typeof buildMetricsChart>["data"];
  config: ChartConfig;
  isLoading: boolean;
  isError: boolean;
...
function ChartScope({
  ...
  filters: MetricsScopeFilter[];
  updateFilters: (filters: MetricsScopeFilter[]) => void;
  open: boolean;
      ...
      <Popover open={open} onOpenChange={onOpenChange}>
        ...
        <PopoverContent align="start" className="w-auto p-0">
          <AppEnvironmentFilterList
            ...
            createFilter={createScopeFilter}
