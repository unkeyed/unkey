...
import { type MetricsScope, buildMetricsChart } from "./metrics-chart-data";
...
describe("buildMetricsChart", () => {
  it("fills empty buckets and maps rows to named series", () => {
    ...
    const result = buildMetricsChart({
      rows,
      metric: "egressGiB",
      groupBy: "project",
      interval: "hour",
  ...
  it("keeps a group's color when its usage rank changes", () => {
    ...
    const second = buildMetricsChart({
      rows: [
        {
          time: 0,
          groupId: "app_1",
    ...
    expect(first.series.find((series) => series.label === "Acme / API")?.color).toBe(
      second.series.find((series) => series.label === "Acme / API")?.color,
    );
