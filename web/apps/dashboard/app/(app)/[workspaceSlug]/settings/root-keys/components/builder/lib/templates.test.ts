...
describe("TEMPLATES", () => {
  ...
  it("leaves decrypt out of All write permissions", () => {
    expect(urnsOf("write")).not.toContain("unkey:v1:ws_123:keyspaces/*/keys/*#decrypt_key");
    expect(urnsOf("write").some((urn) => urn.includes("decrypt"))).toBe(false);
    expect(materialise("write")[0].selection.key).toEqual(["read", "write", "delete"]);
  });

  it("leaves decrypt out of every template", () => {
    for (const template of TEMPLATES) {
      expect(buildUrns(ws, template.materialise()).some((urn) => urn.includes("decrypt"))).toBe(
        false,
      );
    }
  });
