...
describe("actionLabel", () => {
  it("composes the ticked actions", () => {
    ...
    expect(actionLabel(["limit"])).toBe("Limit");
  });
...
describe("policySummary on instance scopes", () => {
  ...
  it("composes grants from the catalogue of its own scope", () => {
    ...
    const namespace = {
      ...
      selection: setRowActions({}, "ratelimit_override", ["read", "write", "delete"]),
    };
    expect(policySummary(namespace).grants).toEqual(["Rate limit overrides Read, Write & Delete"]);
  });

  it("names the narrow key actions in the summary", () => {
    const policy = {
      ...newPolicy("keyspaces"),
      selection: setRowActions({}, "key", ["read", "verify", "decrypt"]),
    };
    expect(policySummary(policy).grants).toEqual(["Keys Read, Verify & Decrypt"]);
  });
