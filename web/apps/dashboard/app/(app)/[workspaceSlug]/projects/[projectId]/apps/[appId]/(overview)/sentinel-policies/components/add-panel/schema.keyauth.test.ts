...
import type { SentinelPolicy } from "./schema";
...
describe("keyauth credits override", () => {
  ...
  it("serializes a 0 credits override onto the wire", () => {
    ...
    const wire = toSentinelPolicy(parsed) as Extract<SentinelPolicy, { type: "keyauth" }>;
  ...
  it("round-trips a credits override through the wire form", () => {
    ...
    const wire = toSentinelPolicy(parsed) as SentinelPolicy;
    const back = fromSentinelPolicy(wire, "__all__");
