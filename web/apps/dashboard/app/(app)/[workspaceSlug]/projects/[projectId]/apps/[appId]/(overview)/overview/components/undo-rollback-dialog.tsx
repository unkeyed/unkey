...
type UndoRollbackDialogProps = {
  isOpen: boolean;
  onClose: () => void;
  // Ready production deployments, newest first. Includes the current live one
  // (the deployment rolled back to) so the user can resume deploys without moving.
  deployments: Deployment[];
  currentDeploymentId: string;
  // Fired after a successful promote, before the collections refetch lands.
  onUndone?: () => void;
};
...
export function UndoRollbackDialog({
  ...
  // Default to the newest deployment that isn't the one we rolled back to — the
  // deployment you'd land on by undoing. Falls back to current if it's the only one.
  const defaultId =
