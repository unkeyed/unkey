...
export const healthcheckSchema = z.object({
  ...
  interval: z
    ...
    // The regex accepts "0s", but the server requires intervalSeconds >= 1;
    // reject a zero interval here so the form fails before the mutation 400s.
    .refine((value) => intervalToSeconds(value) >= 1, "Interval must be at least 1 second"),
});
