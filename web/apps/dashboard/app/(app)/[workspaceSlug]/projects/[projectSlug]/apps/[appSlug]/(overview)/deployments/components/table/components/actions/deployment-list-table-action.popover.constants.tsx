...
export const DeploymentListTableActions = ({
  ...
  const { getDeploymentById, project, projectSlug, appSlug } = useProjectData();
  ...
  const menuItems = useMemo((): MenuItem[] => {
    ...
    return [
      ...
      },
      {
        ...
        onClick: () => {
          router.push(
            `/${workspace.slug}/projects/${projectSlug}/apps/${appSlug}/requests?since=6h&deploymentId=contains:${selectedDeployment.id}`,
          );
        },
      },
      {
        ...
        icon: <Layers3 iconSize="md-regular" />,
        onClick: () => {
          router.push(`/${workspace.slug}/projects/${projectSlug}/apps/${appSlug}/logs`);
        },
      },
      {
        id: "build-steps",
        ...
        onClick: () => {
          router.push(
            `/${workspace.slug}/projects/${projectSlug}/apps/${appSlug}/deployments/${selectedDeployment.id}?build=true`,
          );
        },
      },
    ];
