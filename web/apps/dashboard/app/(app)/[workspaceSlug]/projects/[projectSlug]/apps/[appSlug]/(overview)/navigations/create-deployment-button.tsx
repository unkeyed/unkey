...
export const CreateDeploymentButton = ({
  ...
  const { projectId, appId, projectSlug, appSlug, project, environments } = useProjectData();
  ...
  const createDeployment = trpc.deploy.deployment.create.useMutation({
    async onSuccess(data) {
      ...
      router.push(
        `/${params.workspaceSlug}/projects/${projectSlug}/apps/${appSlug}/deployments/${data.deploymentId}`,
      );
    },
    onError(err) {
      console.error(err);
