...
export const useBreadcrumbConfig = ({
  ...
  // All tabs live under the app, e.g. /projects/[projectSlug]/apps/[appSlug]/deployments
  const appBase = `${basePath}/${projectSlug}/apps/${appSlug}`;
  ...
  const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
      id: "project",
      children: activeProject?.name || projectSlug,
      href: `${basePath}/${projectSlug}/apps`,
      shouldRender: true,
      active: false,
      isLast: false,
      noop: true,
      ...
      quickNavConfig: {
        items: projects.map((project) => ({
          id: project.id,
          label: project.name,
          href: `${basePath}/${project.slug}/apps`,
        })),
        shortcutKey: "N",
      },
    },
    ...
    // 3. Current app with QuickNav (switch apps within the project)
    {
      ...
      quickNavConfig: {
        items: apps.map((app) => ({
          id: app.id,
          label: app.name,
          href: `${basePath}/${projectSlug}/apps/${app.slug}/deployments`,
        })),
        shortcutKey: "A",
      },
    },
