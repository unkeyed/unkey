...
export function DeploymentProgress({ stepsData }: { stepsData?: StepsData }) {
  ...
  const { getDomainsForDeployment, projectSlug, appSlug } = useProjectData();
  ...
  useEffect(() => {
    if (network?.completed) {
      router.push(
        `/${workspaceSlug}/projects/${projectSlug}/apps/${appSlug}/deployments/${deployment.id}`,
      );
    }
  }, [network?.completed, router, workspaceSlug, projectSlug, appSlug, deployment.id]);
    ...
    <div className="flex flex-col gap-5">
      ...
      {isFailed && (
        <FailedDeploymentBanner
          ...
          settingsUrl={`/${workspaceSlug}/projects/${projectSlug}/apps/${appSlug}/settings`}
