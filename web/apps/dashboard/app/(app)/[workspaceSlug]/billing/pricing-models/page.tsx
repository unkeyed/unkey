...
const pricingModelSchema = z.object({
  ...
  verificationUnitPrice: z.number().min(0, "Price must be non-negative"),
  ratelimitUnitPrice: z.number().min(0, "Price must be non-negative"),
});
...
export default function PricingModelsPage() {
  ...
  const handleCreate = (data: PricingModelFormData) => {
    createMutation.mutate({
      ...data,
      verificationUnitPrice: Math.round(data.verificationUnitPrice * 100),
      ratelimitUnitPrice: Math.round(data.ratelimitUnitPrice * 100),
    });
  };
  ...
  const openEditDialog = (model: PricingModelItem) => {
    ...
    editForm.reset({
      ...
      ratelimitUnitPrice: model.ratelimitUnitPrice / 100,
    });
  };
                ...
                >
                  <div className="flex items-center gap-4 w-full justify-end">
                    <div className="text-sm text-gray-11">
                      <div>
                        Verification: ${(model.verificationUnitPrice / 100).toFixed(4)}/unit
                      </div>
                      <div>Rate Limit: ${(model.ratelimitUnitPrice / 100).toFixed(4)}/unit</div>
                    </div>
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent>
          ...
          <form onSubmit={createForm.handleSubmit(handleCreate)} className="space-y-4">
            ...
            <FormInput
              label="Rate Limit Price (per unit)"
              type="number"
              step="0.0001"
              min="0"
              placeholder="0.0001"
              {...createForm.register("ratelimitUnitPrice", { valueAsNumber: true })}
              error={createForm.formState.errors.ratelimitUnitPrice?.message}
            />
        ...
        <DialogContent>
          ...
          <form onSubmit={editForm.handleSubmit(handleUpdate)} className="space-y-4">
            ...
            <FormInput
              ...
              min="0"
              placeholder="0.0001"
              {...editForm.register("ratelimitUnitPrice", { valueAsNumber: true })}
              error={editForm.formState.errors.ratelimitUnitPrice?.message}
            />
