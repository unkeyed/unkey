...
describe("createSubscriptionCheckout", () => {
  it.each(["api", "compute"] as const)(
    "uses the shared payment-capable session shape for %s",
    async (product) => {
  ...
  it("uses a different idempotency key when Checkout parameters change", async () => {
  ...
  it("chains a fresh key after a completed session whose subscription is dead", async () => {
    ...
    expect(create.mock.calls[1]?.[1]).toEqual({
      idempotencyKey: `${initialKey}:cs_stale`,
    });
  });
});
