...
// (canonical), falling back to the legacy deploy_plan key.
function subWithItems(
  ...items: Array<{ id?: string; plan?: string; legacyPlan?: string }>
): Stripe.Subscription {
  return {
    id: "sub_test",
    items: {
      data: items.map(({ id, plan, legacyPlan }) => ({
        price: {
          id: id ?? "price_x",
          metadata: {
            ...(plan === undefined ? {} : { plan }),
            ...(legacyPlan === undefined ? {} : { deploy_plan: legacyPlan }),
          },
        },
      })),
    },
...
describe("detectDeployPlan", () => {
  ...
  it("falls back to the legacy deploy_plan metadata key", () => {
    expect(detectDeployPlan(subWithItems({ legacyPlan: "pro" }))).toBe("pro");
  });
