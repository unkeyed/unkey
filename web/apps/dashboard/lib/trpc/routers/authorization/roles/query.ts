      ...
      switch (filter.operator) {
        ...
        case "contains":
          return sql`id IN (
            ...
            JOIN \`keys\` k ON kr.key_id = k.id
            WHERE kr.workspace_id = ${workspaceId}
              AND LOWER(k.name) LIKE LOWER(${`%${value}%`})
          )`;
        case "startsWith":
          return sql`id IN (
            SELECT DISTINCT kr.role_id
      ...
      switch (filter.operator) {
        ...
        case "contains":
          return sql`id IN (
            ...
            JOIN \`keys\` k ON kr.key_id = k.id
            WHERE kr.workspace_id = ${workspaceId}
              AND LOWER(k.id) LIKE LOWER(${`%${value}%`})
          )`;
        case "startsWith":
          return sql`id IN (
            SELECT DISTINCT kr.role_id
...
function buildFilterConditions(
  ...
  const conditions = filters.map((filter) => {
    ...
    switch (filter.operator) {
      ...
      case "contains":
        return sql`LOWER(${sql.identifier(columnName)}) LIKE LOWER(${`%${value}%`})`;
      case "startsWith":
        return sql`LOWER(${sql.identifier(columnName)}) LIKE LOWER(${`${value}%`})`;
      case "endsWith":
        return sql`LOWER(${sql.identifier(columnName)}) LIKE LOWER(${`%${value}`})`;
      ...
      switch (filter.operator) {
        ...
        case "contains":
          return sql`id IN (
            ...
            JOIN permissions p ON rp.permission_id = p.id
            WHERE rp.workspace_id = ${workspaceId}
              AND LOWER(p.name) LIKE LOWER(${`%${value}%`})
          )`;
        case "startsWith":
          return sql`id IN (
            SELECT DISTINCT rp.role_id
      ...
      switch (filter.operator) {
        ...
        case "contains":
          return sql`id IN (
            ...
            JOIN permissions p ON rp.permission_id = p.id
            WHERE rp.workspace_id = ${workspaceId}
              AND LOWER(p.slug) LIKE LOWER(${`%${value}%`})
          )`;
        case "startsWith":
          return sql`id IN (
            SELECT DISTINCT rp.role_id
