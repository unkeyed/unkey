...
export const stopDeployment = workspaceProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    if (deployment.environment?.kind === "production") {
      throw new TRPCError({
        code: "PRECONDITION_FAILED",
        message: "Production deployments cannot be stopped",
      });
    }
    ...
    try {
      ...
      await insertAuditLogs(db, {
        ...
        description: `Stopped deployment ${input.deploymentId} for ${deployment.project.name}`,
        resources: [
          {
            type: "deployment",
            id: deployment.id,
      ...
      console.error("Stop deployment request failed:", error);
      throw new TRPCError({
