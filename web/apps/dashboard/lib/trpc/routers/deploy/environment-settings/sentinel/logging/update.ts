import { loggingPolicySchema } from "@/lib/collections/deploy/sentinel-policies.schema";
...
export const update = workspaceProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    await db.transaction(async (tx) => {
      ...
      if (current[idx].type !== "logging") {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: `Policy ${input.policy.id} is not a logging policy`,
        });
      }
