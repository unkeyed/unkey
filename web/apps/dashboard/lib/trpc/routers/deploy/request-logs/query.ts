...
export const queryRequestLogs = workspaceProcedure
  ...
  .input(requestLogsRequestSchema.omit({ workspaceId: true }))
  .output(
    z.object({
      logs: z.array(requestLogsResponseSchema),
      total: z.number().int(),
    }),
  )
  .query(async ({ ctx, input }) => {
    try {
      ...
      const transformedInputs = transformRequestLogsFilters(input);
      ...
      const { logsQuery, totalQuery } = await clickhouse.frontline.logs({
