...
export async function upsertPermissions(
  ...
  const requestedSlugs = new Map<string, string>();
  for (const permissionString of permissionStrings) {
    const key = permissionString.toLowerCase();
    if (!requestedSlugs.has(key)) {
      requestedSlugs.set(key, permissionString);
    }
  }
  const slugs = Array.from(requestedSlugs.values());
