...
/**
 * Client-side mirrors of the server's portal validators, needed because the
 * generated OpenAPI schema is not a faithful copy of the Go rules: the slug
 * `pattern` cannot express the consecutive-hyphen rejection `pkg/validation/slug.go`
...
// Covers the same rules as `validation.ErrMsgInvalidSlug`, reworded to fit
// under the input.
...
export const portalSlugSchema = z
  ...
  // The regex above matches `a--b`, which the server rejects in a separate rule.
  .refine((slug) => !slug.includes("--"), INVALID_SLUG_MESSAGE);

// Mirrors the spec's `^\S(.*\S)?$`.
...
// host "logo.png", where Go's url.Parse leaves the host empty and rejects it.
// Requiring a non-slash after the scheme keeps the two parsers agreeing.
...
// Empty passes: clearing branding is legal, and the caller maps empty to the
// API's `null` rather than sending `""`, which the server rejects.
