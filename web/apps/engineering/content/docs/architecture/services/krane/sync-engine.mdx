...
The reconciler maintains a `versionLastSeen` field that tracks the highest version successfully processed. On startup, this is zero. After processing each `State` message, the reconciler tracks the max version seen but only commits it after a clean stream close. When reconnecting after a failure, Krane sends its last-seen version in the `SyncRequest`, allowing the control plane to resume from the correct position.

### Message Types

The `State` message contains a version number and one of two payloads:
...
Stream close signals that the current batch (or bootstrap) is complete. The client tracks the highest version from received messages and uses it for the next sync request.
...
for {
    sleep(random(1s, 5s))
    stream = cluster.Sync(region, versionLastSeen)
    maxVersion = 0
    for message in stream {
        ver = reconciler.HandleState(message)
