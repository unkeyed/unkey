...
Virtual Objects provide key-based concurrency control - only one handler can execute at a time per object key. Example: `DeployService` is keyed by `project_id`, ensuring only one deployment per project runs at a time.
...
Handles deployment lifecycle: building containers via Depot, creating regional topologies, polling instance readiness, and assigning frontline routes.
...
Manages atomic frontline route assignments and updates for deployments with per-tenant sentinel isolation.
...
Services auto-register with Restate on startup via `svc/ctrl/worker/run.go`. Config fields (see `svc/ctrl/worker/config.go`):

- `Restate.URL`: Restate ingress endpoint for invoking workflows
...
- `Restate.APIKey`: API key for authenticating with Restate ingress
