...
`Rollback` switches sticky frontline routes from the current live deployment to a previous one. The rolled-back state is derived by comparing the app's live deployment pointer to the latest ready deployment — if they differ, the app is considered rolled back and future deploys will not automatically claim live routes. `Promote` reverses a rollback by updating the live deployment pointer to the target deployment.
