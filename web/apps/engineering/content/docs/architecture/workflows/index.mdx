...
`Rollback` switches sticky frontline routes from the current live deployment to a previous one and sets the app's `is_rolled_back` flag to true. While rolled back, future deploys will not automatically claim live routes. `Promote` reverses a rollback by updating the live deployment pointer to the target deployment and clearing the `is_rolled_back` flag.
