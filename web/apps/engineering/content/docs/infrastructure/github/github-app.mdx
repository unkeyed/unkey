...
Note: Dashboard env vars don't use the `UNKEY_` prefix. Control plane env vars use `UNKEY_` prefix (e.g., `UNKEY_GITHUB_APP_ID`).
...
| Variable                          | Description                             |
| --------------------------------- | --------------------------------------- |
| `UNKEY_GITHUB_WEBHOOK_SECRET`     | Secret for verifying webhook signatures |

**ctrl-worker** (workflow executor):

| Variable                       | Description                  |
| ------------------------------ | ---------------------------- |
| `UNKEY_GITHUB_APP_ID`          | The GitHub App's numeric ID  |
| `UNKEY_GITHUB_APP_PRIVATE_KEY` | The private key (PEM format) |
...
2. **ctrl-api verifies signature** using `X-Hub-Signature-256` header and `UNKEY_GITHUB_WEBHOOK_SECRET`
...
3. Set **Webhook secret** (same value as `UNKEY_GITHUB_WEBHOOK_SECRET`)
