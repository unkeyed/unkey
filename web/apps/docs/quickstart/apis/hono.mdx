...
<Note>
The Hono middleware doesn't need a root key — it verifies keys directly against your API ID.
</Note>
...
app.use("/api/*", unkey({ apiId: process.env.UNKEY_API_ID }));
...
```ts
unkey({
  apiId: "api_...",           // Required: Your API ID
...
<AccordionGroup>
  <Accordion title="Getting 401 even with a valid key?">
    - Check that `UNKEY_API_ID` matches the API where you created the key
  ...
  <Accordion title="Deploying to Cloudflare Workers?">
    Use wrangler secrets for your API ID:
    ```bash
    npx wrangler secret put UNKEY_API_ID
    ```
    Then access via `c.env.UNKEY_API_ID` instead of `process.env`.
  </Accordion>
</AccordionGroup>
