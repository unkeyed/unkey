  ...
  async fetch(req) {
    ...
    if (!result.valid) {
      ...
      return Response.json(
        { error: "Invalid API key", code: result.code },
        { status: 401 }
      );
    }
...
async function authenticate(req: Request) {
  ...
  if (error) return { valid: false, error: "Verification failed" };
  return result;
}
...
<AccordionGroup>
  <Accordion title="Getting 401 even with a valid key?">
    - Check that `UNKEY_API_ID` in `.env` matches the API where you created the key
