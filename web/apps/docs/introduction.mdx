...
Unkey is the API development platform that handles the infrastructure you'd rather not build yourself: **API key management**, **rate limiting**, and **usage tracking** — all with global edge performance and zero servers to manage.
...
| Manage infrastructure globally | 300+ edge locations |
...
<AccordionGroup>
  <Accordion title="Protect a public API" icon="shield-halved">
    ...
    ```ts
    const { result } = await unkey.keys.verify({ key: "sk_123..." });
...
<CardGroup cols={2}>
  <Card
    title="Quickstart"
    icon="rocket"
    href="/quickstart/onboarding/onboarding-api"
  >
    Create your first API and issue a key in under 5 minutes.
  </Card>
  <Card
...
<CardGroup cols={2}>
  ...
  <Card title="I need both" icon="bolt" href="/quickstart/onboarding/onboarding-api">
