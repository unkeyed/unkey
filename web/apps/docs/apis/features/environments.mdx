...
```typescript
const { result } = await verifyKey({ key: request.apiKey });

if (!result.valid) {
  return unauthorized();
}

const environment = result.meta?.environment ?? "production";
...
```typescript
if (result.meta?.environment === "test") {
...
await chargeUser(result.identity.externalId, amount);
return { charged: true, data: realResponse };
```

### Block test keys from production endpoint

```typescript
if (result.meta?.environment === "test" && isProductionEndpoint) {
...
<CardGroup cols={2}>
  <Card title="Key metadata" icon="tag" href="/apis/features/metadata">
    Store additional data on keys
  </Card>
  <Card title="Rate limiting" icon="gauge" href="/apis/features/ratelimiting">
