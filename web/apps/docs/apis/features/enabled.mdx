...
func main() {
	...
	if err != nil {
		log.Fatalf("Failed to disable key: %v", err)
	}

	log.Println("Key disabled successfully")
}
```
...
func main() {
	...
	if err != nil {
		log.Fatalf("Failed to re-enable key: %v", err)
	}

	log.Println("Key re-enabled successfully")
}
```
