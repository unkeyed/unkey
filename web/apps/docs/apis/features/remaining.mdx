...
func main() {
	...
	res, err := client.Keys.CreateKey(ctx, components.V2KeysCreateKeyRequestBody{
		...
		Credits: &components.KeyCreditsData{
			Remaining: func() *int64 { r := int64(1000); return &r }(),
		},
	})
	...
	fmt.Printf("Key ID: %s\n", res.V2KeysCreateKeyResponseBody.KeyID)
}
```
...
func main() {
	...
	res, err := client.Keys.VerifyKey(ctx, components.V2KeysVerifyKeyRequestBody{
		Key: "sk_...",
		Credits: &components.V2KeysVerifyKeyRequestBodyCredits{
			Cost: func() *int64 { c := int64(10); return &c }(),
		},
	})
	...
	if !res.V2KeysVerifyKeyResponseBody.Valid {
		log.Printf("Invalid key: %s", *res.V2KeysVerifyKeyResponseBody.Code)
		return
	}

	log.Printf("Verification successful, credits remaining: %d", *res.V2KeysVerifyKeyResponseBody.Credits)
}
```
...
func main() {
	...
	_, err := client.Keys.UpdateKey(ctx, components.V2KeysUpdateKeyRequestBody{
		KeyID: "key_...",
		Credits: &components.KeyCreditsData{
			Operation: components.OperationIncrement.Pointer(),
			Value: func() *int64 { v := int64(5000); return &v }(),
		},
	})

	if err != nil {
		log.Fatalf("Failed to update credits: %v", err)
	}

	log.Println("Credits updated successfully")
}
```
