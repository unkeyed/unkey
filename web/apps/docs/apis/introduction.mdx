...
export async function handler(req: Request) {
  ...
  // 2. Verify with Unkey
  const { result, error } = await verifyKey({ key, apiId: "api_..." });
  ...
  // 3. Key is valid — result contains useful metadata
  console.log(`Request from: ${result.identity?.externalId}`);
...
| Field | Description |
|-------|-------------|
| `valid` | Whether the key is valid and allowed to make this request |
| `identity.externalId` | The user/entity this key belongs to (if you set it) |
| `meta` | Any custom metadata you attached when creating the key |
| `credits` | How many credits are left (if you set a limit) |
| `ratelimits` | Array of rate limit statuses (remaining, reset time) |
| `permissions` | Permissions attached to this key (for authorization) |
