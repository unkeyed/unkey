...
export function authMiddleware(options: AuthOptions = {}) {
  return async (c: Context, next: Next) => {
    ...
    try {
      const { data, error } = await c.env.UNKEY.keys.verifyKey({
        key: apiKey,
...
export function authMiddleware(options: AuthOptions = {}) {
  return async (c: Context, next: Next) => {
    ...
    try {
      const { meta, data, error } = await c.env.UNKEY.keys.verifyKey({
        key: apiKey,
      });
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      const { meta, data, error } = await env.UNKEY.keys.verifyKey({
        key: apiKey,
      });
...
[vars]
# Non-sensitive config (UNKEY_ROOT_KEY is safe to include)
# UNKEY_ROOT_KEY = "unkey_..."  # Or use secrets for all
