...
export async function verifyAndCheckFeature(
  ...
  if (requiredFeature) {
    const features = (result.meta?.features as string[]) ?? [];
...
function requireFeature(feature: string) {
  return async (req, res, next) => {
    ...
    if (error || !result.valid) {
      return res.status(401).json({ error: result?.code ?? "Unauthorized" });
    }
    ...
    req.user = {
      id: result.ownerId,
      tier: result.meta?.tier as string,
      features,
    };
...
export async function GET(req: NextRequest) {
  ...
  const tier = TIERS[result.meta?.tier as Tier];
  ...
  return NextResponse.json({
    ...
    usage: {
      credits: {
        used: tier.credits ? tier.credits - (result.remaining ?? 0) : null,
        limit: tier.credits,
