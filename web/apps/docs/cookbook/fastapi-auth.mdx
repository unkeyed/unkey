...
async def get_users(auth = Depends(verify_api_key)):
    # Use auth.owner_id to scope data access
    return {"users": []}
```
...
class AuthResult(BaseModel):
    ...
    class Config:
        extra = "allow"  # Allow additional fields from Unkey
```
...
async def verify_api_key(
    ...
    if result.ratelimit:
        response.headers["X-RateLimit-Limit"] = str(result.ratelimit.limit)
        response.headers["X-RateLimit-Remaining"] = str(result.ratelimit.remaining)
        response.headers["X-RateLimit-Reset"] = str(result.ratelimit.reset)
...
```bash
UNKEY_ROOT_KEY=... UNKEY_API_ID=... uvicorn main:app --reload
```
