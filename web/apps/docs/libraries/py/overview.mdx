...
| `code` | `str` | Result code: `VALID`, `NOT_FOUND`, `EXPIRED`, `RATE_LIMITED`, etc. |
| `owner_id` | `str?` | External ID set when creating the key |
| `meta` | `dict?` | Custom metadata attached to the key |
| `remaining` | `int?` | Credits remaining (if using usage limits) |
| `identity` | `dict?` | Identity info if key is linked |
...
async def protected_route(x_api_key: str = Header(...)):
    async with Unkey(root_key=os.environ["UNKEY_ROOT_KEY"]) as unkey:
        ...
        return {"message": "Access granted", "user": result.owner_id}
```
