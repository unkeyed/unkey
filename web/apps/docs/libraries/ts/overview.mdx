...
async function handleRequest(request: Request) {
  ...
  if (!result.valid) {
    ...
    return new Response(`Unauthorized: ${result.code}`, { status: 401 });
  }

  // Key is valid — access granted
  // result.ownerId, result.meta, result.remaining, etc. available
...
| `code` | `string` | Result code: `VALID`, `NOT_FOUND`, `EXPIRED`, `RATE_LIMITED`, etc. |
| `ownerId` | `string?` | The external ID you set when creating the key |
| `meta` | `object?` | Custom metadata attached to the key |
| `remaining` | `number?` | Credits remaining (if using usage limits) |
| `ratelimit` | `object?` | Rate limit state if configured |
| `identity` | `object?` | Identity info if key is linked to one |
...
if (!result.valid && result.code === "INSUFFICIENT_PERMISSIONS") {
...
// Send result.key to your user (only time you'll see the full key!)
