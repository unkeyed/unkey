...
func permKeyspaceURNGrant(action string) string {
	return urn.New().Workspace(permTestWorkspaceID).Project(permTestProjectID).Keyspace(permTestKeyspaceID).String() + "#" + action
}
...
func TestRerollQueryShapeParity(t *testing.T) {
	urnCreate := rbac.U(
		urn.New().Workspace(permTestWorkspaceID).Project(permTestProjectID).Keyspace(permTestKeyspaceID).Key("*"),
