...
func (h *Handler) Handle(ctx context.Context, s *zen.Session) error {
	...
	case req.Prefix != nil:
		prefix = *req.Prefix
	case api.KeyAuth.DefaultPrefix.Valid:
		prefix = api.KeyAuth.DefaultPrefix.String
	}

	byteLength := ptr.SafeDeref(req.ByteLength)
	if byteLength == 0 && api.KeyAuth.DefaultBytes.Valid {
		byteLength = int(api.KeyAuth.DefaultBytes.Int32)
	}
	if byteLength == 0 {
		byteLength = 16
	}
