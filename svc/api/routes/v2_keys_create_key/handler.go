...
func (h *Handler) Handle(ctx context.Context, s *zen.Session) error {
	...
	err = principal.Authorize(rbac.Or(
		...
		rbac.U(
			urn.New().Workspace(principal.WorkspaceID).Project("*").Keyspace(api.KeyAuthID.String),
	...
	if encrypt {
		...
		err = principal.Authorize(rbac.Or(
			...
			rbac.U(urn.New().Workspace(principal.WorkspaceID).Project("*").Keyspace(keySpace.ID).Key("*"), permissions.EncryptKey{}),
		))
		if err != nil {
			return apierrors.MaskInsufficientPermissionsAsNotFound(
				err,
