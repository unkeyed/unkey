...
func TestMigrateKeysWithURNPermission(t *testing.T) {
	h := testutil.NewHarness(t)
	ctx := context.Background()
	...
	res := testutil.CallRoute[handler.Request, handler.Response](h, route, headers, handler.Request{
		...
		Keys: []openapi.V2KeysMigrateKeyData{{
			Hash:    generatedKey.LongTokenHash,
			Credits: &openapi.KeyCreditsData{Remaining: nullable.Nullable[int64]{}},
			Enabled: ptr.P(true),
		}},
	})
	require.Equal(t, 200, res.Status)
	...
	require.Equal(t, generatedKey.LongTokenHash, res.Body.Data.Migrated[0].Hash)
}
