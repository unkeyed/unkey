...
func TestCreateProjectCtrlInternalError(t *testing.T) {
	h := testutil.NewHarness(t)
	const internalMessage = "database credentials rejected"
	route := &handler.Handler{
		CtrlClient: &testutil.MockProjectClient{
			CreateProjectFunc: func(context.Context, *ctrlv1.CreateProjectRequest) (*ctrlv1.CreateProjectResponse, error) {
				return nil, connect.NewError(connect.CodeInternal, errors.New(internalMessage))
			},
		},
	}
	h.Register(route)
	...
	require.Equal(t, "Failed to create project.", res.Body.Error.Detail)
