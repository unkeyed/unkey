...
func (h *Handler) Handle(ctx context.Context, s *zen.Session) error {
	...
	namespaces, missing, err := h.getNamespaces(ctx, auth.WorkspaceID, names)
	if err != nil {
...
func (h *Handler) createNamespaces(ctx context.Context, s *zen.Session, auth *auth.Principal, names []string) (map[string]db.FindRatelimitNamespace, error) {
	...
	for _, name := range names {
		if ns, ok := created[name]; ok {
			h.NamespaceCache.Set(ctx, cache.ScopedKey{WorkspaceID: auth.WorkspaceID, Key: name}, ns)
			h.NamespaceCache.Set(ctx, cache.ScopedKey{WorkspaceID: auth.WorkspaceID, Key: ns.ID}, ns)
		} else {
