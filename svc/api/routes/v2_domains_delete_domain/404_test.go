...
func TestDeleteDomainNotFound(t *testing.T) {
	...
	require.Empty(t, ctrlClient.DeleteCustomDomainCalls,
		"an identifier that misses the lookup must never reach ctrl")
}

// TestDeleteDomainCrossWorkspace pins that the lookup is workspace-scoped. Domains are
// unique per workspace rather than globally, so the same name can exist in two
// workspaces and a root key must only ever delete its own.
func TestDeleteDomainCrossWorkspace(t *testing.T) {
	h := testutil.NewHarness(t)
	ctrlClient := &testutil.MockCustomDomainClient{}
	route := &handler.Handler{DB: h.DB, CtrlClient: ctrlClient}
	h.Register(route)
...
// TestDeleteDomainCtrlNotFoundIsMasked covers the race the advisory read leaves open:
// the row existed on the replica but was gone when ctrl resolved it on the primary,
// which is also what a concurrent double delete looks like. The caller gets the same
// 404 as for a domain that never existed, with the family wording rather than ctrl's
// internal message.
func TestDeleteDomainCtrlNotFoundIsMasked(t *testing.T) {
	h := testutil.NewHarness(t)
	ctrlClient := &testutil.MockCustomDomainClient{
		DeleteCustomDomainFunc: func(_ context.Context, req *ctrlv1.DeleteCustomDomainRequest) (*ctrlv1.DeleteCustomDomainResponse, error) {
