...
func TestStopDeploymentNotFound(t *testing.T) {
	...
	require.Empty(t, mock.StopDeploymentCalls, "ctrl must not be called for unknown deployments")
}

// A key without stop_deployment must not learn whether the deployment exists:
// the handler masks the authorization failure as a 404.
func TestStopDeploymentInsufficientPermissionsMasked(t *testing.T) {
	h := testutil.NewHarness(t)
	mock := &testutil.MockDeploymentClient{}
	route := newRoute(h, mock)
	h.Register(route)
	...
	require.Equal(t, http.StatusNotFound, res.Status, "expected 404, received: %s", res.RawBody)
	require.Empty(t, mock.StopDeploymentCalls, "ctrl must not be called without stop_deployment")
}
...
func TestStopDeploymentInAnotherWorkspace(t *testing.T) {
	h := testutil.NewHarness(t)
	mock := &testutil.MockDeploymentClient{}
	route := newRoute(h, mock)
	h.Register(route)
	...
	require.Equal(t, http.StatusNotFound, res.Status, "expected 404, received: %s", res.RawBody)
	require.Empty(t, mock.StopDeploymentCalls, "ctrl must not be called for cross-workspace deployments")
}
