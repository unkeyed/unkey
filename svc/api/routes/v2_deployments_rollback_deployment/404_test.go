...
func TestRollbackDeploymentNotFound(t *testing.T) {
	...
	require.Empty(t, mock.RollbackCalls, "ctrl must not be called for unknown deployments")
}

// A key without rollback_deployment must not learn whether the deployment
// exists: the handler masks the authorization failure as a 404.
func TestRollbackDeploymentInsufficientPermissionsMasked(t *testing.T) {
	h := testutil.NewHarness(t)
	mock := &testutil.MockDeploymentClient{}
	route := newRoute(h, mock)
	h.Register(route)
	...
	require.Equal(t, http.StatusNotFound, res.Status, "expected 404, received: %s", res.RawBody)
	require.Empty(t, mock.RollbackCalls, "ctrl must not be called without rollback_deployment")
}
...
func TestRollbackDeploymentInAnotherWorkspace(t *testing.T) {
	h := testutil.NewHarness(t)
	mock := &testutil.MockDeploymentClient{}
	route := newRoute(h, mock)
	h.Register(route)
	...
	require.Equal(t, http.StatusNotFound, res.Status, "expected 404, received: %s", res.RawBody)
	require.Empty(t, mock.RollbackCalls, "ctrl must not be called for cross-workspace deployments")
}
