...
func TestGetEnvironment(t *testing.T) {
	...
	t.Run("with default settings", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				require.Nil(t, runtime.Healthcheck, "no healthcheck configured")
				require.Nil(t, runtime.OpenapiSpecPath, "no openapi spec path configured")

				require.NotNil(t, res.Body.Data.Build)
				build := *res.Body.Data.Build
				require.NotNil(t, build.Dockerfile)
				require.Equal(t, "Dockerfile", *build.Dockerfile)
				require.Equal(t, ".", build.RootDirectory)
				require.True(t, build.AutoDeploy)
				require.Nil(t, build.BuildCommand, "no build command configured")
	...
	t.Run("without settings", func(t *testing.T) {
		...
		require.Nil(t, res.Body.Data.Build)
		require.Nil(t, res.Body.Data.Regions)
	})
