...
func (h *Handler) Handle(ctx context.Context, s *zen.Session) error {
	...
	if idempotencyKey != "" {
		ctrlReq.IdempotencyKey = &idempotencyKey
	}
	...
	// The body is identical either way, so the header is the caller's only
	// signal that the key answered with an existing deployment.
	if ctrlResp.GetReplayed() {
		s.AddHeader("Idempotent-Replayed", "true")
	}
