...
func TestEnvironmentNotDeployable(t *testing.T) {
	...
	t.Run("invalid runtime bounds reports every field", func(t *testing.T) {
		...
		require.Contains(t, res.Body.Error.Detail, "cpu must be greater than 0")
		require.Contains(t, res.Body.Error.Detail, "memory must be greater than 0")
		require.NotContains(t, res.Body.Error.Detail, "regions", "a configured region must not be reported")
		require.False(t, capture.called, "ctrl must not be called for an undeployable environment")
	})
}
