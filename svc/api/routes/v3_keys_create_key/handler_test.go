...
func TestCreateKeyStoresVersion1Format(t *testing.T) {
	...
	require.Regexp(
		t,
		regexp.MustCompile(`^prod_sk_[1-9A-HJ-NP-Za-km-z]{8}unkeyv1[1-9A-HJ-NP-Za-km-z]{42}$`),
		res.Body.Data.Key,
	)
...
func TestCreateKeyRejectsInvalidFormatOptions(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "prefix too long", req: map[string]any{"keyspace": api.KeyAuthID.String, "prefix": "prod_key"}},
