...
func (h *Handler) Handle(ctx context.Context, s *zen.Session) error {
	...
	err = db.TxRetry(ctx, h.DB.RW(), func(ctx context.Context, tx db.DBTX) error {
		...
		roleData, findErr := db.Query.FindRoleByID(ctx, tx, role.ID)
		if findErr != nil {
			return fault.Wrap(findErr, fault.Code(codes.App.Internal.ServiceUnavailable.URN()), fault.Internal("unable to find locked role"), fault.Public("Failed to retrieve role."))
		}
		...
		if len(missing) > 0 {
			if authErr := principal.Authorize(rbac.Or(
				rbac.U(urn.New().Workspace(principal.WorkspaceID).Project(roleData.ProjectID).RBAC().Permission("*"), permissions.CreatePermission{}),
