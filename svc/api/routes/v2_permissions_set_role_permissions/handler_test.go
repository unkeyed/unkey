...
func TestSetRolePermissions(t *testing.T) {
	...
	t.Run("replace, deduplicate, create, repeat, and clear", func(t *testing.T) {
		...
		req := roleRequest(role.ID, []string{"keep.permission", "new.permission", "NEW.Permission", "new.permission"})
		...
		require.ElementsMatch(t, []string{"keep.permission", "new.permission", "NEW.Permission"}, []string{assigned[0].Slug, assigned[1].Slug, assigned[2].Slug})
		logs := h.FindAuditLogsByTargetID(ctx, t, role.ID)
		require.Len(t, logs, 3)
		require.ElementsMatch(t, []string{"authorization.disconnect_role_and_permissions", "authorization.connect_role_and_permission", "authorization.connect_role_and_permission"}, []string{logs[0].Event, logs[1].Event, logs[2].Event})
		...
		require.Len(t, repeat.Body.Data, 3)
		require.Len(t, h.FindAuditLogsByTargetID(ctx, t, role.ID), 3, "an idempotent repeat must not add audit records")

		clear := testutil.CallRoute[handler.Request, handler.Response](h, route, headers, roleRequest(role.ID, []string{}))
	...
	t.Run("accepts a dotted role name", func(t *testing.T) {
		role := h.CreateRole(seed.CreateRoleRequest{WorkspaceID: workspace.ID, Name: "service.levels"})
		res := testutil.CallRoute[handler.Request, handler.Response](h, route, headers, roleRequest(role.Name, []string{"service.read"}))
	...
	t.Run("requires add, remove, and create authorization", func(t *testing.T) {
		...
		for i, grants := range cases {
			...
			res := testutil.CallRoute[handler.Request, openapi.ForbiddenErrorResponse](h, route, authHeaders, roleRequest(role.ID, permissions))
	...
	t.Run("malformed requests", func(t *testing.T) {
		for name, req := range map[string]map[string]any{
			"missing role":        {"permissions": []string{}},
			"both role fields":    {"role": "role_missing", "roleId": "role_missing", "permissions": []string{}},
			"missing permissions": {"role": "role_missing"},
			"invalid slug":        {"role": "role_missing", "permissions": []string{""}},
		} {
...
func TestConcurrentMissingPermission(t *testing.T) {
	...
	warmup := testutil.CallRoute[handler.Request, handler.Response](h, route, headers, roleRequest(roles[0].ID, []string{}))
	require.Equal(t, http.StatusOK, warmup.Status, warmup.RawBody)

	g := errgroup.Group{}
	for _, role := range roles {
		g.Go(func() error {
			body, err := json.Marshal(roleRequest(role.ID, []string{"concurrent.permission"}))
			if err != nil {
				return err
			}
