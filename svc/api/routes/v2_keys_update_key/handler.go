...
func (h *Handler) Handle(ctx context.Context, s *zen.Session) error {
	...
	// Name/enabled-only updates have no data-dependent writes after the
	// authorization preflight. Send the mutation, audit outbox insert, and
	// commit as one MySQL multi-statement query. Rich updates retain the regular
	// transaction path below.
	auditPreparer, canPrepareAudit := h.Auditlogs.(auditlogs.OutboxPreparer)
	canBatch := canPrepareAudit &&
		!req.ExternalId.IsSpecified() &&
		!req.Meta.IsSpecified() &&
		!req.Expires.IsSpecified() &&
		!req.Credits.IsSpecified() &&
		req.Permissions == nil &&
		req.Roles == nil &&
		req.Ratelimits == nil
	if canBatch {
		...
		if len(outboxRows) != 1 {
			return fault.New("invalid audit outbox batch size", fault.Internal("update key batch requires exactly one audit outbox row"))
		}

		batchErr := h.DB.BatchRW().UpdateKeyWithAuditBatch(ctx, update, outboxRows[0])
		// The commit outcome is unknown when the terminal marker is lost. Always
		// evict so an enabled key cannot remain cached after a committed disable.
