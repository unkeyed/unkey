...
func TestRemoveEnvironmentVariablesForbidden(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "canonical delete", permissions: []string{fmt.Sprintf("unkey:v1:%s:projects/%s/apps/%s/environments/%s/variables/*#delete", env.workspaceID, env.projectID, env.appID, env.environmentID)}, shouldPass: true},
		{name: "canonical write is not enough", permissions: []string{fmt.Sprintf("unkey:v1:%s:projects/%s/apps/%s/environments/%s/variables/*#write", env.workspaceID, env.projectID, env.appID, env.environmentID)}, shouldPass: false},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			res := testutil.CallRoute[handler.Request, handler.Response](h, route, headers, makeRequest(env, []string{"KEY"}))
			if tc.shouldPass {
