...
func (h *Handler) Path() string {
	return "/v1/billing/connect"
}

func (h *Handler) Handle(ctx context.Context, s *zen.Session) error {
	h.Logger.Debug("handling request", "requestId", s.RequestID(), "path", "/v1/billing/connect")
	...
	err = h.StripeConnect.DisconnectAccount(ctx, auth.AuthorizedWorkspaceID)
	if err != nil {
		return err
	}
	...
	err = h.Auditlogs.Insert(ctx, h.DB.RW(), []auditlog.AuditLog{
		{
			...
			Event:       "billing.stripe_account.disconnected",
			ActorType:   auditlog.RootKeyActor,
			ActorID:     auth.Key.ID,
			ActorName:   "root key",
			ActorMeta:   map[string]any{},
			Display:     "Disconnected Stripe account",
			RemoteIP:    s.Location(),
			UserAgent:   s.UserAgent(),
			Resources: []auditlog.AuditLogResource{
				{
					Type:        "workspace",
					ID:          auth.AuthorizedWorkspaceID,
					DisplayName: auth.AuthorizedWorkspaceID,
					Name:        auth.AuthorizedWorkspaceID,
					Meta:        map[string]any{},
				},
			},
		},
