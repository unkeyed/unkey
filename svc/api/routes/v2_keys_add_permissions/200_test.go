...
func TestSuccess(t *testing.T) {
	...
	t.Run("legacy permissions can add permissions", func(t *testing.T) {
		...
		key := h.CreateKey(seed.CreateKeyRequest{
			...
			Name:        ptr.P("legacy-add-permission-key"),
		})
		permission := h.CreatePermission(seed.CreatePermissionRequest{
			WorkspaceID: workspace.ID,
			Name:        "documents.write.legacy.add",
			Slug:        "documents.write.legacy.add",
		})

		legacyRootKey := h.CreateRootKey(workspace.ID, "api.*.update_key", "rbac.*.add_permission_to_key")
	...
	t.Run("legacy permissions can create a missing permission while adding", func(t *testing.T) {
		api := h.CreateApi(seed.CreateApiRequest{
			WorkspaceID: workspace.ID,
		})
		key := h.CreateKey(seed.CreateKeyRequest{
			WorkspaceID: workspace.ID,
			KeySpaceID:  api.KeyAuthID.String,
			Name:        ptr.P("legacy-create-permission-key"),
		})

		legacyRootKey := h.CreateRootKey(workspace.ID, "api.*.update_key", "rbac.*.add_permission_to_key", "rbac.*.create_permission")
		legacyHeaders := http.Header{
			"Content-Type":  {"application/json"},
			"Authorization": {fmt.Sprintf("Bearer %s", legacyRootKey)},
		}
		...
		res := testutil.CallRoute[handler.Request, handler.Response](h, route, legacyHeaders, handler.Request{
			KeyId:       key.KeyID,
			Permissions: []string{"documents.write.legacy.create"},
		})
		...
		require.NotNil(t, res.Body.Data)
		require.Len(t, res.Body.Data, 1)
		require.Equal(t, "documents.write.legacy.create", res.Body.Data[0].Name)
	})
