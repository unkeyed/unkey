...
func TestWorkspaceRateLimit_ZeroLimit_Returns429(t *testing.T) {
	...
	err := db.Query.UpsertQuota(ctx, h.DB.RW(), db.UpsertQuotaParams{
		...
		LogsRetentionDays:      30,
		Team:                   false,
		RatelimitLimit:         sql.NullInt32{Valid: true, Int32: 0},
		RatelimitDuration:      sql.NullInt32{Valid: true, Int32: 60000},
	})
	require.NoError(t, err)
...
func TestWorkspaceRateLimit_EnforcesLimit(t *testing.T) {
	...
	err := db.Query.UpsertQuota(ctx, h.DB.RW(), db.UpsertQuotaParams{
		...
		LogsRetentionDays:      30,
		Team:                   false,
		RatelimitLimit:         sql.NullInt32{Valid: true, Int32: 2},
		RatelimitDuration:      sql.NullInt32{Valid: true, Int32: 60000},
	})
	require.NoError(t, err)
...
func TestWorkspaceRateLimit_CreatesNamespace(t *testing.T) {
	h := testutil.NewHarness(t)
	ctx := context.Background()
	...
	err := db.Query.UpsertQuota(ctx, h.DB.RW(), db.UpsertQuotaParams{
		...
		LogsRetentionDays:      30,
		Team:                   false,
		RatelimitLimit:         sql.NullInt32{Valid: true, Int32: 100},
		RatelimitDuration:      sql.NullInt32{Valid: true, Int32: 60000},
	})
	require.NoError(t, err)
	...
	// Namespace should be created in the root key's workspace (RootWorkspace),
	// not in the authorized workspace
	row, err := db.Query.FindRatelimitNamespaceByName(ctx, h.DB.RO(), db.FindRatelimitNamespaceByNameParams{
		WorkspaceID: h.Resources().RootWorkspace.ID,
		Name:        "workspace.ratelimit",
	})
	require.NoError(t, err)
	require.Equal(t, "workspace.ratelimit", row.Name)
	require.Equal(t, h.Resources().RootWorkspace.ID, row.WorkspaceID)
}
