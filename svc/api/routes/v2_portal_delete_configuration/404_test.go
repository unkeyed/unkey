...
func TestDeleteConfigurationNotFound(t *testing.T) {
	...
	req := handler.Request{ConfigId: uid.New(uid.PortalConfigPrefix)}
	res := testutil.CallRoute[handler.Request, openapi.NotFoundErrorResponse](h, route, headers, req)
	require.Equal(t, http.StatusNotFound, res.Status, "expected 404, received: %s", res.RawBody)
	require.Equal(t, "Portal configuration not found.", res.Body.Error.Detail)
}

func TestDeleteConfigurationNotFoundWrongWorkspace(t *testing.T) {
	h := testutil.NewHarness(t)
	...
	require.NoError(t, db.Query.InsertPortalConfig(ctx, h.DB.RW(), db.InsertPortalConfigParams{
		...
		Slug:        "cross-ws-delete",
		KeyAuthID:   sql.NullString{Valid: true, String: uid.New(uid.KeySpacePrefix)},
		Enabled:     true,
		CreatedAt:   time.Now().UnixMilli(),
	}))

	// Delete as workspace B, targeting workspace A's config id.
	...
	req := handler.Request{ConfigId: configID}
	res := testutil.CallRoute[handler.Request, openapi.NotFoundErrorResponse](h, route, headers, req)
	require.Equal(t, http.StatusNotFound, res.Status, "expected 404, received: %s", res.RawBody)
	...
	require.Equal(t, "cross-ws-delete", stored.Slug)
}
