...
func TestCreateAppForbidden(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "urn style wildcard project permission", permissions: []string{"unkey:v1:" + workspace.ID + ":projects/*#create_app"}, shouldPass: true},
		{name: "urn style specific project permission", permissions: []string{"unkey:v1:" + workspace.ID + ":projects/" + project.ID + "#create_app"}, shouldPass: true},
		{name: "urn style wrong action", permissions: []string{"unkey:v1:" + workspace.ID + ":projects/*#read_app"}, shouldPass: false},
		{name: "urn style other project", permissions: []string{"unkey:v1:" + workspace.ID + ":projects/" + uid.New(uid.ProjectPrefix) + "#create_app"}, shouldPass: false},
