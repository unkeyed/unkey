...
func TestMapPolicyFromProtoVariants(t *testing.T) {
	t.Run("keyauth with all fields", func(t *testing.T) {
		...
		require.Equal(t, new("documents.read"), got.Keyauth.PermissionQuery)
		require.Equal(t, new(int64(0)), got.Keyauth.Credits)
...
func TestMapMatchExprFromProto(t *testing.T) {
	t.Run("path with ignoreCase", func(t *testing.T) {
		...
		require.Equal(t, new("/internal/"), got.Path.Path.Prefix)
		require.Equal(t, new(true), got.Path.Path.IgnoreCase)
	...
	t.Run("ignoreCase false is omitted", func(t *testing.T) {
		...
		require.Equal(t, new("/health"), got.Path.Path.Exact)
	})
	...
	t.Run("regex", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, new("^/v[0-9]+/"), got.Path.Path.Regex)
	})
	...
	t.Run("header present", func(t *testing.T) {
		...
		require.Equal(t, new(openapi.FieldMatchPresent(true)), got.Header.Present)
	...
	t.Run("header value", func(t *testing.T) {
		...
		require.Equal(t, new("KEBAP"), got.Header.Value.Exact)
	})
	...
	t.Run("queryParam value", func(t *testing.T) {
		...
		require.Equal(t, new("1"), got.QueryParam.Value.Exact)
	})
