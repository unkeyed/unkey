...
// TestHTTPStatus_Mapping locks in the URN to HTTP status mapping for the api
// service. Most rows resolve via pkg/codes (Code.HTTPStatus); a few resolve via
// apiStatusOverrides (the api treats missing/malformed auth as 400 Bad Request
// rather than 401 for backwards compatibility).
//
...
func TestHTTPStatus_Mapping(t *testing.T) {
	...
	tests := []struct {
		...
		// Inherited from pkg/codes (CategoryUserBadRequest default -> 400).
		...
		// CategoryUnkeyData has no category default; each code maps via a
		// per-code override.
...
// TestBuildErrorBody_PicksRightShape verifies that the body builder returns the
// correct openapi response type for each status. Drift here would mean clients
// receive a body shape that doesn't match the status code.
...
func TestErrorTitle(t *testing.T) {
	...
	// 499 has no stdlib reason phrase, so it must come from the override map.
	...
	// The standard reason phrase kicks in when no override exists.
