...
type V2DomainsCreateDomainResponseData struct {
	...
	DomainConnectProvider *string `json:"domainConnectProvider,omitempty"`

	// DomainConnectUrl Signed Domain Connect URL that pre-fills this domain's required DNS records at the provider. Open it
	// in a browser and the domain owner approves the records in one step instead of entering them by hand.
	// The URL is signed with an Unkey key, so it cannot be constructed or altered by the caller.
	//
	...
	DomainConnectUrl *string `json:"domainConnectUrl,omitempty"`

	// DomainId The unique identifier of the newly created domain, generated by Unkey.
	// Always begins with 'dom_' followed by a unique alphanumeric sequence.
	DomainId string `json:"domainId"`

	// TargetCname The CNAME target for this domain. Point a CNAME record for the requested domain at this value.
	// The value is unique per domain and is the fastest path to verification.
	TargetCname string `json:"targetCname"`

	// VerificationToken Ownership proof token. Publish it as a TXT record on '_unkey.<domain>' with the value
	// 'unkey-domain-verify=<verificationToken>'. Required when the CNAME record cannot be used, which is
	// the case for apex domains and for any domain another workspace has already verified.
