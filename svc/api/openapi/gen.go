...
const (
	DeploymentDomainStatusActive  DeploymentDomainStatus = "active"
	DeploymentDomainStatusFailed  DeploymentDomainStatus = "failed"
	DeploymentDomainStatusPending DeploymentDomainStatus = "pending"
)

// Defines values for DeploymentDomainType.
const (
	DeploymentDomainTypeCustom DeploymentDomainType = "custom"
	DeploymentDomainTypeSystem DeploymentDomainType = "system"
)

// Defines values for DeploymentFailureCode.
const (
	DeploymentFailureCodeBuildFailed            DeploymentFailureCode = "build_failed"
	DeploymentFailureCodeCpuQuotaExceeded       DeploymentFailureCode = "cpu_quota_exceeded"
	DeploymentFailureCodeInvalidRuntimeSettings DeploymentFailureCode = "invalid_runtime_settings"
	DeploymentFailureCodeMemoryQuotaExceeded    DeploymentFailureCode = "memory_quota_exceeded"
	DeploymentFailureCodeNoSchedulableRegions   DeploymentFailureCode = "no_schedulable_regions"
	DeploymentFailureCodeStorageQuotaExceeded   DeploymentFailureCode = "storage_quota_exceeded"
	DeploymentFailureCodeUnknown                DeploymentFailureCode = "unknown"
)
...
type Deployment struct {
	// AppId The app this deployment belongs to.
	AppId string `json:"appId"`

	// AvailableActions Lifecycle operations that are legal to call on this deployment right now.
	...
	// Domains Hostnames this deployment is reachable at. Only returned by getDeployment.
	Domains *[]DeploymentDomain `json:"domains,omitempty"`

	// EnvironmentId The environment this deployment belongs to.
	...
	// IsCurrent True when the deployment's app currently routes traffic to it. A deployment
	// can be `ready` without being current, for example one that was superseded or
	// not yet promoted.
...
// state, given its status, desiredState, environment, and whether it is the
// current deployment.
...
type DeploymentDomain struct {
	// Domain Fully qualified domain name this deployment is reachable at.
	Domain string `json:"domain"`

	// Status `active` serves traffic. `pending` is a custom domain awaiting
	// verification. `failed` is a custom domain whose verification failed.
	Status DeploymentDomainStatus `json:"status"`

	// Type `system` is an Unkey-managed hostname (always active). `custom` is a
	// customer-owned domain whose `status` reflects verification.
	Type DeploymentDomainType `json:"type"`
}

// DeploymentDomainStatus `active` serves traffic. `pending` is a custom domain awaiting
// verification. `failed` is a custom domain whose verification failed.
type DeploymentDomainStatus string

// DeploymentDomainType `system` is an Unkey-managed hostname (always active). `custom` is a
// customer-owned domain whose `status` reflects verification.
type DeploymentDomainType string

// DeploymentFailure defines model for DeploymentFailure.
type DeploymentFailure struct {
	...
	Code DeploymentFailureCode `json:"code"`
...
// DeploymentFailureCode The reason a deployment failed. `unknown` means Unkey could not classify the
// failure; see `message` for details.
type DeploymentFailureCode string
