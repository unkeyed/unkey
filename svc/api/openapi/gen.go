...
// PortalBranding Optional branding applied to the Customer Portal for a configuration.
type PortalBranding struct {
	// LogoUrl URL of the logo displayed in the portal header.
	LogoUrl string `json:"logoUrl,omitempty"`

	// PrimaryColor Primary accent color for the portal, as a 6-digit hex color.
	PrimaryColor string `json:"primaryColor,omitempty"`
}

// PortalConfiguration A Customer Portal configuration. Maps to exactly one keyspace or one app and
// scopes the portal experience for a workspace's end users.
type PortalConfiguration struct {
	// AppId The app this configuration maps to. Present when the configuration is
	...
	// Id The unique identifier of the portal configuration, generated by Unkey.
	// Always begins with 'pc_'.
	Id string `json:"id"`

	// KeyspaceId The keyspace this configuration maps to. Present when the configuration is
	// keyspace-mapped; omitted when it is app-mapped.
	KeyspaceId string `json:"keyspaceId,omitempty"`

	// ReturnUrl URL the end user is returned to when they exit the portal.
	ReturnUrl string `json:"returnUrl,omitempty"`

	// Slug Human-readable slug, unique within the workspace. Used by
	// `portal.createSession` to select the configuration.
	Slug string `json:"slug"`

	// UpdatedAt Unix timestamp in milliseconds when the configuration was last updated.
...
// V2PortalCreateConfigurationRequestBody defines model for V2PortalCreateConfigurationRequestBody.
type V2PortalCreateConfigurationRequestBody struct {
	// AppId The app the configuration maps to. Provide exactly one of `keyspaceId` or
	// `appId`.
	AppId *string `json:"appId,omitempty"`
	...
	// KeyspaceId The keyspace the configuration maps to. Provide exactly one of
	// `keyspaceId` or `appId`.
	KeyspaceId *string `json:"keyspaceId,omitempty"`
	...
	// Slug Human-readable slug for the configuration, unique within the workspace.
...
// V2PortalCreateConfigurationResponseBody defines model for V2PortalCreateConfigurationResponseBody.
type V2PortalCreateConfigurationResponseBody struct {
	// Data A Customer Portal configuration. Maps to exactly one keyspace or one app and
	// scopes the portal experience for a workspace's end users.
...
// V2PortalDeleteConfigurationRequestBody defines model for V2PortalDeleteConfigurationRequestBody.
type V2PortalDeleteConfigurationRequestBody struct {
	// ConfigId The identifier of the portal configuration to delete. Always begins with
	// 'pc_'.
	ConfigId string `json:"configId"`
}

// V2PortalDeleteConfigurationResponseBody defines model for V2PortalDeleteConfigurationResponseBody.
type V2PortalDeleteConfigurationResponseBody struct {
	// Data Empty response object by design. A successful response indicates this operation was successfully executed.
	Data EmptyResponse `json:"data"`
...
// V2PortalUpdateConfigurationRequestBody defines model for V2PortalUpdateConfigurationRequestBody.
type V2PortalUpdateConfigurationRequestBody struct {
	// AppId The app the configuration maps to. Provide exactly one of `keyspaceId` or
	...
	// ReturnUrl URL the end user is returned to when they exit the portal. Omit to clear.
	ReturnUrl *string `json:"returnUrl,omitempty"`

	// Slug Human-readable slug, unique within the workspace. Must be 3-64 characters,
	// lowercase alphanumeric and hyphens only, must not start or end with a
	// hyphen, and must not contain consecutive hyphens.
	Slug string `json:"slug"`
}

// V2PortalUpdateConfigurationResponseBody defines model for V2PortalUpdateConfigurationResponseBody.
type V2PortalUpdateConfigurationResponseBody struct {
	// Data A Customer Portal configuration. Maps to exactly one keyspace or one app and
	// scopes the portal experience for a workspace's end users.
	Data PortalConfiguration `json:"data"`
...
// PortalCreateConfigurationJSONRequestBody defines body for PortalCreateConfiguration for application/json ContentType.
type PortalCreateConfigurationJSONRequestBody = V2PortalCreateConfigurationRequestBody
...
// PortalDeleteConfigurationJSONRequestBody defines body for PortalDeleteConfiguration for application/json ContentType.
type PortalDeleteConfigurationJSONRequestBody = V2PortalDeleteConfigurationRequestBody
...
// PortalUpdateConfigurationJSONRequestBody defines body for PortalUpdateConfiguration for application/json ContentType.
type PortalUpdateConfigurationJSONRequestBody = V2PortalUpdateConfigurationRequestBody
