...
type mockQuerier struct {
	...
	findInstances   func(ctx context.Context, deploymentID string) ([]db.FindInstancesByDeploymentIDRow, error)
}
...
func (m *mockQuerier) FindInstancesByDeploymentID(ctx context.Context, deploymentID string) ([]db.FindInstancesByDeploymentIDRow, error) {
...
func newTestRouter(t *testing.T, mock *mockQuerier) *service {
	t.Helper()

	clk := clock.New()

	routeCache, err := cache.New(cache.Config[string, db.FindFrontlineRouteByFQDNRow]{
		Fresh:    time.Minute,
		Stale:    5 * time.Minute,
		MaxSize:  100,
	...
	instanceCache, err := cache.New(cache.Config[string, []db.FindInstancesByDeploymentIDRow]{
		Fresh:    time.Minute,
		Stale:    5 * time.Minute,
		MaxSize:  100,
...
func TestRoute_DeploymentRouteUseSentinel(t *testing.T) {
	t.Parallel()
	...
	mock := &mockQuerier{
		...
		findInstances: func(_ context.Context, _ string) ([]db.FindInstancesByDeploymentIDRow, error) {
			return []db.FindInstancesByDeploymentIDRow{
