...
func Run(ctx context.Context, cfg Config) error {
	...
	// Prometheus /metrics endpoint. heimdall's metric defs live in
	// pkg/metrics using promauto, which registers into the default
	// registry — so prometheus.New() (which wraps the default registerer)
	// picks them up without any extra wiring. Gated on a non-zero port
	// via [observability.metrics] prometheus_port; omit the section in
	// dev to run without a scraper.
	//
	// hostNetwork is true in k8s, so the port we pick has to coexist
	// with node-exporter (9100) and anything else the node runs. The
	// chart defaults to 9402.
	if cfg.Observability.Metrics != nil && cfg.Observability.Metrics.PrometheusPort > 0 {
		prom, promErr := prometheus.New()
		if promErr != nil {
			return fmt.Errorf("unable to build prometheus server: %w", promErr)
		}

		addr := fmt.Sprintf(":%d", cfg.Observability.Metrics.PrometheusPort)
		promListener, listenErr := net.Listen("tcp", addr)
		if listenErr != nil {
