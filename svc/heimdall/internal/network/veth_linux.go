...
func hostVethIfindex(netnsPath string) (uint32, error) {
	...
	if err != nil {
		logger.Debug("hostVethIfindex: open /proc/self/ns/net failed", "error", err.Error())
	...
	if err != nil {
		logger.Debug("hostVethIfindex: open pod netns failed", "netns_path", netnsPath, "error", err.Error())
	...
	if err := unix.Setns(int(podNs.Fd()), unix.CLONE_NEWNET); err != nil {
		logger.Debug("hostVethIfindex: setns into pod netns failed", "netns_path", netnsPath, "errno", err.Error())
	...
	if err := unix.Setns(int(origNs.Fd()), unix.CLONE_NEWNET); err != nil {
		logger.Debug("hostVethIfindex: setns back to host netns failed — leaking OS thread", "netns_path", netnsPath, "errno", err.Error())
...
func findVethPeerIfindex() (uint32, error) {
	...
	if err != nil {
		logger.Debug("findVethPeerIfindex: netlink link list failed", "error", err.Error())
	...
	logger.Debug("findVethPeerIfindex: no veth in pod netns", "links_seen", seen)
