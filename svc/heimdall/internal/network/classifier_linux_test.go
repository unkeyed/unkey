...
//
// Requires CAP_BPF (or root) + a Linux kernel >= 5.8 to load tc/TCX-era
...
func loadTestObjects(t *testing.T) *bpfObjects {
	...
	if err := loadBpfObjects(objs, opts); err != nil {
		...
		// stale-binding regressions actually fail the test. EPERM/EACCES =
		// missing CAP_BPF; ENOSYS / EINVAL = kernel too old or BPF disabled
		// at config-time. Anything else is a real bug in the program.
		if isEnvSkipError(err) {
			t.Skipf("cannot load BPF objects (kernel or privileges missing): %v", err)
		}
		t.Fatalf("loadBpfObjects failed (verifier or binding regression): %v", err)
	}
...
// readCounters fetches the sole map entry created by BPF_PROG_TEST_RUN.
// The program keys the map by POD_KEY (a rodata constant baked in at load
// time; the test harness leaves it at its default 0). Each test loads a
// fresh map and runs a single program pair, so there is at most one entry;
// iterate to find it.
func readCounters(t *testing.T, m *ebpf.Map) bpfCounters {
	...
	var c bpfCounters
	it := m.Iterate()
	require.True(t, it.Next(&key, &c), "expected exactly one entry in counter map")
	require.NoError(t, it.Err(), "map iterator")
	return c
}
...
func TestClassifier_V4_Multicast(t *testing.T) {
	// 224.0.0.0/4 is multicast — not billable public egress.
	objs := loadTestObjects(t)
	frame := buildIPv4Frame(t,
		net.IPv4(10, 244, 0, 5),
		net.IPv4(239, 255, 255, 250), // SSDP
		500,
	)
	...
	c := readCounters(t, objs.PodCounters)
	require.EqualValues(t, 500, c.EgressPrivate, "224.0.0.0/4 is multicast (private)")
}

func TestClassifier_V4_LimitedBroadcast(t *testing.T) {
	// 255.255.255.255 is the limited broadcast address — never billable.
	objs := loadTestObjects(t)
	frame := buildIPv4Frame(t,
		net.IPv4(10, 244, 0, 5),
	...
	c := readCounters(t, objs.PodCounters)
	require.EqualValues(t, 500, c.EgressPrivate, "255.255.255.255 is limited broadcast (private)")
}

func TestClassifier_V4_ThisNetwork(t *testing.T) {
	// 0.0.0.0/8 is "this network" — shows up as DHCP source, never a
	...
	c := readCounters(t, objs.PodCounters)
	require.EqualValues(t, 500, c.EgressPrivate, "0.0.0.0/8 is 'this network' (private)")
}
...
func TestClassifier_NonIP(t *testing.T) {
	...
	// Non-IP packets must not trigger lookup_or_init, so the map should
	// be empty. Iterate to confirm — no entry means no classification
	// happened.
	var key uint64
	var c bpfCounters
	it := objs.PodCounters.Iterate()
	require.False(t, it.Next(&key, &c), "non-IP frame must not create a map entry")
	require.NoError(t, it.Err(), "map iterator")
}
...
// bpffsTempDir returns a unique directory under /sys/fs/bpf for test pins.
// LIBBPF_PIN_BY_NAME requires the PinPath to be on a real bpffs; t.TempDir()
// is on tmpfs/ext4 and the kernel rejects pin syscalls there with
// "is not on a bpf filesystem". Tests on hosts without bpffs (macOS dev,
// CI runners without a bpffs mount) get skipped instead of failing.
func bpffsTempDir(t *testing.T) string {
	t.Helper()
	const root = "/sys/fs/bpf"
	if _, err := os.Stat(root); err != nil {
		t.Skipf("no bpffs at %s (host lacks BPF filesystem): %v", root, err)
	}
	dir, err := os.MkdirTemp(root, "heimdall-test-*")
	if err != nil {
		if isEnvSkipError(err) {
			t.Skipf("cannot create dir under %s (likely missing CAP_SYS_ADMIN): %v", root, err)
		}
		t.Fatalf("MkdirTemp under %s failed: %v", root, err)
	}
	t.Cleanup(func() { _ = os.RemoveAll(dir) })
	return dir
}

// isEnvSkipError returns true for errors that mean "this runner cannot
// load BPF programs" (missing capability, kernel too old, BPF disabled
// in the kernel config). Verifier rejections, stale generated bindings,
// and missing-symbol errors are NOT in this set — those are real
// regressions and the test should fail loud, not green-skip.
func isEnvSkipError(err error) bool {
	if err == nil {
		return false
	}
