...
// layer, and the host-side veth is bypassed by Cilium's bpf_redirect_peer.
//
...
//   - veth_linux.go         - setns into pod netns, find eth0, read netns cookie, clone spec + bake POD_KEY, load per-pod programs, attach both TCX programs
