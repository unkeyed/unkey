...
// __sk_buff is the BPF program's context (also used by tc/TCX programs).
// Hundreds of fields exist; we only touch `data`, `data_end`, and `len`.
// Using `__u32` for the pointer fields matches the kernel's userspace-safe
// layout.
struct __sk_buff {
    __u32 len;
    __u32 pkt_type;
