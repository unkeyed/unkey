...
// torn down). If cpu.stat succeeds but a memory file is *missing* (the
// narrow race window of partial teardown), returns the CPU reading with
// memoryBytes=0. Any other memory read/parse error is returned to the
// caller — masking those would silently under-report memory if the parser
// or the kernel format ever drifted.
func (r *cgroupReader) read(uid types.UID, qos corev1.PodQOSClass) (cgroupReading, error) {
	...
	// Only swallow ErrNotExist (file disappeared between reads — partial
	// teardown); real parse/IO errors propagate so corrupt counters or
	// parser regressions don't silently under-report memory as zeros.
	...
	if err != nil {
		...
		return cgroupReading{}, fmt.Errorf("reading memory.current: %w", err)
	}
	...
	if err != nil {
		...
		return cgroupReading{}, fmt.Errorf("reading memory.stat: %w", err)
	}
