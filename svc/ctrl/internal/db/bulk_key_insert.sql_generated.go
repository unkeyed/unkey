...
const bulkInsertKey = `INSERT INTO ` + "`" + `keys` + "`" + ` ( id, key_auth_id, hash, prefix, start, end, workspace_id, for_workspace_id, name, identity_id, meta, expires, created_at_m, enabled, remaining_requests, refill_day, refill_amount, pending_migration_id ) VALUES %s`

// InsertKeys performs bulk insert in a single query

func (q *BulkQueries) InsertKeys(ctx context.Context, args []InsertKeyParams) error {
	...
	for _, arg := range args {
		...
		allArgs = append(allArgs, arg.End)
