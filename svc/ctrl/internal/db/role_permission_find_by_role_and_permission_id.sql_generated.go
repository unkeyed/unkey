// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: role_permission_find_by_role_and_permission_id.sql

package db

import (
	"context"
)

const findRolePermissionByRoleAndPermissionID = `-- name: FindRolePermissionByRoleAndPermissionID :many
SELECT pk, role_id, permission_id, workspace_id, created_at_m, updated_at_m
FROM roles_permissions
WHERE role_id = ?
  AND permission_id = ?
`

type FindRolePermissionByRoleAndPermissionIDParams struct {
	RoleID       string `db:"role_id"`
	PermissionID string `db:"permission_id"`
}

// FindRolePermissionByRoleAndPermissionID
//
//	SELECT pk, role_id, permission_id, workspace_id, created_at_m, updated_at_m
//	FROM roles_permissions
//	WHERE role_id = ?
//	  AND permission_id = ?
func (q *Queries) FindRolePermissionByRoleAndPermissionID(ctx context.Context, arg FindRolePermissionByRoleAndPermissionIDParams) ([]RolesPermission, error) {
	rows, err := q.db.QueryContext(ctx, findRolePermissionByRoleAndPermissionID, arg.RoleID, arg.PermissionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []RolesPermission
	for rows.Next() {
		var i RolesPermission
		if err := rows.Scan(
			&i.Pk,
			&i.RoleID,
			&i.PermissionID,
			&i.WorkspaceID,
			&i.CreatedAtM,
			&i.UpdatedAtM,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
