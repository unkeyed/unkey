// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: key_space_update_key_encryption.sql

package db

import (
	"context"
)

const updateKeySpaceKeyEncryption = `-- name: UpdateKeySpaceKeyEncryption :exec
UPDATE ` + "`" + `key_auth` + "`" + ` SET store_encrypted_keys = ? WHERE id = ?
`

type UpdateKeySpaceKeyEncryptionParams struct {
	StoreEncryptedKeys bool   `db:"store_encrypted_keys"`
	ID                 string `db:"id"`
}

// UpdateKeySpaceKeyEncryption
//
//	UPDATE `key_auth` SET store_encrypted_keys = ? WHERE id = ?
func (q *Queries) UpdateKeySpaceKeyEncryption(ctx context.Context, arg UpdateKeySpaceKeyEncryptionParams) error {
	_, err := q.db.ExecContext(ctx, updateKeySpaceKeyEncryption, arg.StoreEncryptedKeys, arg.ID)
	return err
}
