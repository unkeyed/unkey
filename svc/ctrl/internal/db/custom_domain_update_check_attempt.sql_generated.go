// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: custom_domain_update_check_attempt.sql

package db

import (
	"context"
	"database/sql"
)

const updateCustomDomainCheckAttempt = `-- name: UpdateCustomDomainCheckAttempt :exec
UPDATE custom_domains
SET check_attempts = ?,
    last_checked_at = ?,
    updated_at = ?
WHERE id = ?
`

type UpdateCustomDomainCheckAttemptParams struct {
	CheckAttempts int32         `db:"check_attempts"`
	LastCheckedAt sql.NullInt64 `db:"last_checked_at"`
	UpdatedAt     sql.NullInt64 `db:"updated_at"`
	ID            string        `db:"id"`
}

// UpdateCustomDomainCheckAttempt
//
//	UPDATE custom_domains
//	SET check_attempts = ?,
//	    last_checked_at = ?,
//	    updated_at = ?
//	WHERE id = ?
func (q *Queries) UpdateCustomDomainCheckAttempt(ctx context.Context, arg UpdateCustomDomainCheckAttemptParams) error {
	_, err := q.db.ExecContext(ctx, updateCustomDomainCheckAttempt,
		arg.CheckAttempts,
		arg.LastCheckedAt,
		arg.UpdatedAt,
		arg.ID,
	)
	return err
}
