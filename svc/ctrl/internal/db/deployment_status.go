...
// The deployment lifecycle enums live in pkg/mysql/types so this package and
// pkg/db share one type instead of each generating its own copy (which forced
// casting at every boundary). The sqlc go_type override points the generated
// deployments.status/desired_state columns at that package; these aliases and
// re-exports keep the db.DeploymentsStatus* call sites unchanged. See
...
var (
	...
	ActiveComputeDeploymentStatuses = mysqltype.ActiveComputeDeploymentStatuses
