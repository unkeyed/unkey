// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: key_update_credits_refill.sql

package db

import (
	"context"
	"database/sql"
)

const updateKeyCreditsRefill = `-- name: UpdateKeyCreditsRefill :exec
UPDATE ` + "`" + `keys` + "`" + ` SET refill_amount = ?, refill_day = ? WHERE id = ?
`

type UpdateKeyCreditsRefillParams struct {
	RefillAmount sql.NullInt64 `db:"refill_amount"`
	RefillDay    sql.NullInt16 `db:"refill_day"`
	ID           string        `db:"id"`
}

// UpdateKeyCreditsRefill
//
//	UPDATE `keys` SET refill_amount = ?, refill_day = ? WHERE id = ?
func (q *Queries) UpdateKeyCreditsRefill(ctx context.Context, arg UpdateKeyCreditsRefillParams) error {
	_, err := q.db.ExecContext(ctx, updateKeyCreditsRefill, arg.RefillAmount, arg.RefillDay, arg.ID)
	return err
}
