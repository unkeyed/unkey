// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: project_list_by_workspace.sql

package db

import (
	"context"
	"database/sql"
)

const listProjectsByWorkspaceId = `-- name: ListProjectsByWorkspaceId :many
SELECT
    id,
    workspace_id,
    name,
    slug,
    delete_protection,
    created_at,
    updated_at
FROM projects
WHERE workspace_id = ?
  AND id >= ?
ORDER BY id ASC
LIMIT ?
`

type ListProjectsByWorkspaceIdParams struct {
	WorkspaceID string `db:"workspace_id"`
	IDCursor    string `db:"id_cursor"`
	Limit       int32  `db:"limit"`
}

type ListProjectsByWorkspaceIdRow struct {
	ID               string        `db:"id"`
	WorkspaceID      string        `db:"workspace_id"`
	Name             string        `db:"name"`
	Slug             string        `db:"slug"`
	DeleteProtection sql.NullBool  `db:"delete_protection"`
	CreatedAt        int64         `db:"created_at"`
	UpdatedAt        sql.NullInt64 `db:"updated_at"`
}

// ListProjectsByWorkspaceId
//
//	SELECT
//	    id,
//	    workspace_id,
//	    name,
//	    slug,
//	    delete_protection,
//	    created_at,
//	    updated_at
//	FROM projects
//	WHERE workspace_id = ?
//	  AND id >= ?
//	ORDER BY id ASC
//	LIMIT ?
func (q *Queries) ListProjectsByWorkspaceId(ctx context.Context, arg ListProjectsByWorkspaceIdParams) ([]ListProjectsByWorkspaceIdRow, error) {
	rows, err := q.db.QueryContext(ctx, listProjectsByWorkspaceId, arg.WorkspaceID, arg.IDCursor, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListProjectsByWorkspaceIdRow
	for rows.Next() {
		var i ListProjectsByWorkspaceIdRow
		if err := rows.Scan(
			&i.ID,
			&i.WorkspaceID,
			&i.Name,
			&i.Slug,
			&i.DeleteProtection,
			&i.CreatedAt,
			&i.UpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
