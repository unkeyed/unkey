// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: sentinel_list_desired.sql

package db

import (
	"context"
)

const listDesiredSentinels = `-- name: ListDesiredSentinels :many
SELECT pk, id, workspace_id, project_id, environment_id, k8s_name, k8s_address, region_id, image, running_image, desired_state, health, desired_replicas, available_replicas, deploy_status, cpu_millicores, memory_mib, created_at, updated_at
FROM ` + "`" + `sentinels` + "`" + `
WHERE (? = '' OR region_id = ?)
    AND desired_state = ?
    AND id > ?
ORDER BY id ASC
LIMIT ?
`

type ListDesiredSentinelsParams struct {
	RegionID         string                `db:"region_id"`
	DesiredState     SentinelsDesiredState `db:"desired_state"`
	PaginationCursor string                `db:"pagination_cursor"`
	Limit            int32                 `db:"limit"`
}

// ListDesiredSentinels returns all sentinels matching the desired state for a region.
// Used during bootstrap to stream all running sentinels to krane.
//
//	SELECT pk, id, workspace_id, project_id, environment_id, k8s_name, k8s_address, region_id, image, running_image, desired_state, health, desired_replicas, available_replicas, deploy_status, cpu_millicores, memory_mib, created_at, updated_at
//	FROM `sentinels`
//	WHERE (? = '' OR region_id = ?)
//	    AND desired_state = ?
//	    AND id > ?
//	ORDER BY id ASC
//	LIMIT ?
func (q *Queries) ListDesiredSentinels(ctx context.Context, arg ListDesiredSentinelsParams) ([]Sentinel, error) {
	rows, err := q.db.QueryContext(ctx, listDesiredSentinels,
		arg.RegionID,
		arg.RegionID,
		arg.DesiredState,
		arg.PaginationCursor,
		arg.Limit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Sentinel
	for rows.Next() {
		var i Sentinel
		if err := rows.Scan(
			&i.Pk,
			&i.ID,
			&i.WorkspaceID,
			&i.ProjectID,
			&i.EnvironmentID,
			&i.K8sName,
			&i.K8sAddress,
			&i.RegionID,
			&i.Image,
			&i.RunningImage,
			&i.DesiredState,
			&i.Health,
			&i.DesiredReplicas,
			&i.AvailableReplicas,
			&i.DeployStatus,
			&i.CpuMillicores,
			&i.MemoryMib,
			&i.CreatedAt,
			&i.UpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
