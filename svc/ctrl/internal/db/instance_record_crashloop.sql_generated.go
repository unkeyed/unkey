...
//		AND region_id = ?
//		AND CAST(JSON_VALUE(container_status, '$.restartCount') AS UNSIGNED) <= CAST(? AS UNSIGNED)
func (q *Queries) RecordInstanceCrashLoopBackOff(ctx context.Context, arg RecordInstanceCrashLoopBackOffParams) error {
	_, err := q.db.ExecContext(ctx, recordInstanceCrashLoopBackOff, arg.K8sName, arg.RegionID, arg.RestartCount)
	return err
}
