...
const bulkInsertDeployment = `INSERT INTO ` + "`" + `deployments` + "`" + ` ( id, k8s_name, workspace_id, project_id, app_id, environment_id, git_commit_sha, git_branch, sentinel_config, git_commit_message, git_commit_author_handle, git_commit_author_avatar_url, git_commit_timestamp, encrypted_environment_variables, command, status, cpu_millicores, memory_mib, storage_mib, port, shutdown_signal, upstream_protocol, healthcheck, pr_number, fork_repository_full_name, image, ` + "`" + `trigger` + "`" + `, triggered_by, trigger_reason, created_at, updated_at ) VALUES %s`

// InsertDeployments performs bulk insert in a single query

func (q *BulkQueries) InsertDeployments(ctx context.Context, args []InsertDeploymentParams) error {
	...
	for _, arg := range args {
		...
		allArgs = append(allArgs, arg.EncryptedEnvironmentVariables)
		...
		allArgs = append(allArgs, arg.PrNumber)
		allArgs = append(allArgs, arg.ForkRepositoryFullName)
		allArgs = append(allArgs, arg.Image)
		allArgs = append(allArgs, arg.DeploymentTrigger)
		allArgs = append(allArgs, arg.TriggeredBy)
		allArgs = append(allArgs, arg.TriggerReason)
