...
//	WHERE kp.key_id = ?
//	ORDER BY p.slug
func (q *Queries) ListDirectPermissionsByKeyID(ctx context.Context, keyID string) ([]Permission, error) {
	rows, err := q.db.QueryContext(ctx, listDirectPermissionsByKeyID, keyID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
