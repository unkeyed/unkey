...
//	SET status = ?, token = ?, authorization = ?, updated_at = ?
//	WHERE domain_id = ?
func (q *Queries) UpdateAcmeChallengePending(ctx context.Context, arg UpdateAcmeChallengePendingParams) error {
	_, err := q.db.ExecContext(ctx, updateAcmeChallengePending,
		arg.Status,
		arg.Token,
		arg.Authorization,
		arg.UpdatedAt,
