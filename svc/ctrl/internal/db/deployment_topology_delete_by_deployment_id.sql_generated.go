// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: deployment_topology_delete_by_deployment_id.sql

package db

import (
	"context"
)

const deleteDeploymentTopologyByDeploymentId = `-- name: DeleteDeploymentTopologyByDeploymentId :exec
DELETE FROM ` + "`" + `deployment_topology` + "`" + `
WHERE deployment_id = ?
`

// DeleteDeploymentTopologyByDeploymentId
//
//	DELETE FROM `deployment_topology`
//	WHERE deployment_id = ?
func (q *Queries) DeleteDeploymentTopologyByDeploymentId(ctx context.Context, deploymentID string) error {
	_, err := q.db.ExecContext(ctx, deleteDeploymentTopologyByDeploymentId, deploymentID)
	return err
}
