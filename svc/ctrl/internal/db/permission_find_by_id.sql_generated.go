// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: permission_find_by_id.sql

package db

import (
	"context"
)

const findPermissionByID = `-- name: FindPermissionByID :one
SELECT pk, id, workspace_id, name, slug, description, created_at_m, updated_at_m
FROM permissions
WHERE id = ?
LIMIT 1
`

// Finds a permission record by its ID
// Returns: The permission record if found
//
//	SELECT pk, id, workspace_id, name, slug, description, created_at_m, updated_at_m
//	FROM permissions
//	WHERE id = ?
//	LIMIT 1
func (q *Queries) FindPermissionByID(ctx context.Context, permissionID string) (Permission, error) {
	row := q.db.QueryRowContext(ctx, findPermissionByID, permissionID)
	var i Permission
	err := row.Scan(
		&i.Pk,
		&i.ID,
		&i.WorkspaceID,
		&i.Name,
		&i.Slug,
		&i.Description,
		&i.CreatedAtM,
		&i.UpdatedAtM,
	)
	return i, err
}
