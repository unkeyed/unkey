// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: key_encryption_find_by_key_id.sql

package db

import (
	"context"
)

const findKeyEncryptionByKeyID = `-- name: FindKeyEncryptionByKeyID :one
SELECT pk, workspace_id, key_id, created_at, updated_at, encrypted, encryption_key_id FROM encrypted_keys WHERE key_id = ?
`

// FindKeyEncryptionByKeyID
//
//	SELECT pk, workspace_id, key_id, created_at, updated_at, encrypted, encryption_key_id FROM encrypted_keys WHERE key_id = ?
func (q *Queries) FindKeyEncryptionByKeyID(ctx context.Context, keyID string) (EncryptedKey, error) {
	row := q.db.QueryRowContext(ctx, findKeyEncryptionByKeyID, keyID)
	var i EncryptedKey
	err := row.Scan(
		&i.Pk,
		&i.WorkspaceID,
		&i.KeyID,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.Encrypted,
		&i.EncryptionKeyID,
	)
	return i, err
}
