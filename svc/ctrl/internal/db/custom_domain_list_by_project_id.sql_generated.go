...
//	WHERE project_id = ?
//	ORDER BY created_at DESC
func (q *Queries) ListCustomDomainsByProjectID(ctx context.Context, projectID string) ([]CustomDomain, error) {
	rows, err := q.db.QueryContext(ctx, listCustomDomainsByProjectID, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
