// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: custom_domain_update_failed.sql

package db

import (
	"context"
	"database/sql"
)

const updateCustomDomainFailed = `-- name: UpdateCustomDomainFailed :exec
UPDATE custom_domains
SET verification_status = ?,
    verification_error = ?,
    updated_at = ?
WHERE id = ?
`

type UpdateCustomDomainFailedParams struct {
	VerificationStatus CustomDomainsVerificationStatus `db:"verification_status"`
	VerificationError  sql.NullString                  `db:"verification_error"`
	UpdatedAt          sql.NullInt64                   `db:"updated_at"`
	ID                 string                          `db:"id"`
}

// UpdateCustomDomainFailed
//
//	UPDATE custom_domains
//	SET verification_status = ?,
//	    verification_error = ?,
//	    updated_at = ?
//	WHERE id = ?
func (q *Queries) UpdateCustomDomainFailed(ctx context.Context, arg UpdateCustomDomainFailedParams) error {
	_, err := q.db.ExecContext(ctx, updateCustomDomainFailed,
		arg.VerificationStatus,
		arg.VerificationError,
		arg.UpdatedAt,
		arg.ID,
	)
	return err
}
