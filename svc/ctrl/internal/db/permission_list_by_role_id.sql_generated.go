...
//	WHERE rp.role_id = ?
//	ORDER BY p.slug
func (q *Queries) ListPermissionsByRoleID(ctx context.Context, roleID string) ([]Permission, error) {
	rows, err := q.db.QueryContext(ctx, listPermissionsByRoleID, roleID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
