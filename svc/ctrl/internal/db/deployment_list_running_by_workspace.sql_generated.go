...
//	  AND d.desired_state = 'running'
//	  AND d.status NOT IN ('stopped', 'failed', 'cancelled', 'superseded', 'skipped')
func (q *Queries) ListRunningDeploymentsByWorkspaceId(ctx context.Context, workspaceID string) ([]ListRunningDeploymentsByWorkspaceIdRow, error) {
	rows, err := q.db.QueryContext(ctx, listRunningDeploymentsByWorkspaceId, workspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
