// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: identity_list_ratelimits_by_id.sql

package db

import (
	"context"
	"database/sql"
)

const listIdentityRatelimitsByID = `-- name: ListIdentityRatelimitsByID :many
SELECT pk, id, name, workspace_id, created_at, updated_at, key_id, identity_id, ` + "`" + `limit` + "`" + `, duration, auto_apply FROM ratelimits WHERE identity_id = ?
`

// ListIdentityRatelimitsByID
//
//	SELECT pk, id, name, workspace_id, created_at, updated_at, key_id, identity_id, `limit`, duration, auto_apply FROM ratelimits WHERE identity_id = ?
func (q *Queries) ListIdentityRatelimitsByID(ctx context.Context, identityID sql.NullString) ([]Ratelimit, error) {
	rows, err := q.db.QueryContext(ctx, listIdentityRatelimitsByID, identityID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Ratelimit
	for rows.Next() {
		var i Ratelimit
		if err := rows.Scan(
			&i.Pk,
			&i.ID,
			&i.Name,
			&i.WorkspaceID,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.KeyID,
			&i.IdentityID,
			&i.Limit,
			&i.Duration,
			&i.AutoApply,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
