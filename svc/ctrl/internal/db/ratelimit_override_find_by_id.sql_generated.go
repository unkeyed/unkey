// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: ratelimit_override_find_by_id.sql

package db

import (
	"context"
)

const findRatelimitOverrideByID = `-- name: FindRatelimitOverrideByID :one
SELECT pk, id, workspace_id, namespace_id, identifier, ` + "`" + `limit` + "`" + `, duration, created_at_m, updated_at_m, deleted_at_m FROM ratelimit_overrides
WHERE
    workspace_id = ?
    AND id = ?
`

type FindRatelimitOverrideByIDParams struct {
	WorkspaceID string `db:"workspace_id"`
	OverrideID  string `db:"override_id"`
}

// FindRatelimitOverrideByID
//
//	SELECT pk, id, workspace_id, namespace_id, identifier, `limit`, duration, created_at_m, updated_at_m, deleted_at_m FROM ratelimit_overrides
//	WHERE
//	    workspace_id = ?
//	    AND id = ?
func (q *Queries) FindRatelimitOverrideByID(ctx context.Context, arg FindRatelimitOverrideByIDParams) (RatelimitOverride, error) {
	row := q.db.QueryRowContext(ctx, findRatelimitOverrideByID, arg.WorkspaceID, arg.OverrideID)
	var i RatelimitOverride
	err := row.Scan(
		&i.Pk,
		&i.ID,
		&i.WorkspaceID,
		&i.NamespaceID,
		&i.Identifier,
		&i.Limit,
		&i.Duration,
		&i.CreatedAtM,
		&i.UpdatedAtM,
		&i.DeletedAtM,
	)
	return i, err
}
