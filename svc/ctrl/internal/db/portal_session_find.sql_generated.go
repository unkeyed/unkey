// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: portal_session_find.sql

package db

import (
	"context"
)

const findValidPortalSession = `-- name: FindValidPortalSession :one
SELECT pk, id, workspace_id, portal_config_id, external_id, permissions, preview, expires_at, created_at FROM portal_sessions
WHERE id = ?
  AND expires_at > ?
`

type FindValidPortalSessionParams struct {
	ID  string `db:"id"`
	Now int64  `db:"now"`
}

// FindValidPortalSession
//
//	SELECT pk, id, workspace_id, portal_config_id, external_id, permissions, preview, expires_at, created_at FROM portal_sessions
//	WHERE id = ?
//	  AND expires_at > ?
func (q *Queries) FindValidPortalSession(ctx context.Context, arg FindValidPortalSessionParams) (PortalSession, error) {
	row := q.db.QueryRowContext(ctx, findValidPortalSession, arg.ID, arg.Now)
	var i PortalSession
	err := row.Scan(
		&i.Pk,
		&i.ID,
		&i.WorkspaceID,
		&i.PortalConfigID,
		&i.ExternalID,
		&i.Permissions,
		&i.Preview,
		&i.ExpiresAt,
		&i.CreatedAt,
	)
	return i, err
}
