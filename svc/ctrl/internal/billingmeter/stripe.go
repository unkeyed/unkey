...
// meter IDs.
const (
	eventCPU    = "deploy.cpu_seconds"
	eventMemory = "deploy.memory_gib_seconds"
	eventEgress = "deploy.egress_public_gib"
	eventDisk   = "deploy.disk_gib_seconds"
)
...
func NewStripe(secretKey string) Pusher {
	return &stripePusher{client: stripeclient.New(secretKey, nil)}
}
...
func (p *stripePusher) SetMonthToDate(ctx context.Context, req PushRequest) (int, error) {
	...
	for _, e := range events {
		_, err := p.client.BillingMeterEvents.New(&stripe.BillingMeterEventParams{
