...
func NewGitHubWebhook(restateClient *restateingress.Client, webhookSecret string) http.Handler {
	...
	return webhook.New("github", githubverifier.New(webhookSecret)).
		On(webhook.Typed(s.handlePush), "push").
		On(webhook.Typed(s.handlePullRequest), "pull_request").
...
func (s *githubWebhook) handlePush(ctx context.Context, event webhook.Event, payload pushPayload) error {
...
func (s *githubWebhook) handlePullRequest(
	ctx context.Context,
	event webhook.Event,
