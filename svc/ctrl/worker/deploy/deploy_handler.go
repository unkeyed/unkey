...
func (w *Workflow) Deploy(ctx restate.WorkflowContext, req *hydrav1.DeployRequest) (_ *hydrav1.DeployResponse, retErr error) {
	...
	// superseded while it waited, and a cancel can land during the build. The
	...
	}, restate.WithName("loading deployment, assigning workspace namespace"), restate.WithMaxRetryAttempts(runMaxAttempts))
	if err != nil {
		return nil, fault.Wrap(err, fault.Public("Failed to read from database. Please try again."))
	}
	if deployment.Status.IsTerminal() {
...
func (w *Workflow) reserveTopologies(ctx context.Context, req reserveTopologiesRequest) (topologyReservation, error) {
	return db.TxWithResultRetry(ctx, w.db.RW(), func(txCtx context.Context, tx db.DBTX) (topologyReservation, error) {
		...
		changes := make([]db.InsertDeploymentChangeParams, 0, len(topologies))
		for i := range topologies {
			topologies[i].CreatedAt = now
			changes = append(changes, db.InsertDeploymentChangeParams{
				ResourceType: db.DeploymentChangesResourceTypeDeploymentTopology,
				ResourceID:   topologies[i].DeploymentID,
				RegionID:     topologies[i].RegionID,
				CreatedAt:    now,
			})
		}
		bulk := db.NewBulkQueries(tx)
		if err := bulk.InsertDeploymentTopologies(txCtx, topologies); err != nil {
			return topologyReservation{}, err
		}
		return topologyReservation{}, bulk.InsertDeploymentChanges(txCtx, changes)
	})
}
