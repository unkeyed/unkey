...
const (
	...
	testReferencedTag      = "proj_referenced-d_referenced"
	testOrphanTag          = "proj_orphan-d_orphan"
	testChangedTag         = "proj_changed-d_changed"
	testRestoredTag        = "proj_restored-d_restored"
)
...
func TestDeploymentGarbageCollection_RetainsHistoryAndReconcilesDepot(t *testing.T) {
	...
	depot := newFakeDepot(
		[]deploymentgc.DepotImage{
			...
			{Tag: "proj_young-d_young", Digest: "sha256:young", PushedAt: young},
			{Tag: "proj_nodigest-d_nodigest", PushedAt: old},
			{Tag: "proj_notime-d_notime", Digest: "sha256:notime"},
			{Tag: "foreign", Digest: "sha256:foreign", PushedAt: old},
		},
		[]deploymentgc.DepotProject{
			{ID: "depot-referenced", Name: testProjectPrefix + "-proj_referenced", CreatedAt: old},
			{ID: "depot-transient", Name: testProjectPrefix + "-proj_transient", CreatedAt: old},
			{ID: "depot-orphan", Name: testProjectPrefix + "-proj_orphan", CreatedAt: old},
			{ID: "depot-restored", Name: testProjectPrefix + "-proj_restored", CreatedAt: old},
			{ID: "depot-young", Name: testProjectPrefix + "-proj_young", CreatedAt: young},
			{ID: "depot-no-time", Name: testProjectPrefix + "-proj_notime"},
			{ID: "registry-project", Name: testProjectPrefix + "-proj_registry", CreatedAt: old},
	...
	require.Equal(t, int32(2), response.GetDepotResourcesDeleted())
...
func setDeploymentDesiredState(
	t *testing.T,
	h *harness.Harness,
	deploymentID string,
