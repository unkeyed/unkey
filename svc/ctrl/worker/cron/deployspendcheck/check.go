...
// highest crossed), at most once per threshold per period. The cap is on total
// metered spend, credits included. It reads no usage itself: the orchestrator
// prices every workspace from one ClickHouse scan and only dispatches the ones
// near or over budget, so this handler is pure state transition plus email.
