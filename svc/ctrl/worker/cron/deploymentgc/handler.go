...
func (h *Handler) Handle(
	...
	_ *hydrav1.RunDeploymentGarbageCollectionRequest,
) (*hydrav1.RunDeploymentGarbageCollectionResponse, error) {
	...
	projectsDeleted, err := h.deleteOrphanProjects(
		ctx,
		now,
		databaseProjects,
		depotProjects,
		maxDepotResourceDeletes-imagesDeleted,
	)
	if err != nil {
		return nil, err
	}
...
func (h *Handler) loadDatabaseInventory(ctx restate.ObjectContext) (map[string]struct{}, []projectReference, error) {
	...
	projects := make([]projectReference, 0)
	cursor = 0
	for {
...
func (h *Handler) clearStaleProjectReferences(
	ctx restate.ObjectContext,
	now time.Time,
	databaseProjects []projectReference,
	depotProjects map[string]DepotProject,
) (int32, error) {
	var cleared int32
...
func (h *Handler) deleteOrphanImages(
	ctx restate.ObjectContext,
	now time.Time,
	databaseImages map[string]struct{},
	depotImages map[string]DepotImage,
	limit int32,
) (int32, error) {
	if limit <= 0 {
		return 0, nil
	...
	for _, tag := range candidateTags {
		...
		if !exists || !isManagedImageTag(image.Tag) || image.Digest != candidate.Digest {
			continue
		}
		...
		referenced, lookupErr := restate.Run(ctx, func(runCtx restate.RunContext) (bool, error) {
			return h.db.DeploymentImageExists(runCtx, sql.NullString{
				String: h.registryRepository + ":" + tag,
				Valid:  true,
			})
		}, restate.WithName("recheck Depot image reference"), restate.WithMaxRetryAttempts(runMaxAttempts))
		if lookupErr != nil {
			return deleted, fmt.Errorf("recheck Depot image %s: %w", tag, lookupErr)
		}
		if referenced {
			continue
		}
...
func isManagedImageTag(tag string) bool {
	projectID, deploymentID, ok := strings.Cut(tag, "-")
	return ok && !strings.Contains(deploymentID, "-") &&
		isResourceID(projectID, uid.ProjectPrefix) && isResourceID(deploymentID, uid.DeploymentPrefix)
}

func (h *Handler) deleteOrphanProjects(
	ctx restate.ObjectContext,
	now time.Time,
	databaseProjects []projectReference,
	depotProjects map[string]DepotProject,
