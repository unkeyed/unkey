...
// New opens a writable MySQL replica connection and returns a [Querier] ready
// for use. The connection is created with mode "rw" so tracing and metrics
// labels distinguish worker primary traffic from read-only consumers.
//
...
func New(url string) (Querier, func() error, error) {
	replica, err := mysql.NewReplica(url, "rw")
	if err != nil {
		return nil, nil, err
	}
