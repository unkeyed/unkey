...
func (s *Service) GetDeployment(
	...
	if err != nil {
		...
		logger.Error("failed to load deployment", "error", err, "deployment_id", req.Msg.GetDeploymentId())
...
func convertDbStatusToProto(status db.DeploymentsStatus) ctrlv1.DeploymentStatus {
	...
	case db.DeploymentsStatusRejected:
		return ctrlv1.DeploymentStatus_DEPLOYMENT_STATUS_REJECTED
	default:
