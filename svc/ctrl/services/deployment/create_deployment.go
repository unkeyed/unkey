...
func (s *Service) createAndDeploy(ctx context.Context, p createParams) (string, error) {
	...
	case c.app.SourceType == db.AppsSourceTypeDockerImage:
		...
		if dockerErr != nil {
			...
			return "", connect.NewError(connect.CodeInternal,
				fmt.Errorf("failed to load Docker source: %w", dockerErr))
		}
		imageReference, imageErr := imageref.Normalize(dockerSource.ImageReference)
		if imageErr != nil {
			return "", connect.NewError(connect.CodeFailedPrecondition,
	...
	default:
		imageReference, dockerErr := buildDockerSource(ctx, s.db, c.app, deploymentID)
		if dockerErr != nil {
			return "", dockerErr
		}
		commit = commitFields{ //nolint:exhaustruct
	...
	if useGit {
		...
		if buildErr != nil {
			if db.IsNotFound(buildErr) {
				return "", connect.NewError(connect.CodeFailedPrecondition,
					fmt.Errorf("GitHub-sourced app %q has no build settings for environment %q", c.app.ID, c.env.Environment.Slug))
			}
	...
	err := s.db.InsertDeployment(ctx, db.InsertDeploymentParams{
		...
		RequestedImage:                sql.NullString{String: requestedImage, Valid: requestedImage != ""},
...
// buildDockerSource looks up the app's current deployment's Docker image for a
