...
func (s *Service) CreateDeployment(
	...
	// Populate git fields from caller-provided info. Empty branch falls back
	// to the app's default branch so git-connected redeploys without a branch
	// still have something to resolve HEAD against.
	var commit commitFields
	commit.Branch = branchFromGitCommit(req.Msg.GetGitCommit(), appWithSettings.App.DefaultBranch)
	...
	// Fill missing commit metadata synchronously so the deployment row has
	// complete info at insert time. buildImage no longer fetches these.
	...
	case hasRepoConnection:
		// Git-connected app: build from git. SHA is guaranteed non-empty by
		// the fill step above.
		deployReq = &hydrav1.DeployRequest{
			DeploymentId: deploymentID,
			KeyAuthId:    keyAuthID,
	...
	default:
		// No docker image, no repo connection: reuse current deployment's image.
...
func (cf *commitFields) fillFromGitHub(
	...
	hasAuth := !allowUnauth || installationID != 0
