...
func (s *Service) CreateProject(
	...
	// Deploy entitlement gate. This is the authoritative check: CLI, API, and
	// git-triggered deploys bypass the dashboard, so the real enforcement lives
	// here. In observe mode (the default) it only logs when it would block, so
	// we can validate the signal before enforcing.
