...
func (s *Service) UpdateOciImageSource(
	ctx context.Context,
	req *connect.Request[ctrlv1.UpdateOciImageSourceRequest],
) (*connect.Response[ctrlv1.UpdateOciImageSourceResponse], error) {
	...
	if app.SourceType != db.AppsSourceTypeOci {
		return nil, connect.NewError(connect.CodeFailedPrecondition,
			fmt.Errorf("app %q is not OCI-sourced", app.ID))
	}

	currentSource, err := s.db.FindAppSourceOciByAppId(ctx, app.ID)
	if err != nil {
		if db.IsNotFound(err) {
			return nil, connect.NewError(connect.CodeFailedPrecondition,
				fmt.Errorf("OCI-sourced app %q has no source configuration", app.ID))
		}
		return nil, connect.NewError(connect.CodeInternal, fmt.Errorf("find OCI source: %w", err))
	}
	...
	err = db.TxRetry(ctx, s.db.RW(), func(txCtx context.Context, tx db.DBTX) error {
		...
		if txErr := queries.UpdateAppSourceOciImageReference(txCtx, db.UpdateAppSourceOciImageReferenceParams{
			...
			return fmt.Errorf("update OCI source: %w", txErr)
		}
		...
		if txErr := s.auditlogs.Insert(txCtx, tx, []auditlog.AuditLog{
			{
				...
				Display:       fmt.Sprintf("Updated OCI image source for app %s", app.ID),
				ActorID:       a.GetId(),
				ActorName:     a.GetName(),
	...
	return connect.NewResponse(&ctrlv1.UpdateOciImageSourceResponse{
		ImageReference: imageReference,
	}), nil
}
