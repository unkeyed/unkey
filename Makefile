...
# ─────────────────────────────────────────────────────────────
# OpenAPI Validation Testing (Schemathesis via Docker)
# ─────────────────────────────────────────────────────────────

BASE_URL ?= http://host.docker.internal:7070
TOKEN ?=

.PHONY: schemathesis
schemathesis: ## Run Schemathesis tests against local API (usage: make schemathesis TOKEN=unkey_xxx)
	docker run --rm \
		-v $(CURDIR)/svc/api/openapi:/specs:ro \
		schemathesis/schemathesis \
		run /specs/openapi-generated.yaml \
		--url=$(BASE_URL) \
		--checks all \
		$(if $(TOKEN),--header="Authorization: Bearer $(TOKEN)",)

.PHONY: schemathesis-verbose
schemathesis-verbose: ## Run Schemathesis with all phases (examples, coverage, fuzzing)
	docker run --rm \
		-v $(CURDIR)/svc/api/openapi:/specs:ro \
		schemathesis/schemathesis \
		run /specs/openapi-generated.yaml \
...
.PHONY: schemathesis-quick
schemathesis-quick: ## Run Schemathesis quick test (examples only)
	docker run --rm \
		-v $(CURDIR)/svc/api/openapi:/specs:ro \
		schemathesis/schemathesis \
		run /specs/openapi-generated.yaml \
...
test-validation: ## Run validation package tests only
	bazel test //pkg/zen/validation:validation_test --test_output=errors
