...
generate-schema: ## Generate TypeScript schema from pkg/mysql/schema.sql via drizzle-kit introspect
	@echo "Building MySQL OCI image..."
	@bazel run //:oci_load_mysql 2>&1 | tail -1
	@echo "Starting MySQL container..."
	@docker rm -f unkey-schema-gen 2>/dev/null || true
	@docker run -d --rm --name unkey-schema-gen \
		-e MYSQL_ROOT_PASSWORD=root \
		-e MYSQL_DATABASE=unkey \
		-p 23306:3306 \
		unkey/mysql:latest >/dev/null
	@(sleep 120 && docker stop unkey-schema-gen >/dev/null 2>&1) &
	@echo "Waiting for MySQL to be ready..."
	@for i in $$(seq 1 60); do \
		mysql -h 127.0.0.1 -P 23306 -uroot -proot unkey -e "SHOW TABLES" 2>/dev/null | grep -q apis && break; \
		if [ $$i -eq 60 ]; then echo "MySQL failed to start"; docker rm -f unkey-schema-gen >/dev/null; exit 1; fi; \
		sleep 1; \
	done
	@echo "Introspecting schema..."
	@mkdir -p ./web/internal/db/src/schema/generated
	@cd web/internal/db && pnpm drizzle-kit introspect \
		--dialect=mysql --url="mysql://root:root@localhost:23306/unkey" \
		--out=src/schema/generated
	@mv ./web/internal/db/src/schema/generated/schema.ts ./web/internal/db/src/schema/generated/tables.ts
	@rm -f ./web/internal/db/src/schema/generated/relations.ts
	@rm -rf ./web/internal/db/src/schema/generated/meta
	@rm -f ./web/internal/db/src/schema/generated/*.sql
	@echo "Stopping MySQL container..."
	@docker rm -f unkey-schema-gen >/dev/null
	@echo "Done."
...
.PHONY: generate
generate: generate-schema ## Generate code from protobuf and other sources
	rm -rf ./gen || true
