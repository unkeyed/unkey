...
func TestSWRWithFallback_RevalidatesStaleHigherPriorityKey(t *testing.T) {
	ctx := context.Background()
	mockClock := clock.NewTestClock()
	c, err := cache.New(cache.Config[string, string]{
		Fresh:    time.Minute,
	...
	c.Set(ctx, "exact", "stale exact")
	mockClock.Tick(2 * time.Minute)
	c.Set(ctx, "wildcard", "fresh wildcard")

	revalidated := make(chan struct{})
	value, hit, err := c.SWRWithFallback(
		ctx,
		[]string{"exact", "wildcard"},
		func(context.Context) (string, string, error) {
			close(revalidated)
			return "fresh exact", "exact", nil
		},
		func(error) cache.Op { return cache.WriteValue },
	)
	require.NoError(t, err)
	require.Equal(t, cache.Hit, hit)
	require.Equal(t, "stale exact", value)
