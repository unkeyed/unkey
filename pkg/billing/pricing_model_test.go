...
func TestCalculateCharges_FlatPricing(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := service.CalculateCharges(ctx, tt.model, tt.verifications, tt.rateLimits)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
...
func TestPricingModelService_Validation(t *testing.T) {
	...
	t.Run("rejects nil model in CalculateCharges", func(t *testing.T) {
		_, err := service.CalculateCharges(ctx, nil, 100, 100)
		require.Error(t, err)
	})
	...
	t.Run("rejects negative verifications", func(t *testing.T) {
		...
		_, err := service.CalculateCharges(ctx, model, -1, 100)
		require.Error(t, err)
	})

	t.Run("rejects negative rate limits", func(t *testing.T) {
		model := &PricingModel{
			VerificationUnitPrice: 100,
			RatelimitUnitPrice:    50,
		}
		_, err := service.CalculateCharges(ctx, model, 100, -1)
		require.Error(t, err)
	})
}
