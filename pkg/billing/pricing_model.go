...
type PricingModelService interface {
	...
	CalculateCharges(ctx context.Context, model *PricingModel, verifications, rateLimits int64) (int64, error)
}
...
func (s *pricingModelService) CreatePricingModel(
	...
	if req.RatelimitUnitPrice < 0 {
		return nil, fault.Wrap(
			fmt.Errorf("ratelimit unit price cannot be negative"),
			fault.Code(codes.App.Validation.InvalidInput.URN()),
			fault.Public("Rate limit unit price must be non-negative"),
		)
	}
	...
	insertParams := db.PricingModelInsertParams{
		...
		RatelimitUnitPrice:    req.RatelimitUnitPrice,
...
func (s *pricingModelService) CalculateCharges(
	ctx context.Context,
	model *PricingModel,
	verifications, rateLimits int64,
) (int64, error) {
	if model == nil {
		return 0, fault.Wrap(
	...
	if rateLimits < 0 {
		return 0, fault.Wrap(
			fmt.Errorf("rate limits cannot be negative"),
			fault.Code(codes.App.Validation.InvalidInput.URN()),
			fault.Public("Rate limit count must be non-negative"),
		)
	}
...
func dbModelToPricingModel(dbModel *db.PricingModel) (*PricingModel, error) {
	...
	return &PricingModel{
		...
		VerificationUnitPrice: dbModel.VerificationUnitPrice,
		RatelimitUnitPrice:    dbModel.RatelimitUnitPrice,
		TieredPricing:         tieredPricing,
