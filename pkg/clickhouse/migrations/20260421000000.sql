...
-- aggregates:
--   - per_day_v1  : 365 day TTL, for month-over-month trend dashboards
--   - per_month_v1: 5 year TTL, for long-range trend charts + audit
--
-- Both read raw directly (flat cascade). DASHBOARDS / AUDIT ONLY:
-- sample_count double-counts duplicates (MVs fire before
-- ReplacingMergeTree dedupe), so billing code still has to hit the raw
-- table with FINAL — not these MVs. See metrics-architecture.mdx.
