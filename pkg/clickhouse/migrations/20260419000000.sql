...
-- collapsed by ReplacingMergeTree. Retries of the same row carry identical
-- (workspace_id, container_uid, ts, event_kind) by construction, so dedup
-- still works for retries; only distinct kinds at the same ms survive.
...
-- Per-day dashboard MV (reads raw directly) ─────────────────────────────
-- 365 day TTL, for month-over-month trend dashboards.
...
-- Per-month dashboard MV (reads raw directly) ────────────────────────────
-- 5 year TTL, for long-range trend charts + audit. Year-partitioned so
-- year-over-year queries hit a small number of parts.
