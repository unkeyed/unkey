...
CREATE TABLE instance_checkpoints_v1 (
  ...
  -- Network byte counters, monotonic per container_uid. Written by the
  -- per-pod eBPF TCX programs (svc/heimdall/internal/network); public vs
  -- private split classifies on destination IP (RFC1918 + link-local +
  -- loopback = private). Billing: max(counter) - min(counter). Delta+ZSTD
  -- like CPU. Zero when the eBPF reader is unavailable (macOS dev stub)
  -- or attach hasn't completed yet — see attributes.network_attached to
  -- tell "no traffic" apart from "not measured".
...
-- event_kind is in the dedup key so a same-millisecond collision between a
-- periodic checkpoint and a lifecycle event (start/stop) doesn't get
-- collapsed. Retries of the same row carry identical
-- (workspace_id, container_uid, ts, event_kind) tuples by construction, so
-- dedup still works for retries; only distinct kinds at the same ms survive.
ORDER BY (workspace_id, container_uid, ts, event_kind)
