...
type Email struct {
	...
	// template exactly. A missing variable renders the template's declared
	// fallback value (the sync script declares one for every variable); the
	// provider only rejects the send when a missing variable has no fallback.
	Variables map[string]string
	// Subject overrides the template's default subject when set.
	Subject string
	// IdempotencyKey dedupes sends at Resend when a caller retries after a
	// transient failure (1–256 chars; empty omits the header). Keys expire
	// after 24 hours.
