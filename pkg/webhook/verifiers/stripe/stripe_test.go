...
func TestVerifier(t *testing.T) {
	...
	t.Run("valid signature yields the unwrapped event", func(t *testing.T) {
		event, err := New(secret).Verify(request(sign(secret, body, time.Now())), []byte(body))
	...
	t.Run("wrong secret is rejected", func(t *testing.T) {
		_, err := New(secret).Verify(request(sign("whsec_other", body, time.Now())), []byte(body))
		require.Error(t, err)
	})

	t.Run("stale timestamp is rejected", func(t *testing.T) {
		_, err := New(secret).Verify(
			request(sign(secret, body, time.Now().Add(-time.Hour))), []byte(body))
		require.Error(t, err)
	})
}
