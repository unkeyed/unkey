...
//
// Panics if the clock was constructed via the zero value (`var c
// clock.MonotonicClock`) instead of NewMonotonic — without the anchor,
// time.Since against a zero time.Time produces nonsense timestamps that
// would silently corrupt billing math. Fail fast at the call site.
func (c *MonotonicClock) Now() time.Time {
	if c.anchor.IsZero() {
		panic("clock.MonotonicClock used with zero anchor; construct via clock.NewMonotonic()")
	}
