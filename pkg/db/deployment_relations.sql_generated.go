...
// versions:
//   sqlc v1.30.0
// source: deployment_relations.sql
...
const findDeploymentRelations = `-- name: FindDeploymentRelations :one
SELECT
...
type FindDeploymentRelationsRow struct {
...
//	JOIN apps a ON a.id = d.app_id
//	WHERE d.id = ?
func (q *Queries) FindDeploymentRelations(ctx context.Context, db DBTX, id string) (FindDeploymentRelationsRow, error) {
	row := db.QueryRowContext(ctx, findDeploymentRelations, id)
	var i FindDeploymentRelationsRow
	err := row.Scan(&i.EnvironmentSlug, &i.AppCurrentDeploymentID, &i.AppIsRolledBack)
	return i, err
}

const listDeploymentRelations = `-- name: ListDeploymentRelations :many
SELECT
...
type ListDeploymentRelationsRow struct {
...
//	JOIN apps a ON a.id = d.app_id
//	WHERE d.id IN (/*SLICE:deployment_ids*/?)
func (q *Queries) ListDeploymentRelations(ctx context.Context, db DBTX, deploymentIds []string) ([]ListDeploymentRelationsRow, error) {
	query := listDeploymentRelations
	...
	defer rows.Close()
	var items []ListDeploymentRelationsRow
	for rows.Next() {
		var i ListDeploymentRelationsRow
		if err := rows.Scan(
			&i.DeploymentID,
			&i.EnvironmentSlug,
