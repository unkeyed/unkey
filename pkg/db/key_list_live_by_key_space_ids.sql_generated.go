...
// versions:
//   sqlc v1.30.0
// source: key_list_live_by_key_space_ids.sql
...
type ListLiveKeysByKeySpaceIDsParams struct {
...
type ListLiveKeysByKeySpaceIDsRow struct {
	Pk                 uint64         `db:"pk"`
	ID                 string         `db:"id"`
...
//	ORDER BY k.id ASC
//	LIMIT ?
func (q *Queries) ListLiveKeysByKeySpaceIDs(ctx context.Context, db DBTX, arg ListLiveKeysByKeySpaceIDsParams) ([]ListLiveKeysByKeySpaceIDsRow, error) {
	query := listLiveKeysByKeySpaceIDs
	...
	queryParams = append(queryParams, arg.IdentityID)
	queryParams = append(queryParams, arg.IdentityID)
	...
	defer rows.Close()
	var items []ListLiveKeysByKeySpaceIDsRow
	for rows.Next() {
		var i ListLiveKeysByKeySpaceIDsRow
		if err := rows.Scan(
			&i.Pk,
			&i.ID,
			&i.KeyAuthID,
