...
func TestRuntimeViolations(t *testing.T) {
	...
	t.Run("port bounds", func(t *testing.T) {
		...
		require.Len(t, low, 1)
		require.Equal(t, MsgPortOutOfRange, low[0].Message)
		require.Equal(t, int32(0), low[0].Actual)

		high := RuntimeViolations(65536, 250, 256)
		require.Len(t, high, 1)
		require.Equal(t, MsgPortOutOfRange, high[0].Message)
		require.Equal(t, int32(65536), high[0].Actual)
	})
	...
	t.Run("reports every failing field at once", func(t *testing.T) {
		...
		require.Equal(t, []string{MsgPortOutOfRange, MsgCPUTooLow, MsgMemoryTooLow}, messages)
	})
}
